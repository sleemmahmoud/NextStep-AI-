...
رد بصيغة JSON بس من غير أي كلام زيادة قبله أو بعده ومن غير أي علامات markdown، بالشكل ده بالظبط: {"goal":"النص المحسّن للهدف","achievements":"سطر1\\nسطر2\\nسطر3"}`;
    ...
    const reason = (err && err.name==="AbortError") ? "الـAI استغرق أكتر من 25 ثانية ومردش" : "مشكلة شبكة أثناء الاتصال بالـAI";
    return {profile:p, polished:false, reason}; // أي مشكلة (شبكة، JSON مش مظبوط...) نرجع للنص الأصلي بدل ما نوقف الـCV
  }
}
...
لو مش متأكد إن الفرصة حقيقية، متضيفهاش. لو مفيش فرص كفاية رجّع [].`;
  try{
    const controller = new AbortController();
    // مهلة أطول من الشات (55 ثانية) لإن الطلب ده بيولّد لحد 5 فرص بتفاصيلها كـJSON
    // منظّم، وده بياخد وقت أطول من رد شات عادي.
    ...
    if(err && err.name==="AbortError") return {error:"البحث استغرق وقت طويل جدًا (أكتر من 55 ثانية) والـAI مردش، جرب تاني — ممكن يكون في ضغط مؤقت على الخدمة، أو جرب موضوع بحث أبسط."};
