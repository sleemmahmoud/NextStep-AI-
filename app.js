...
// حد منفصل لعدد مرات عمل الـCV في اليوم، لكل مستخدم لوحده — كل نسخة (تصميم
// مميز / ATS) بعداد مستقل عن التانية بالكامل، يعني مسموح بواحد تصميم وواحد
// ATS في نفس اليوم من غير ما استخدام واحد يأثر على التاني.
...
// دالة جديدة منفصلة تمامًا عن البحث عن الفرص فوق — بتدوّر على كورسات حقيقية
// بس (Coursera, edX, FutureLearn... إلخ) وتضيفها في مصادر التعلّم (resources)
// بدل الفرص (opportunities). بتستخدم بحث جوجل الحي عشان تتأكد إن الكورس لسه متاح.
async function fetchAiCourses(topic){
  const todayStr = new Date().toISOString().slice(0,10);
  const prompt = `النهاردة تاريخ ${todayStr}. استخدم بحث جوجل الحقيقي والحي دلوقتي عشان تلاقي أشهر وأهم الكورسات المجانية أو شبه المجانية **الحقيقية والمعروفة والمتاحة فعلاً دلوقتي** (بحد أقصى 12 كورس) عن موضوع: "${topic}"، مناسبة لطلاب أو خريجين مصريين.
...
شروط أساسية:
- افتح فعليًا (عن طريق البحث) صفحة كل كورس قبل ما ترشحه، وتأكد إنه لسه متاح ومنشور فعلاً دلوقتي مش صفحة قديمة اتشالت.
- الرابط لازم يكون رابط حقيقي مباشر لصفحة الكورس نفسه من نتيجة البحث بتاعتك، مش رابط عام للمنصة.
- حدد بصراحة "isPremium": true لو المحتوى نفسه محتاج اشتراك مدفوع، أو false لو مجاني بالكامل أو متاح بنظام Audit مجاني.
- صنّف كل كورس بـ"tags" — اختار من القايمة دي بالظبط: [${TAGS.map(t=>`"${t}"`).join(", ")}]
- الوصف بالعربي دايمًا حتى لو الكورس بالإنجليزي.
رجّع بس JSON array بالشكل ده بالظبط:
[{"title":"عنوان الكورس + اسم المنصة بين قوسين","description":"وصف قصير من سطرين بالعربي","link":"رابط صفحة الكورس المباشر","tags":["وسم1"],"isPremium":false}]
لو مش متأكد إن الكورس حقيقي ومتاح فعلًا دلوقتي، متضيفهوش خالص. لو مفيش كورسات موثوقة كفاية للموضوع ده رجّع [].`;
  try{
    const res = await fetch(GEMINI_PROXY_URL, {
      method:"POST",
      headers:{"Content-Type":"application/json"},
    ...
    const gm = cand && cand.groundingMetadata;
    const sources = (gm && gm.groundingChunks || []).map(c=>c.web).filter(Boolean);
    // ملحوظة: uri الرجعة من groundingChunks بتكون رابط تحويل بتاع Google مش
    // الدومين الحقيقي، فالاسم الحقيقي للمصدر موجود في title بس.
...
// بتحفظ في resources (مش opportunities)، وبتتأكد إن الرابط مش موجود قبل كده
// (dedupe) عشان منكررش نفس الكورس لو دورت على نفس الموضوع تاني.
