...
function generateCertId(recipientName, itemTitle, dateStr){
  const raw = `${recipientName}|${itemTitle}|${dateStr}`;
  let hash = 0;
...
function certificateHtml(recipientName, itemLabel, itemTitle, bodyText, dateStr){
  // تصميم مستوحى من شهادات Coursera/Google Career Certificates/Microsoft Learn:
  // خلفية بيضاء نضيفة، ألوان المنصة (تركواز غامق + أخضر) بدل الذهبي التقيل،
  // برواز رفيع مش تقيل، ورقم شهادة + QR للمظهر الاحترافي اللي يصلح للـCV/LinkedIn.
  // اسم المتدرب وعنوان الكورس بيتكتبوا مباشرة من الكود (مش من نص الـAI) عشان
  // يبقوا مضمونين 100% صح.
  const certId = generateCertId(recipientName, itemTitle, dateStr);
  const qrPayload = encodeURIComponent(`NextStep AI Certificate\n${recipientName}\n${itemTitle}\nID: ${certId}\nDate: ${dateStr}`);
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&margin=0&data=${qrPayload}`;
  return `
  <div style="width:1200px;height:850px;background:#FFFFFF;position:relative;box-sizing:border-box;font-family:'Cairo',Tahoma,Arial,sans-serif;color:#1A2B2E;">
    ...
    <div style="position:absolute;inset:0;border:1px solid #DCE3E1;margin:22px;"></div>

    <div style="position:absolute;top:56px;left:0;right:0;display:flex;align-items:center;justify-content:center;gap:10px;">
      <div style="width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,#0A2C35,#1F7A47);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:16px;">N</div>
      <div style="font-family:'Montserrat',sans-serif;font-weight:800;font-size:20px;letter-spacing:1.5px;color:#0A2C35;">NextStep AI</div>
    </div>

    <div style="position:absolute;top:34px;right:48px;text-align:right;">
      <div style="font-size:10.5px;color:#8C9A9C;">Certificate ID</div>
      <div style="font-size:12.5px;font-weight:700;color:#0A2C35;letter-spacing:.5px;">${certId}</div>
    </div>
    <div style="position:absolute;top:34px;left:48px;display:flex;align-items:center;gap:6px;background:#EAF6EF;border-radius:20px;padding:5px 12px;">
      <span style="color:#1F7A47;font-size:13px;">✓</span>
      <span style="font-size:11px;font-weight:700;color:#1F7A47;">شهادة صادرة رقميًا — Digitally Issued</span>
    </div>

    <div style="position:absolute;top:150px;left:0;right:0;text-align:center;padding:0 100px;">
      <div style="font-size:12px;letter-spacing:4px;color:#1F7A47;font-weight:700;">CERTIFICATE OF COMPLETION</div>
      <div style="font-size:38px;font-weight:800;margin:10px 0 4px;color:#0A2C35;">شهادة إتمام</div>
      <div style="width:70px;height:3px;background:linear-gradient(90deg,#0A2C35,#1F7A47);margin:0 auto 26px;border-radius:2px;"></div>

      <div style="font-size:14.5px;color:#5B6B6E;">تُمنح هذه الشهادة إلى</div>
      <div style="font-family:'Amiri','Cairo',serif;font-size:42px;font-weight:800;color:#0A2C35;margin:10px 0;">${escapeHtml(recipientName)}</div>
      <div style="width:320px;height:2px;background:#1F7A47;margin:0 auto 22px;"></div>

      <div style="font-size:15.5px;color:#3A4749;line-height:1.7;">تقديرًا لإتمامه بنجاح ${itemLabel} <strong>"${escapeHtml(itemTitle)}"</strong></div>
      <div style="font-size:14px;color:#5B6B6E;max-width:720px;margin:12px auto 0;line-height:1.8;">${escapeHtml(bodyText)}</div>
    </div>

    <div style="position:absolute;bottom:54px;left:70px;text-align:center;">
      <img src="${qrUrl}" width="72" height="72" style="border-radius:6px;border:1px solid #DCE3E1;" crossorigin="anonymous">
      <div style="font-size:10px;color:#8C9A9C;margin-top:6px;">امسح لعرض تفاصيل الشهادة</div>
    </div>

    <div style="position:absolute;bottom:44px;left:50%;transform:translateX(-50%);text-align:center;">
      <div style="font-size:13px;font-weight:700;color:#1A2B2E;">${dateStr}</div>
      <div style="width:110px;height:2px;background:#DCE3E1;margin:6px auto 4px;"></div>
      <div style="font-size:11.5px;color:#5B6B6E;">تاريخ الإصدار — Date Issued</div>
    </div>

    <div style="position:absolute;bottom:44px;right:70px;text-align:center;">
      <div style="font-family:'Brush Script MT','Segoe Script',cursive;font-size:28px;color:#0A2C35;">Sleem Mahmoud</div>
      <div style="width:170px;height:2px;background:#1F7A47;margin:4px auto 6px;"></div>
      <div style="font-size:11.5px;color:#5B6B6E;">سليم محمود — المؤسس والمدرب الرئيسي</div>
      <div style="font-size:10px;color:#8C9A9C;">Founder &amp; Lead Trainer</div>
    </div>

    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-58%);width:70px;height:70px;border-radius:50%;background:radial-gradient(circle at 32% 30%, #2A9D5C, #0A2C35);opacity:.06;"></div>
  </div>`;
}
async function downloadCertificate(quizId){
  ...
  const html = certificateHtml(state.profile.name.trim(), "اختبار", quiz.title, bodyText, dateStr);
...
async function downloadCourseCertificate(resourceId){
  ...
  try{
    ...
    const bodyText = await generateCertificateText(r.title, state.profile.name.trim());
    const html = certificateHtml(state.profile.name.trim(), "كورس", r.title, bodyText, dateStr);
...
function renderAiSearchPanel(){
  ...
  <div class="card" style="padding:14px;margin-bottom:10px;">
    ...
    ${(item.deadline && item.deadline!=="غير معلن")?`<div class="opp-deadline normal">آخر موعد (حسب البحث): ${escapeHtml(item.deadline)}</div>`:`<div class="opp-deadline urgent">⚠️ الموعد مش واضح — افتح الرابط وتأكد بنفسك</div>`}
...
function renderResourcesTab(){
  ...
  const bySection = {
    tips: state.resources.filter(r=>resourceSection(r)==="tips"),
    courses: state.resources.filter(r=>resourceSection(r)==="courses"),
    practical: state.resources.filter(r=>resourceSection(r)==="practical")
  };
  ...
  for(let attempt=0; attempt<=retries; attempt++){
    ...
    try{
      const res = await fetch(url, {
        ...
        body: JSON.stringify({...body, uid: state.user && state.user.uid}),
...
function localAssistantReply(rawMessage){
  ...
  const p = state.profile || {};
  const has = (...words) => words.some(w=>message.includes(w) || lower.includes(w));
  ...
  if(has("سلام","اهلا","أهلا","هاي","هلا","صباح الخير","مساء الخير")){
    ...
    return `أهلاً ${firstName||""}! 👋 اسألني عن "فرص مناسبة ليا"، "إزاي أحسّن الـCV"، أو "عايز أتعلم إيه" وهرد عليك فورًا.`;
  }
  ...
  if(has("مين انت","انت مين","بوت","ذكاء اصطناعي"," ai ")){
    return "أنا المساعد بتاع NextStep AI — بساعدك تلاقي فرص وتطوّر مسارك المهني بناءً على بياناتك وبيانات الفرص المتاحة فعليًا على المنصة.";
  }
  return `مش متأكد إني فاهم قصدك بالظبط 🙂 جرب تسأل عن: فرص مناسبة ليك، تحسين الـCV، كورسات ومهارات، أو الشهادات — أو تصفح مباشرة من تبويبي "الفرص المقترحة" و"مركز التعلم".`;
}
...
لو مش متأكد إن الفرصة حقيقية، متضيفهاش. لو مفيش فرص كفاية رجّع [].`;
  try{
    // مهلة 80 ثانية + إعادة محاولة تلقائية مرة — الطلب ده بيولّد لحد 5 فرص
    // بتفاصيلها كـJSON منظّم، وده بياخد وقت أطول من رد شات عادي. رفعنا
    // maxOutputTokens لـ4096 (كان 2048) لأن 5 فرص بوصفها بيتخطوا الرقم القديم
    // بسهولة، وده كان بيقطع الرد في النص (finishReason: MAX_TOKENS) فيبوظ الـJSON.
    const result = await callAiProxy("", {
      contents:[{role:"user", parts:[{text:prompt}]}],
      generationConfig: {maxOutputTokens: 4096}
    }, {timeoutMs:80000, retries:1});
    if(!result.ok){
    ...
    const truncatedNote = cand && cand.finishReason==="MAX_TOKENS"
      ? `ملحوظة: رد الـAI اتقطع لإنه كان طويل، فظهرلك ${items.length} فرصة بس بدل 5 (اللي اتقطعت اتشالت تلقائيًا، مش هتشوف بيانات ناقصة).`
      : "";
    return {items, sources, searchEntryHtml, note: truncatedNote};
  }catch(err){
    console.error("fetchAiOpportunities: unexpected error:", err);
    return {error:"حصل خطأ غير متوقع أثناء معالجة نتيجة البحث، جرب تاني."};
  }
}
...
async function searchForOpportunities(topic){
  ...
  try{
    ...
    await loadOpportunities();
    state.searchTopic="";
    state.searchNote = result.note || "";
    // بنعرض للأدمن قايمة بالفرص اللي الـAI لقاها ودخلها فعليًا على طول (من غير
    // ما نحتاج موافقة لكل فرصة لوحدها زي ما طلب) — عشان يشوفها ويراجعها بسرعة
    // بدل ما يدور عليها وسط كل الفرص التانية.
    state.lastFoundItems = result.items;
  }catch(err){
