...
[{"id":"...","tags":["وسم1","وسم2"],"stageTags":["high","university"]}]`;
    // مكالمة Gemini واحدة بس — من غير أي إعادة محاولة على الكوتا (retries:1
...
// (مقارنة رابط بالظبط — آمنة 100%، ومفيهاش أي طلب Gemini خالص)، وبعدين
// بتبعت أول دفعة بس من الباقي للـAI (مكالمة Gemini واحدة بالظبط) عشان يكتشف
// أي فرص شكلها مكرر بمعنى تاني أو وهمية، ويمسحهم على طول.
// لو فيه فرص أكتر من حجم الدفعة، دوس الزرار تاني بعد ما ده يخلص — كل ضغطة
// زرار = مكالمة Gemini واحدة بس، مش لفة تلقائية على كل الفرص.
// ملحوظة: كانت فيه نسخة قديمة بتلف على الفرص كلها تلقائيًا (لحد 30 طلب AI
// ورا بعض في نفس الكبسة)، وده كان بيستهلك حصة الأدمن اليومية من ضغطة واحدة
// ويطلّع خطأ quota — رجّعناها لمكالمة واحدة بس عشان كده بالظبط.
...
async function cleanupFakeAndDuplicateOpportunities(){
  ...
  // الخطوة 1: تكرار بنفس الرابط بالظبط — حذف مباشر وأكيد 100%، من غير AI خالص.
  ...
  // الخطوة 2: مراجعة بالـAI — دفعة واحدة بس (مكالمة Gemini واحدة)، تكرار
  // بصياغة مختلفة أو فرص وهمية/غير منطقية.
  const batch = state.opportunities.slice(0, FAKE_CHECK_BATCH_SIZE);
  if(batch.length>0){
...
["id1","id2"]`;
  ...
  await loadOpportunities();
  const remainingCount = Math.max(0, state.opportunities.length - batch.length);
  ...
  state.adminMsg = (total>0
    ...
    : "مفيش فرص مكررة أو وهمية اتلاقت في الدفعة دي.")
    + (remainingCount>0 ? ` — دوس الزرار تاني عشان تراجع باقي الفرص (${remainingCount}).` : "");
  }catch(err){
