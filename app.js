...
async function deleteCollegeGuideEntry(id){
  ...
  }catch(err){ console.error("deleteCollegeGuideEntry error:", err); }
  render();
}
...
async function deleteScholarshipGuideEntry(id){
  ...
  }catch(err){ console.error("deleteScholarshipGuideEntry error:", err); }
  render();
}
...
// المفتاح بقى مخبّى تمامًا في السيرفر (Vercel Serverless Function جوه /api/chat)
// — الموقع بيكلم البروكسي ده بس، ومفيش أي اعتماد على Cloudflare خالص.
