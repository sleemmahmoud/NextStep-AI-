...
let state = {
  ...
  setupPhone:"", setupLinkedin:"", setupGithub:"",
...
function buildCvHtml(p){
  ...
  <div style="width:900px;min-height:1273px;background:#fff;box-sizing:border-box;padding:0;">
    ...
    <div style="padding:35px 50px;">
      ...
      ${p.cvLink?`<div><div style="font-size:14px;font-weight:800;color:#0E3A45;border-bottom:2px solid #E8A33D;display:inline-block;padding-bottom:4px;margin-bottom:10px;">مرفقات إضافية</div><div style="font-size:13px;color:#164B58;">${escapeHtml(p.cvLink)}</div></div>`:""}
    </div>
  </div>`;
}
async function generateCV(){
  ...
  try{
    await htmlToPdf(buildCvHtml(p), `CV - ${p.name||"NextStep"}.pdf`, 900, 1273);
  }catch(err){
...
function printCV(){
  ...
  win.document.write(`<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>CV - ${escapeHtml(p.name||"")}</title>
  <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&display=swap" rel="stylesheet">
  <style>body{margin:0;font-family:'Cairo',Tahoma,Arial,sans-serif;}</style></head><body>${buildCvHtml(p)}</body></html>`);
  win.document.close();
...
async function submitProfile(){
  ...
  const profile = {
    ...
    interests: [...state.setupInterests], skills: [...state.setupSkills],
...
function renderUserAiSearchBox(){
  ...
  <div class="card" style="padding:16px;margin-bottom:14px;">
    ...
    <form id="user-ai-search-form" style="display:flex;gap:8px;">
      <input id="user-ai-search-input" type="text" value="${escapeHtml(state.userSearchTopic)}" placeholder="مثال: منح في ألمانيا، فرص تطوع، تدريب Data Analysis..." style="flex:1;padding:12px 14px;border:1.5px solid var(--border);border-radius:10px;font-size:16px;">
...
function renderOppList(){
  ...
  if(list.length===0){
    ...
    return aiBox + `<div class="empty-state"><h3>مفيش فرص متاحة دلوقتي</h3><p>لو انت الأدمن، ضيف فرص حقيقية من تبويب "الإدارة" — يدويًا أو بالبحث بالـAI.</p></div>`;
  }
...
function renderProfileTab(){
  ...
  <button class="btn btn-ghost btn-block" data-action="print-cv" style="margin-bottom:10px;">🖨️ طباعة CV كنص (أفضل لأنظمة ATS)</button>
...
function renderResourcesTab(){
  if(state.resources.length===0){
    return `<div class="empty-state"><h3>لسه مفيش مصادر تعليمية</h3><p>ضيف أول مصدر من تبويب "ملفي" — قسم "إضافة محتوى".</p></div>`;
  }
...
function renderChatTab(){
  ...
  return `
  <div class="chat-wrap">
    ${state.aiConnected?`<div class="note-box chat-status" style="background:#EAF6EF;color:#1F7A47;">متصل بـ Gemini ✓ — لو ظهرلك رسالة خطأ، ابعتهالي زي ما هي.</div>`:""}
...
// NOTE ON SECURITY: this key lives in the page's own code, so anyone who views
// the page source can see it. That's an acceptable trade-off ONLY while this is
// a free-tier key with no billing attached and only you are testing — worst case
// someone burns your free daily quota, not your money. Before this goes live to
// real students, this call needs to move behind a small server function so the
// key never reaches the browser.
const GEMINI_API_KEY = "AQ.Ab8RN6J6tPOB-X3C9Bj2-MKSc2diiFBHZrDLvqhSzDy1SLXWhg";
...
اكتب ردودك في فقرات قصيرة أو نقط لو الموضوع فيه أكتر من نقطة، بلاش فقرة واحدة طويلة مضغوطة.
...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  const contents = state.chatMessages.slice(0,-1).map(m=>({
  ...
  try{
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`, {
      ...
      body: JSON.stringify({ contents, systemInstruction:{parts:[{text:systemText}]}, tools:[{google_search:{}}] })
    });
    const data = await res.json();
    if(!res.ok){
      const errMsg = (data && data.error && data.error.message) || `خطأ غير معروف (كود ${res.status})`;
      return `⚠️ جوجل رجّع الخطأ ده: "${errMsg}"\n\nده غالبًا معناه إن المفتاح مش صحيح أو لسه مفعّلش. ارجع aistudio.google.com/app/apikey وتأكد إنك ناسخ اللي مكتوب جنب "API key" بالحرف، وابعتهولي تاني.`;
    }
    const cand = data && data.candidates && data.candidates[0];
    const text = cand && cand.content && cand.content.parts && cand.content.parts.map(p=>p.text||"").join("");
    const sources = (cand && cand.groundingMetadata && cand.groundingMetadata.groundingChunks || []).map(c=>c.web).filter(Boolean);
    let out = text || "معرفتش أرد على السؤال ده، جرب تصيغه بطريقة تانية.";
    if(sources.length){
      out += "\n\nمصادر:\n" + sources.slice(0,4).map(s=>`↗ ${s.title||s.uri}`).join("\n");
    }
    return out;
  }catch(err){
...
async function maybeAutoSearch(){
  const last = state.profile.lastAutoSearchAt || 0;
  if((Date.now()-last)/(1000*60*60) < 20) return;
  const dayIndex = Math.floor(Date.now()/86400000);
  const topic1 = AUTO_SEARCH_TOPICS[dayIndex % AUTO_SEARCH_TOPICS.length];
  const topic2 = AUTO_SEARCH_TOPICS[(dayIndex+1) % AUTO_SEARCH_TOPICS.length];
  state.profile.lastAutoSearchAt = Date.now();
  try{ await updateDoc(doc(db,"users",state.user.uid), {lastAutoSearchAt: Date.now()}); }catch(e){}
  await searchForOpportunities(topic1);
  await searchForOpportunities(topic2);
}
...
async function fetchAiOpportunities(topic){
  ...
  const prompt = `النهاردة تاريخ ${todayStr} (يعني إحنا في سنة ${currentYear}). ابحث في جوجل بجد واجتهاد عن أحدث الفرص الحقيقية المتاحة فعليًا دلوقتي (بحد أقصى منطقي حوالي 15) مناسبة لموضوع: "${topic}"، تستهدف طلاب أو خريجين مصريين أو فرص دولية متاحة لهم. اعمل أكتر من صياغة بحث لنفس الموضوع (بالعربي وبالإنجليزي) عشان تلاقي أكبر عدد ممكن من النتائج المحدّثة فعلًا، ولو الموضوع عن مؤتمرات أو مسابقات دور بجد واجتهد تلاقي أكتر عدد ممكن منها.
مصادر مقترحة تدور فيها: Coursera وedX وUdemy المجاني وFutureLearn وGoogle وMicrosoft Learn وUN Volunteers وUN Careers وErasmus+ وEuropean Youth Portal وDAAD وChevening وBritish Council ومواقع الجامعات والوزارات ومنظمات رسمية وكيانات شبابية ومنظمات تطوعية، وكمان مصادر مصرية زي وزارة الشباب والرياضة وITI وDigital Egypt وNTI وTIEC. (ملحوظة: فيسبوك ولينكدإن غالبًا نتايجهم محدودة لأنهم بيمنعوا الفهرسة، فركّز على المواقع الرسمية المفتوحة).
شروط أساسية للتحديث ولازم تلتزم بيها بدقة:
- الفرصة لازم يكون التقديم عليها مفتوح فعليًا دلوقتي (يعني آخر موعد بعد ${todayStr} أو فرصة مستمرة/دورية بدون موعد ثابت).
- لو الصفحة اللي لقيتها بتتكلم عن دورة قديمة (زي سنة ${currentYear-1} أو قبل) وموعدها فات، ومفيش على الصفحة نفسها أو في نتيجة بحث تانية تأكيد واضح إن فيه دورة جديدة لسنة ${currentYear} أو ${currentYear+1} لسه مفتوحة، متضيفش الفرصة دي خالص — تجاهلها تمامًا، ومتحاولش تخمن أو تحسب موعد جديد بناءً على نمط السنين اللي فاتت.
- افتكر إن فرص كتير بتتكرر سنويًا وصفحتها القديمة بتفضل موجودة على جوجل بعد ما تقفل، فلازم تتأكد إن المعلومات اللي هتديهالي محدّثة فعلاً للدورة الحالية أو المقبلة، مش نسخة قديمة متخزنة.
- لو مش متأكد إن الفرصة لسه مفتوحة، ولا لقيت تأكيد لدورة جديدة، سيبها ومترجعهاش بدل ما تدّيني معلومة غلط.
- لازم كل فرصة يكون ليها رابط حقيقي موجود فعليًا من نتيجة البحث.
...
لو مفيش رابط حقيقي موثوق للفرصة، متضيفهاش خالص. لو مفيش فرص نشطة موثوقة كفاية رجّع [].`;
  try{
    ...
    const data = await res.json();
    if(!res.ok){
      return {error: (data && data.error && data.error.message) || `خطأ (كود ${res.status})`};
    }
    const cand = data && data.candidates && data.candidates[0];
    const text = cand && cand.content && cand.content.parts && cand.content.parts.map(p=>p.text||"").join("") || "";
...
// admin flow: results go to "pendingOpportunities" and need admin approval before anyone sees them
async function searchForOpportunities(topic){
  ...
  for(const it of result.items){
    await addDoc(collection(db,"pendingOpportunities"), {
...
async function userSearchForOpportunities(topic){
  state.userSearchBusy = true; state.userSearchErr=""; render();
  const result = await fetchAiOpportunities(topic);
...
const SETUP_TEXT_FIELD_MAP = {
  ...
  "setup-linkedin":"setupLinkedin", "setup-github":"setupGithub"
};
...
app.addEventListener("click",(e)=>{
  ...
  if(action==="show-login"){ state.authMode="login"; state.authError=""; render(); }
  ...
  else if(action==="print-cv"){ printCV(); }
    ...
    state.setupInterests=new Set(p.interests||[]); state.setupSkills=new Set(p.skills||[]);
    ...
    try{
      if(user){
        ...
        if(isComplete){
          ...
          state.setupInterests=new Set((profile&&profile.interests)||[]); state.setupSkills=new Set((profile&&profile.skills)||[]);
