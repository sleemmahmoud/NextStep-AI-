...
function renderResourcesTab(){
  const introNote = `<div class="note-box" style="margin-bottom:14px;">📚 دي كورساتي ونصايحي واختباراتي الخاصة اللي بحطها بنفسي.</div>`;
  ...
  return introNote
    + (tips.length ? `<div class="section-label" style="font-size:15px;">💡 نصائح سريعة</div>` + tips.map(renderResourceCard).join("") : "")
    + (courses.length ? `<div class="section-label" style="font-size:15px;margin-top:${tips.length?"18px":"0"};">🎓 الكورسات</div>` + courses.map(renderResourceCard).join("") : "");
}
...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  ...
  try{
    ...
    if(!res.ok){
      ...
      // 🔧 ملحوظة مؤقتة للتشخيص: بنعرض الطلب اللي اتبعت فعليًا عشان نتأكد مين
      // اللي ضايف "feature" — شيل السطر ده أول ما تتأكد من السبب وتصلحه.
      return {ok:false, text: `⚠️ حصل خطأ: "${(data && data.error && data.error.message) || res.status}"\n\n[نسخة السيرفر اللي رد]: ${data && data.__buildMark ? data.__buildMark : "❌ مفيش علامة — يبقى ده كود قديم لسه شغال، مش الأخير"}\n\n[تشخيص مؤقت] الطلب اللي اتبعت فعليًا:\n${requestBody.slice(0,500)}`};
    }
    const cand = data && data.candidates && data.candidates[0];
