...
const GEMINI_API_KEY = "AQ.Ab8RN6KVry_tnZeY2VwPM5giazlpiI-fG2qMY02BVyjeEUNLRA";
...
// Runs automatically whenever the admin opens the dashboard (throttled globally so it
// doesn't re-run on every single admin visit). Results are published LIVE right away —
// no manual approval step — so the admin never has to type a topic or click anything
// for the opportunities pool to keep growing on its own.
async function maybeAutoSearch(){
  try{
    const metaRef = doc(db,"meta","autoSearch");
    const snap = await getDoc(metaRef);
    const last = (snap.exists() && snap.data().lastRunAt) || 0;
    if((Date.now()-last)/(1000*60*60) < 12) return;
...
async function fetchAiOpportunities(topic){
  ...
  const prompt = `النهاردة تاريخ ${todayStr} (يعني إحنا في سنة ${currentYear}). ابحث في جوجل بجد عن أحدث الفرص الحقيقية المتاحة فعليًا دلوقتي (بحد أقصى 8 فرص، الأهم والأحدث فقط) مناسبة لموضوع: "${topic}"، تستهدف طلاب أو خريجين مصريين أو فرص دولية متاحة لهم. دوّر بصياغتين بحث كحد أقصى (عربي وإنجليزي) عشان تكون سريع ودقيق مع بعض.
مصادر مقترحة: Coursera وedX وUdemy المجاني وFutureLearn وMicrosoft Learn وUN Volunteers وUN Careers وErasmus+ وEuropean Youth Portal وDAAD وChevening وBritish Council ومواقع الجامعات والوزارات ومنظمات رسمية، وكمان مصادر مصرية زي وزارة الشباب والرياضة وITI وDigital Egypt وNTI وTIEC. (فيسبوك ولينكدإن نتايجهم محدودة، ركّز على المواقع الرسمية المفتوحة).
