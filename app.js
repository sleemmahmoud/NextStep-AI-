...
let state = {
  ...
  resourcesSubTab:"courses", adminEditResourceId:null,
...
// كل فيديوهات الكورس، من غير ما يحتاج يعدي اختبار الأول.
async function downloadCourseCertificate(resourceId){
  ...
  try{
    ...
    <div style="width:1200px;height:850px;background:linear-gradient(135deg,#0A2C35,#0E3A45);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;box-sizing:border-box;border:8px solid #E8A33D;padding:60px;text-align:center;">
      ...
      <div style="width:110px;height:3px;background:#E8A33D;margin-bottom:30px;"></div>
      <div style="font-size:30px;font-weight:800;color:#E8A33D;margin-bottom:18px;">${escapeHtml(state.profile.name||"")}</div>
      <div style="font-size:19px;max-width:780px;line-height:1.8;">أكمل بنجاح كورس "${escapeHtml(r.title)}" (${total} فيديو)</div>
      <div style="font-size:14px;margin-top:50px;opacity:.75;">${dateStr}</div>
    </div>`;
...
async function submitAdminResource(form){
  ...
  const editId = state.adminEditResourceId;
  const res = { title: fd.get("title"), type, description: fd.get("description")||"", tags: [...state.adminResTags], isPremium: fd.get("isPremium")==="true" };
  if(type==="course"){
    // كل سطر فيديو بالشكل: عنوان | رابط يوتيوب | رابط PDF (اختياري) | تاريخ نشر YYYY-MM-DD (اختياري) | روابط إضافية (اختياري، الشكل: عنوان1>رابط1;عنوان2>رابط2)
    const lines = (fd.get("lessonsRaw")||"").split("\n").map(l=>l.trim()).filter(Boolean);
    const lessons = lines.map(line=>{
      const parts = line.split("|").map(p=>p.trim());
      const title = parts[0]||"";
      const link = parts[1]||"";
      if(!title || !isValidLink(link)) return null;
      const pdfRaw = (parts[2]||"").trim();
      const pdfLink = isValidLink(pdfRaw) ? pdfRaw : "";
      const releaseRaw = (parts[3]||"").trim();
      const releaseAt = /^\d{4}-\d{2}-\d{2}$/.test(releaseRaw) ? new Date(releaseRaw+"T00:00:00").getTime() : null;
      const extraRaw = (parts[4]||"").trim();
      const links = extraRaw ? extraRaw.split(";").map(s=>s.trim()).filter(Boolean).map(pair=>{
        const gi = pair.indexOf(">");
        if(gi===-1) return null;
        const label = pair.slice(0,gi).trim();
        const url = pair.slice(gi+1).trim();
        return (label && isValidLink(url)) ? {label, url} : null;
      }).filter(Boolean) : [];
      return {title, link, pdfLink, releaseAt, links};
    }).filter(Boolean);
    if(lessons.length===0){
  ...
  try{
    if(editId){
      await updateDoc(doc(db,"resources", editId), res);
    } else {
      await addDoc(collection(db,"resources"), res);
    }
    await loadResources();
    state.adminMsg = editId ? "تم حفظ التعديلات على الكورس بنجاح ✓" : "تمت إضافة المصدر بنجاح ✓ هتلاقيه في تبويب مركز التعلم";
  }catch(err){
    console.error("submitAdminResource error:", err);
    state.adminMsg = "حصل خطأ أثناء حفظ المصدر، جرب تاني.";
  }
  state.adminResTags = new Set();
  state.adminEditResourceId = null;
  state.adminOpen = null;
  render();
  setTimeout(()=>{ state.adminMsg=""; render(); }, 4000);
}
...
// بتحوّل درس (لسون) لسطر نصي في التكستاريا، عشان نقدر نعرض الكورس الحالي
// جاهز للتعديل بدل ما الأدمن يعيد كتابته من الأول.
function lessonToLine(l){
  const dateStr = l.releaseAt ? new Date(l.releaseAt).toISOString().slice(0,10) : "";
  const linksStr = (l.links||[]).map(x=>`${x.label}>${x.url}`).join(";");
  return `${l.title} | ${l.link} | ${l.pdfLink||""} | ${dateStr} | ${linksStr}`;
}
function renderAdminResourceForm(){
  const editing = state.adminEditResourceId ? state.resources.find(r=>r.id===state.adminEditResourceId) : null;
  const lessonsPrefill = editing && editing.lessons ? editing.lessons.map(lessonToLine).join("\n") : "";
  return `
  ${editing?`<div class="note-box" style="background:#FDF3E2;color:#8A5B10;">✏️ بتعدّل على كورس موجود: "${escapeHtml(editing.title)}". أي فيديو تمسحه من الصندوق مش هيتحذف إلا لو مسحته فعليًا من هنا.</div>`:""}
  <form id="admin-res-form">
    <div class="field"><label>عنوان المصدر</label><input name="title" type="text" required value="${editing?escapeHtml(editing.title):""}"></div>
    <div class="field"><label>النوع</label><select name="type" ${editing?"disabled":""}>
      <option value="video" ${editing&&editing.type==="video"?"selected":""}>فيديو نصيحة (لينك يوتيوب واحد)</option>
      <option value="course" ${editing&&editing.type==="course"?"selected":""}>كورس (كذا فيديو يوتيوب)</option>
      <option value="pdf" ${editing&&editing.type==="pdf"?"selected":""}>ملف PDF</option>
      <option value="article" ${editing&&editing.type==="article"?"selected":""}>مقال</option>
    </select></div>
    ${editing?`<input type="hidden" name="type" value="${editing.type}">`:""}
    <div class="field"><label>مجاني ولا مميز؟</label><select name="isPremium"><option value="false" ${editing&&!editing.isPremium?"selected":""}>مجاني</option><option value="true" ${editing&&editing.isPremium?"selected":""}>مميز (Premium)</option></select></div>
    <div class="field"><label>وصف قصير</label><input name="description" type="text" value="${editing?escapeHtml(editing.description||""):""}"></div>
    <div class="field"><label>الرابط (لو النوع فيديو نصيحة أو PDF أو مقال)</label><input name="link" type="url" placeholder="https://..." value="${editing&&editing.type!=="course"?escapeHtml(editing.link||""):""}"></div>
    <div class="field"><label>فيديوهات الكورس (لو النوع "كورس" بس — سطر لكل فيديو)</label>
      <textarea name="lessonsRaw" rows="6" placeholder="عنوان الفيديو | رابط اليوتيوب | رابط PDF اختياري | تاريخ نشر اختياري YYYY-MM-DD | روابط إضافية اختيارية">${lessonsPrefill}</textarea>
    </div>
    <div class="note-box">كل فيديو له صندوقه الخاص: ملف PDF ولينكات مرتبطة بيه بس، مش بالكورس كله. الشكل الكامل للسطر:<br>عنوان الفيديو | رابط يوتيوب | رابط PDF (سيبه فاضي لو مفيش) | تاريخ النشر YYYY-MM-DD (سيبه فاضي لو هيتفتح فورًا) | روابط إضافية بالشكل: اسم1>رابط1;اسم2>رابط2</div>
    <div class="note-box">مثال: المقدمة وأساسيات المونتاج | https://youtu.be/xxxxxxxx | https://drive.google.com/xxx.pdf | 2026-08-01 | ملف تدريبي>https://example.com</div>
    <div class="note-box">لو حطيت تاريخ نشر في المستقبل، الفيديو ده هيفضل مقفول للطالب لحد ما يجي معاده — كده تقدر تضيف فيديوهات الكورس أول بأول (فيديو النهاردة، وفيديو تاني بعد أسبوع، وهكذا) من غير ما الطلاب يشوفوها قبل معادها.</div>
    <div class="note-box">عايز تضيف فيديو جديد لكورس موجود؟ اضغط "تعديل" على الكورس من تبويب مركز التعلم، وضيف سطر جديد في آخر الصندوق من غير ما تمسح اللي موجود.</div>
    <div class="field"><label>شهادة إتمام للكورس ده؟</label><select name="hasCertificate">
      <option value="false" ${!editing||!editing.hasCertificate?"selected":""}>لأ، من غير شهادة</option>
      <option value="true" ${editing&&editing.hasCertificate?"selected":""}>آه، هيطلع شهادة لما يخلص كل الفيديوهات</option>
    </select></div>
    ...
    <button type="submit" class="btn btn-gold btn-block">${editing?"حفظ التعديلات":"إضافة المصدر"}</button>
    ${editing?`<button type="button" class="btn btn-ghost btn-block" data-action="cancel-edit-resource" style="margin-top:8px;">إلغاء التعديل</button>`:""}
  </form>
...
function renderResourcesTab(){
  const sub = state.resourcesSubTab || "courses";
  const subTabsHtml = `
  <div class="tabs" style="margin:0 0 14px;position:static;top:auto;max-width:none;">
    <button class="tab ${sub==="courses"?"active":""}" data-action="res-subtab" data-sub="courses">🎓 الكورسات</button>
    <button class="tab ${sub==="tips"?"active":""}" data-action="res-subtab" data-sub="tips">💡 نصائح عملية</button>
  </div>`;
  const allItems = sub==="courses" ? state.resources.filter(r=>r.type==="course") : state.resources.filter(r=>r.type!=="course");
  if(allItems.length===0){
    const emptyMsg = sub==="courses" ? "لسه مفيش كورسات" : "لسه مفيش نصائح";
    if(isAdmin()){
      return subTabsHtml + `<div class="empty-state"><h3>${emptyMsg}</h3><p>ضيف أول مصدر من تبويب "الإدارة".</p></div>`;
    }
    return subTabsHtml + `<div class="empty-state"><h3>${emptyMsg}</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`;
  }
  const typeLabel = {video:"فيديو نصيحة",pdf:"ملف PDF",article:"مقال",course:"كورس (فيديوهات)"};
  return subTabsHtml + allItems.map(r=>{
    ...
    const courseComplete = isCourse && totalLessons>0 && doneLessons>=totalLessons;
    const now = Date.now();
    return `
    <div class="card" style="padding:16px;margin-bottom:12px;">
      ...
      ${isCourse ? `
        ...
        ${r.lessons.map((lesson,i)=>{
          const locked = lesson.releaseAt && lesson.releaseAt > now;
          if(locked){
            const dateStr = new Date(lesson.releaseAt).toLocaleDateString("ar-EG");
            return `
            <div style="border:1px solid var(--border);border-radius:12px;padding:10px;margin-bottom:10px;opacity:.6;">
              <div style="font-weight:700;font-size:13.5px;margin-bottom:6px;">${i+1}. ${escapeHtml(lesson.title)}</div>
              <div class="note-box" style="margin:0;">🔒 هيتفتح يوم ${dateStr}</div>
            </div>`;
          }
          ...
          <div style="border:1px solid var(--border);border-radius:12px;padding:10px;margin-bottom:10px;">
            ...
            ${(lesson.pdfLink || (lesson.links&&lesson.links.length)) ? `
            <div style="display:flex;flex-direction:column;gap:4px;margin-top:8px;">
              ${lesson.pdfLink?`<a href="${escapeHtml(lesson.pdfLink)}" target="_blank" rel="noopener" class="link-btn">📄 ملف PDF الفيديو ↗</a>`:""}
              ${(lesson.links||[]).map(l=>`<a href="${escapeHtml(l.url)}" target="_blank" rel="noopener" class="link-btn">🔗 ${escapeHtml(l.label)} ↗</a>`).join("")}
            </div>`:""}
        ...
        ${r.hasCertificate && courseComplete?`<button class="btn btn-gold btn-block" data-action="download-course-cert" data-id="${r.id}">🎓 تحميل شهادة إتمام الكورس</button>`:""}
      ` : ""}
...
// ده عداد "المساعد الذكي" (الشات) بس — منفصل تمامًا عن عداد الـCV وعداد أدوات
// البحث/التصنيف بتاعت الأدمن (اللي ليها عدّاد خاص بيها في worker.ts، اسمه
// "search"). الرقمين هنا لازم يفضلوا مطابقين لـFEATURE_LIMITS.chat في worker.ts.
...
// مميز / ATS) بعداد مستقل عن التانية بالكامل. الأدمن ليه حد أعلى شوية برضه
// (مش لا نهائي) عشان يقدر يختبر المنصة من غير ما يستهلك كوتا الـAI بسرعة.
// ده عداد محلي (Firestore) بس — فيه كمان عداد سيرفر منفصل تمامًا اسمه "cv"
// في worker.ts (FEATURE_LIMITS.cv)، فتوليد الـCV مبيلمسش عداد الشات أو
// عداد أدوات البحث خالص.
...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  ...
  try{
    ...
    if(!res.ok){
      ...
      return {ok:false, text: `⚠️ حصل خطأ: "${errMsg}"${extractQuotaDetail(data)}`};
    }
    const cand = data && data.candidates && data.candidates[0];
...
[{"id":"...","tags":["وسم1","وسم2"],"stageTags":["high","university"]}]`;
    try{
      const res = await fetch(GEMINI_PROXY_URL, {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ contents:[{role:"user", parts:[{text:prompt}]}], uid: state.user && state.user.uid, feature: "search" })
      });
      const data = await res.json();
      if(!res.ok){
...
app.addEventListener("click",(e)=>{
  ...
  if(action==="show-login"){ state.authMode="login"; state.authError=""; render(); }
    ...
    if(r){
      state.adminEditResourceId = r.id;
      state.adminResTags = new Set(r.tags||[]);
      state.adminOpen = "res";
      render();
    }
  }
  else if(action==="cancel-edit-resource"){ state.adminEditResourceId=null; state.adminResTags=new Set(); state.adminOpen=null; render(); }
  else if(action==="res-subtab"){ state.resourcesSubTab = t.dataset.sub; render(); }
