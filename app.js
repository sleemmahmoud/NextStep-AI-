...
const ADMIN_EMAILS = ["mhmwdshhath468@gmail.com"];
function isAdmin(){
...
let state = {
  ...
  userSearchResults:[], userSearchTopic:"", userSearchBusy:false, userSearchErr:"",
  quizzes:[], completedResourceIds:[], quizResults:{}, activeQuizId:null, quizAnswers:{}, quizSubmitted:false
};
...
function calcMatch(profile, opp){
  ...
  const stageOk = !opp.stageTags || opp.stageTags.includes(profile.stage);
...
async function downloadCertificate(quizId){
  ...
  const html = `
  <div style="width:1200px;height:850px;background:linear-gradient(135deg,#0A2C35,#0E3A45);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;box-sizing:border-box;border:8px solid #E8A33D;padding:60px;text-align:center;">
    <div style="font-family:'Montserrat',sans-serif;font-weight:800;font-size:26px;letter-spacing:2px;">NextStep AI</div>
    <div style="font-size:42px;font-weight:800;margin:30px 0 8px;">شهادة إتمام</div>
    <div style="width:110px;height:3px;background:#E8A33D;margin-bottom:30px;"></div>
    <div style="font-size:30px;font-weight:800;color:#E8A33D;margin-bottom:18px;">${escapeHtml(state.profile.name||"")}</div>
    <div style="font-size:19px;max-width:780px;line-height:1.8;">أكمل بنجاح اختبار "${escapeHtml(quiz.title)}" بنتيجة ${result.score}/${result.total}</div>
    <div style="font-size:14px;margin-top:50px;opacity:.75;">${dateStr}</div>
  </div>`;
...
async function polishAchievements(){
  const raw = (state.setupAchievements||"").trim();
  if(!raw){ state.polishError = "اكتب إنجازاتك الأول، وبعدين اضغط تحسين."; render(); return; }
  const allowed = await tryConsumeAiUsage();
  if(!allowed){ state.polishError = `⚠️ وصلت للحد اليومي لاستخدام الذكاء الاصطناعي (${AI_DAILY_LIMIT} طلب في اليوم).`; render(); return; }
  state.polishBusy = true; state.polishError=""; render();
  try{
    const prompt = `أنت خبير كتابة سير ذاتية. حوّل النقط التالية اللي كتبها طالب/خريج عن إنجازاته وخبراته ومشاركاته إلى نقط احترافية مختصرة بالعربية الفصحى البسيطة، كل نقطة تبدأ بسطر جديد، من غير مقدمة أو خاتمة أو أي كلام زيادة، وحافظ على الحقائق زي ما هي من غير ما تختلق أي حاجة جديدة. النص الأصلي:\n"""${raw}"""`;
    ...
    if(!res.ok){
      state.polishError = (data && data.error && data.error.message) || `تعذّر التحسين حاليًا (كود ${res.status}).`;
    }else{
      const cand = data && data.candidates && data.candidates[0];
      const text = (cand && cand.content && cand.content.parts && cand.content.parts.map(x=>x.text||"").join("") || "").trim();
      if(text) state.setupAchievements = text;
      else state.polishError = "معرفتش أحسّن النص ده، جرب تفصّل شوية وحاول تاني.";
    }
  }catch(err){
    state.polishError = "مقدرتش أوصل للذكاء الاصطناعي دلوقتي، اتأكد من الإنترنت وجرب تاني.";
  }
  state.polishBusy = false; render();
}
async function generateCV(){
  const allowed = await tryConsumeCvUsage();
  if(!allowed){ alert(`وصلت للحد اليومي لعمل الـCV (${CV_DAILY_LIMIT} في اليوم). جرب تاني بكرة.`); return; }
  const p = {...state.profile, email: state.profile.contactEmail || state.user.email};
  try{
...
async function submitAdminResource(form){
  ...
  const fd = new FormData(form);
  const res = { title: fd.get("title"), type: fd.get("type"), description: fd.get("description")||"", link: fd.get("link"), tags: [...state.adminResTags], isPremium: fd.get("isPremium")==="true" };
  try{
    await addDoc(collection(db,"resources"), res);
    await loadResources();
    state.adminMsg = "تمت إضافة المصدر بنجاح ✓ هتلاقيه في تبويب الموارد";
  }catch(err){
...
async function submitProfile(){
  ...
  const profile = {
    ...
    completedResourceIds: state.completedResourceIds||[], quizResults: state.quizResults||{},
...
function renderProfileSetup(){
  ...
  <div class="center-screen">
    <div class="card setup-wrap">
      ...
      <div class="field"><label>إنجازاتك ومشاركاتك (اختياري)</label><textarea id="setup-achievements" rows="3" placeholder="خبرات، تطوع، شهادات، مشاريع، أنشطة رياضية... أي حاجة تفتكرها">${escapeHtml(state.setupAchievements)}</textarea>
        <button type="button" class="btn btn-ghost" data-action="polish-achievements" style="margin-top:6px;font-size:12.5px;padding:6px 10px;" ${state.polishBusy?"disabled":""}>${state.polishBusy?"⏳ بتتحسّن دلوقتي...":"✨ حسّن الصياغة بالذكاء الاصطناعي"}</button>
        ${state.polishError?`<div style="color:#B33A3A;font-size:12px;margin-top:4px;">${escapeHtml(state.polishError)}</div>`:""}
      </div>
...
function renderDashboard(){
  ...
  app.innerHTML = `
    ...
    <div class="tabs">
      ...
      <button class="tab ${tab==="resources"?"active":""}" data-action="tab" data-tab="resources">الموارد</button>
...
function renderUserAiSearchBox(){
  const p = state.profile||{};
  const learnSkills = [...(p.learnSkills||[])];
  const quickChips = [
    {label:"🎓 كورسات مجانية", topic:"كورسات مجانية معتمدة أونلاين"},
    {label:"🗓️ مؤتمرات وفعاليات", topic:"مؤتمرات وفعاليات للطلاب والشباب"},
    ...learnSkills.map(s=>({label:`🎓 كورسات ${s}`, topic:`كورسات مجانية لتعلم ${s}`})),
  ];
  return `
  <div class="card" style="padding:16px;margin-bottom:14px;">
    <div style="font-weight:800;font-size:14.5px;margin-bottom:6px;">🔍 ابحث بالذكاء الاصطناعي عن فرص تانية</div>
    <div style="font-size:12.5px;color:var(--ink-muted);margin-bottom:10px;">النتايج دي ليك انت بس — مش بتتحفظ عندنا، ومش هتظهر لأي مستخدم تاني، ومفيهاش علامة "تم التحقق".</div>
    <div style="font-size:11.5px;color:var(--ink-muted);margin-bottom:10px;">متبقي ليك النهاردة: ${aiUsageRemaining()} من ${AI_DAILY_LIMIT} طلب (شات + بحث ذكي مع بعض)</div>
    <div class="tag-grid" style="margin-bottom:10px;">
      ${quickChips.map(c=>`<button type="button" class="pill" data-action="quick-search" data-topic="${escapeHtml(c.topic)}">${c.label}</button>`).join("")}
    </div>
    <form id="user-ai-search-form" style="display:flex;gap:8px;">
      <input id="user-ai-search-input" type="text" value="${escapeHtml(state.userSearchTopic)}" placeholder="مثال: منح في ألمانيا، فرص تطوع، تدريب Data Analysis، مؤتمر تصميم، كورس مونتاج..." style="flex:1;padding:12px 14px;border:1.5px solid var(--border);border-radius:10px;font-size:16px;">
      <button type="submit" class="btn btn-gold" ${state.userSearchBusy?"disabled":""}>${state.userSearchBusy?"بيدور...":"دور"}</button>
    </form>
    ${state.userSearchErr?`<div class="error-box" style="margin-top:12px;">${state.userSearchErr}</div>`:""}
    ${state.userSearchResults.length?renderUserAiSearchResults():""}
  </div>`;
}
function renderUserAiSearchResults(){
  return `<div style="margin-top:14px;display:flex;flex-direction:column;gap:10px;">
    ${state.userSearchResults.map(item=>`
    <div class="card" style="padding:14px;">
      <span class="opp-cat">${CATEGORIES[item.category]||item.category}</span>
      <span class="pill" style="margin-inline-start:6px;background:#F6F8F9;color:var(--ink-muted);">نتيجة بحث خاصة — مش موثقة من المنصة</span>
      <div class="opp-title" style="margin-top:8px;">${escapeHtml(item.title)}</div>
      <div class="opp-org">${escapeHtml(item.organization||"")}</div>
      ${(item.deadline && item.deadline!=="غير معلن")?`<div class="opp-deadline normal">آخر موعد (حسب البحث): ${escapeHtml(item.deadline)}</div>`:""}
      <div class="desc-text" style="margin:8px 0;">${escapeHtml(item.description||"")}</div>
      ${item.link?`<a href="${escapeHtml(item.link)}" target="_blank" rel="noopener" class="link-btn" style="display:inline-block;">فتح الرابط ↗</a>`:""}
    </div>`).join("")}
  </div>`;
}
...
function renderOppList(){
  ...
  const aiBox = renderUserAiSearchBox();
  if(list.length===0){
    if(state.searchQuery.trim()){
      return aiBox + searchBar + `<div class="empty-state"><h3>مفيش نتايج لـ "${escapeHtml(state.searchQuery)}"</h3><p>جرب كلمة تانية أو امسح البحث.</p></div>`;
    }
    return aiBox + searchBar + `<div class="empty-state"><h3>مفيش فرص متاحة دلوقتي</h3><p>لو انت الأدمن، ضيف فرص حقيقية من تبويب "الإدارة" — يدويًا أو بالبحث بالـAI.</p></div>`;
  }
  const withScores = list.map(o=>({o, m:calcMatch(state.profile,o)}));
  withScores.sort((a,b)=>b.m.score-a.m.score);
  return aiBox + searchBar + withScores.map(({o,m})=>{
...
function renderProfileTab(){
  ...
  <button class="btn btn-gold btn-block" data-action="generate-cv" style="margin-bottom:8px;">📄 تحميل CV (تصميم مميز)</button>
  <button class="btn btn-ghost btn-block" data-action="print-cv" style="margin-bottom:6px;">📄 تحميل CV بصيغة ATS (PDF بسيط)</button>
  <div style="font-size:11.5px;color:var(--ink-muted);margin-bottom:10px;">متبقي ليك النهاردة: ${cvUsageRemaining()} من ${CV_DAILY_LIMIT} (العدّاد مشترك بين النسختين)</div>
...
function renderAdminResourceForm(){
  ...
  <form id="admin-res-form">
    ...
    <div class="field"><label>النوع</label><select name="type"><option value="video">فيديو</option><option value="pdf">ملف PDF</option><option value="article">مقال</option><option value="course">كورس خارجي</option></select></div>
...
function renderAiSearchPanel(){
  ...
  <form id="ai-search-form" style="display:flex;gap:8px;margin-bottom:16px;">
    ...
    <button type="submit" class="btn btn-gold" ${state.searchBusy?"disabled":""}>${state.searchBusy?"بيدور...":"دور"}</button>
  </form>
...
function renderResourcesTab(){
  if(state.resources.length===0){
    if(isAdmin()){
      return `<div class="empty-state"><h3>لسه مفيش مصادر تعليمية</h3><p>ضيف أول مصدر من تبويب "الإدارة".</p></div>`;
    }
    return `<div class="empty-state"><h3>لسه مفيش مصادر تعليمية</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`;
  }
  const typeLabel = {video:"فيديو",pdf:"ملف PDF",article:"مقال",course:"كورس خارجي"};
  return state.resources.map(r=>{
    ...
    <div class="card" style="padding:16px;margin-bottom:12px;">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center;">
        ${r.link?`<a href="${escapeHtml(r.link)}" target="_blank" rel="noopener" class="link-btn">فتح المصدر ↗</a>`:""}
        <button class="pill ${isDone?"selected":""}" data-action="toggle-complete" data-id="${r.id}">${isDone?"✓ خلصته":"علّمه كمكتمل"}</button>
      </div>
      ${quiz?`
...
function renderChatTab(){
  ...
  return `
  <div class="chat-wrap">
    ${state.aiConnected?`<div class="note-box chat-status" style="background:#EAF6EF;color:#1F7A47;">متصل بـ Gemini ✓ — متبقي ليك النهاردة ${aiUsageRemaining()} من ${AI_DAILY_LIMIT} طلب (شات + بحث ذكي مع بعض). لو ظهرلك رسالة خطأ، ابعتهالي زي ما هي.</div>`:""}
...
// غيّر الرابط ده بعد ما تعمل الـWorker (هيكون شكله https://<اسم>.<حسابك>.workers.dev)
...
// خليت الحد أضيق شوية (بدل ٢٠) عشان الاستخدام يبقى محكوم أكتر، ويسيب مساحة
// إننا نقدر نضيف مستقبلًا خطة "بريميوم" بحد أعلى للمستخدمين اللي بيدفعوا.
...
// بترجع true لو المستخدم لسه عنده مساحة، وبتزوّد العداد وتحفظه في مستنده هو بس.
// بترجع false من غير ما تزوّد أو تكلم السيرفر لو وصل لحده اليومي.
async function tryConsumeAiUsage(){
  ...
  try{
    ...
    console.error("tryConsumeAiUsage error:", err);
    // حتى لو فشل الحفظ في Firestore، سيبها تكمل الطلب بدل ما توقف المستخدم بسبب مشكلة شبكة عابرة
  }
  return true;
}
// حد منفصل لعدد مرات عمل الـCV في اليوم (تصميم مميز + ATS مع بعض)، لكل مستخدم
// لوحده، بنفس فكرة وتخزين aiUsage بالظبط لكن بعداد مستقل خاص بالـCV.
...
function cvUsageRemaining(){
  return Math.max(0, CV_DAILY_LIMIT - getCvUsage().count);
}
async function tryConsumeCvUsage(){
  const usage = getCvUsage();
  if(usage.count >= CV_DAILY_LIMIT) return false;
  const updated = {date: usage.date, count: usage.count+1};
...
async function askAI(message){
  ...
  const systemText = `إنت "المساعد الذكي" جوه منصة NextStep AI، منصة بتساعد طلاب المدارس والجامعات والخريجين الجدد في مصر يلاقوا فرص (منح دراسية، تدريب، وظايف، مسابقات) ويطوروا مسارهم المهني. رد دايمًا بالعربية المصرية البسيطة، بأسلوب ودود ومشجع ومباشر، وخليك مختصر ومفيد ومش طويل أوي.
اكتب ردودك في فقرات قصيرة أو نقط لو الموضوع فيه أكتر من نقطة، بلاش فقرة واحدة طويلة مضغوطة. لو استخدمت بحث جوجل في إجابتك وعندك مصدر حقيقي، اذكره باختصار.
...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  ...
  try{
    ...
    if(!res.ok){
      if(res.status===429){
        return (data && data.error && data.error.message) || `⚠️ وصلت للحد اليومي لاستخدام المساعد الذكي، جرب تاني بكرة.`;
      }
      const errMsg = (data && data.error && data.error.message) || `خطأ غير معروف (كود ${res.status})`;
      return `⚠️ حصل خطأ: "${errMsg}"${extractQuotaDetail(data)}`;
    }
    const cand = data && data.candidates && data.candidates[0];
    const text = cand && cand.content && cand.content.parts && cand.content.parts.map(x=>x.text||"").join("") || "";
    if(!text) return "معرفتش أرد على السؤال ده، جرب تصيغه بطريقة تانية.";
    ...
    return sourceHosts.length ? `${text}\n\nمصادر: ${sourceHosts.join("، ")}` : text;
  }catch(err){
    return "مقدرتش أوصل لخدمة الذكاء الاصطناعي دلوقتي، تأكد من اتصال الإنترنت وجرب تاني.";
  }
}
async function sendChatMessage(text){
  ...
  const allowed = await tryConsumeAiUsage();
  if(!allowed){
    state.chatMessages.push({role:"assistant", text:`⚠️ وصلت للحد اليومي لاستخدام المساعد الذكي (${AI_DAILY_LIMIT} طلب في اليوم). هيتصفر تلقائيًا بكرة، جرب تاني وقتها.`});
    render();
    return;
  }
...
const AUTO_SEARCH_TOPICS = ["منح دراسية للطلاب المصريين","تدريب صيفي للطلاب والخريجين الجدد","فرص تطوع للشباب","مسابقات ريادة أعمال وتكنولوجيا للطلاب","كورسات مجانية معتمدة أونلاين","مؤتمرات دولية ومحلية للطلاب والشباب"];
// Runs automatically once every 24 hours (max) whenever the admin opens the dashboard —
// throttled globally via a shared Firestore doc so it never fires more than once a day,
// no matter how many times the admin visits. On that one daily run it goes through EVERY
// topic in AUTO_SEARCH_TOPICS to bring in as many fresh opportunities as possible in one go.
// Results are published LIVE right away — no manual approval step needed.
async function maybeAutoSearch(){
  try{
    const metaRef = doc(db,"meta","autoSearch");
    const snap = await getDoc(metaRef);
    const last = (snap.exists() && snap.data().lastRunAt) || 0;
    if((Date.now()-last)/(1000*60*60) < 24) return;
    await setDoc(metaRef, {lastRunAt: Date.now()}, {merge:true});
  }catch(e){ return; } // if we can't confirm the throttle, skip rather than risk spamming the API
  for(const topic of AUTO_SEARCH_TOPICS){
    await searchForOpportunities(topic);
  }
}
...
async function fetchAiOpportunities(topic){
  ...
  const prompt = `النهاردة تاريخ ${todayStr} (يعني إحنا في سنة ${currentYear}). ابحث في جوجل بجد عن أحدث الفرص الحقيقية المتاحة فعليًا دلوقتي (بحد أقصى 8 فرص، الأهم والأحدث فقط) مناسبة لموضوع: "${topic}"، تستهدف طلاب أو خريجين مصريين أو فرص دولية متاحة لهم.
مهم جدًا: "فرصة" هنا معناها أي نوع من الآتي، مش بس منح أو تطوع: منح دراسية، تدريب، وظائف، تطوع، مسابقات، هاكاثونات، برامج تبادل، **كورسات مجانية أونلاين**، بوت كامب، **مؤتمرات وفعاليات**. لو الموضوع بيتكلم عن "كورس" أو "تعلّم" أو "أتعلم" ركّز بحثك على كورسات ودورات تدريبية حقيقية (Coursera, edX, Udemy المجاني, Microsoft Learn, freeCodeCamp) بالظبط للمهارة المطلوبة. لو الموضوع بيتكلم عن "مؤتمر" أو "فعالية" ركّز على مؤتمرات وفعاليات حقيقية قريبة أو مفتوح التسجيل ليها. بلاش تفترض إن كل موضوع معناه منحة أو تطوع.
دوّر بصياغتين بحث كحد أقصى (عربي وإنجليزي) عشان تكون سريع ودقيق مع بعض.
مصادر مقترحة: Coursera وedX وUdemy المجاني وFutureLearn وMicrosoft Learn وfreeCodeCamp وUN Volunteers وUN Careers وErasmus+ وEuropean Youth Portal وDAAD وChevening وBritish Council ومواقع الجامعات والوزارات ومنظمات رسمية، وكمان مصادر مصرية زي وزارة الشباب والرياضة وITI وDigital Egypt وNTI وTIEC. (فيسبوك ولينكدإن نتايجهم محدودة، ركّز على المواقع الرسمية المفتوحة).
شروط أساسية ولازم تلتزم بيها بدقة:
- الفرصة لازم يكون التقديم عليها مفتوح فعليًا دلوقتي (آخر موعد بعد ${todayStr} أو فرصة مستمرة بدون موعد ثابت).
- لو الصفحة بتتكلم عن دورة قديمة (${currentYear-1} أو قبل) وموعدها فات، ومفيش تأكيد واضح لدورة جديدة ${currentYear} أو ${currentYear+1}، تجاهلها تمامًا ومتخمنش موعد جديد.
- لو مش متأكد إن الفرصة لسه مفتوحة، سيبها ومترجعهاش بدل ما تدّيني معلومة غلط.
- لازم كل فرصة يكون ليها رابط حقيقي موجود فعليًا من نتيجة البحث نفسها.
رجّع بس JSON array، من غير أي نص تاني قبله أو بعده، بالشكل ده بالظبط:
[{"title":"عنوان الفرصة","organization":"اسم الجهة","description":"وصف قصير من سطرين بالعربي دايمًا حتى لو المصدر إنجليزي","category":"scholarship أو internship أو job أو volunteering أو competition أو conference أو hackathon أو exchange أو course أو bootcamp أو event","deadline":"YYYY-MM-DD لو معروف وإلا اكتب غير معلن","link":"الرابط الحقيقي من نتيجة البحث، لازم يكون موجود فعليًا"}]
لو مفيش رابط حقيقي موثوق للفرصة، متضيفهاش خالص. لو مفيش فرص نشطة موثوقة كفاية رجّع [].`;
  try{
    const res = await fetch(GEMINI_PROXY_URL, {
      method:"POST",
      headers:{"Content-Type":"application/json"},
...
async function searchForOpportunities(topic){
  ...
  for(const it of result.items){
    await addDoc(collection(db,"opportunities"), {
      ...
      tags:[], stageTags:[], requirements:[], reviewed:true,
...
// personal user flow: NEVER written to Firestore, NEVER shown to any other user, no "تم التحقق" badge.
// results live only in this browser tab's memory and disappear on refresh/logout.
// بحث المستخدمين الشخصي هو اللي بيستهلك أكتر حاجة من حصة "grounding" (بحث جوجل
// الحقيقي جوه Gemini) — وهي الحصة الأضيق والأصعب في الحساب المجاني. عشان كده،
// غير الحد الشخصي (٢٠ لكل مستخدم)، بنحط سقف عام مشترك لكل الموقع في اليوم،
// عشان نحمي الحصة المشتركة من إنها تخلص بسرعة مهما كان عدد المستخدمين.
const GLOBAL_PERSONAL_SEARCH_DAILY_CAP = 8;
async function checkGlobalSearchQuota(){
  const metaRef = doc(db,"meta","personalSearchQuota");
  const today = todayStr();
  try{
    const snap = await getDoc(metaRef);
    const data = snap.exists() ? snap.data() : {date:"", count:0};
    const usage = data.date===today ? data : {date:today, count:0};
    if(usage.count >= GLOBAL_PERSONAL_SEARCH_DAILY_CAP) return false;
    await setDoc(metaRef, {date:today, count:usage.count+1}, {merge:true});
    return true;
  }catch(e){ return false; } // لو مقدرناش نتأكد من السقف، الأضمن نمنع بدل ما نخاطر نستهلك من غير حساب
}
async function userSearchForOpportunities(topic){
  state.userSearchErr="";
  const globalOk = await checkGlobalSearchQuota();
  if(!globalOk){
    state.userSearchErr = `⚠️ البحث الذكي وصل لأقصى عدد طلبات مسموح بيه للموقع كله النهاردة (المورد ده محدود من جوجل). جرب تاني بكرة.`;
    render();
    return;
  }
  const allowed = await tryConsumeAiUsage();
  if(!allowed){
    state.userSearchErr = `⚠️ وصلت للحد اليومي لاستخدام البحث الذكي (${AI_DAILY_LIMIT} طلب في اليوم). هيتصفر تلقائيًا بكرة، جرب تاني وقتها.`;
    render();
    return;
  }
  state.userSearchBusy = true; render();
  const result = await fetchAiOpportunities(topic);
  if(result.error){ state.userSearchErr = result.error; state.userSearchBusy=false; render(); return; }
  state.userSearchResults = result.items.map((it,i)=>({...it, _localId:`u${Date.now()}_${i}`}));
  state.userSearchTopic = "";
  state.userSearchBusy=false; render();
}
...
app.addEventListener("click",(e)=>{
  ...
  else if(action==="quick-search"){ const topic=t.dataset.topic; state.userSearchTopic=topic; userSearchForOpportunities(topic); }
  ...
  else if(action==="close-detail"){ state.openOppId=null; render(); }
  ...
  else if(action==="polish-achievements"){ polishAchievements(); }
  else if(action==="admin-toggle"){
    state.adminOpen = state.adminOpen===t.dataset.form ? null : t.dataset.form;
    render();
    if(state.adminOpen==="ai" && isAdmin()) maybeAutoSearch();
  }
...
app.addEventListener("submit",(e)=>{
  ...
  else if(e.target.id==="user-ai-search-form"){
    const topic = document.getElementById("user-ai-search-input").value.trim();
    if(topic) userSearchForOpportunities(topic);
  }
});
  ...
  onAuthStateChanged(auth, async (user)=>{
    try{
      if(user){
        ...
        if(isComplete){
          state.profile = profile;
          state.completedResourceIds = profile.completedResourceIds||[]; state.quizResults = profile.quizResults||{};
          await loadOpportunities();
          await loadResources();
          await loadPendingOpps();
          await loadQuizzes();
