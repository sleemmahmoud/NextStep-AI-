...
let state = {
  ...
  learningSubTab:"tips", learningCenterSection:"content",
...
const HELP_TEXT = {
  files:{title:"ملفات", body:"لوحتك الشخصية: ملخّص سريع لآخر الفرص المناسبة ونسبة تقدمك، وتحتها كل الفرص اللي قررت تقدّم عليها — غيّر حالتها (سأقدم، تم التقديم، مقبول..) وسجّل ملاحظاتك عشان تفضل منظم."},
...
function renderFilesTab(){
  ...
  <div class="card" style="padding:20px;margin-top:0;">
    ...
    </div>` : `<div class="note-box" style="margin-bottom:14px;">أول مرة بتفتح "ملفات" — من الزيارة الجاية هنعرضلك هنا كل حاجة جديدة اتضافت من آخر مرة دخلت فيها.</div>`}
    ${lastRun?`<div style="margin-top:2px;margin-bottom:6px;font-size:12.5px;color:var(--ink-muted);">آخر بحث تلقائي بالـAI: ${new Date(lastRun).toLocaleString("ar-EG")}</div>`:""}
  </div>

  <div class="section-label">متابعتي للفرص</div>
  <div class="cat-chip-row">
    <button type="button" class="pill ${!state.trackFilter?"selected":""}" data-action="track-filter" data-status="">الكل (${items.length && !state.trackFilter?items.length:Object.keys(tracking).length})</button>
