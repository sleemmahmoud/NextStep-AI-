...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  ...
  const result = await callAiProxy("/chat", { systemInstruction: {parts:[{text: systemText}]}, contents }, {timeoutMs:35000, retries:1});
  if(!result.ok){
    if(result.reason==="quota") return {ok:false, text: result.message || "⚠️ وصلت للحد اليومي لاستخدام المساعد الذكي، جرب تاني بكرة."};
    if(result.reason==="timeout") return {ok:false, text: `⚠️ ${result.message} جرب تاني.`};
    if(result.reason==="network") return {ok:false, text: "مقدرتش أوصل لخدمة الذكاء الاصطناعي دلوقتي، تأكد من اتصال الإنترنت وجرب تاني."};
    return {ok:false, text: `⚠️ حصل خطأ: "${result.message}"`};
  }
  const data = result.data;
  const cand = data && data.candidates && data.candidates[0];
