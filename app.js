...
function calcMatch(profile, opp){
  ...
  // ملحوظة: لازم نتعامل مع stageTags=[] (مصفوفة فاضية) زي ما لو كانت مش موجودة
  // خالص، يعني "متاحة لكل المراحل". قبل كده الكود كان بيشوف [] على إنها true
  // (المصفوفة الفاضية truthy في JS)، فكان بيدخل يفحص .includes() على مصفوفة
  // فاضية وترجع false دايمًا — يعني كل فرصة مضافة بالـAI (اللي بتتحفظ بـ
  // stageTags:[] افتراضيًا) كانت بتاخد عقوبة stageOk=false تلقائيًا مهما كان
  // بروفايل المستخدم، وده اللي كان بيطلع نسبة 18% ثابتة لكل الفرص ولكل الناس.
...
async function generateCV(){
  ...
  if(!allowed){ alert(`وصلت للحد اليومي لعمل الـCV بالتصميم المميز (${CV_DAILY_LIMIT} في اليوم). جرب تاني بكرة.`); return; }
...
async function downloadAtsCv(){
  ...
  if(!allowed){ alert(`وصلت للحد اليومي لعمل الـCV بصيغة ATS (${CV_DAILY_LIMIT} في اليوم). جرب تاني بكرة.`); return; }
  state.cvGenerating = true; render();
  let p = {...state.profile, email: state.profile.contactEmail || state.user.email};
  try{
    p = await polishCvContent(p);
...
function isOppExpired(o){
  // أي فرصة من غير تاريخ deadline حقيقي بصيغة واضحة بقت بتتحجب هي كمان،
  // مش بس الفرص اللي معادها فات. من دلوقتي البحث بالـAI مبقاش يرجع فرصة
  // من غير تاريخ مؤكد أصلاً، فده بس شبكة أمان إضافية للبيانات القديمة.
  if(!/^\d{4}-\d{2}-\d{2}$/.test(o.deadline||"")) return true;
  const today = new Date(new Date().toISOString().slice(0,10));
...
function renderProfileTab(){
  ...
  <div style="font-size:11.5px;color:var(--ink-muted);margin-bottom:10px;">متبقي ليك النهاردة: ${cvUsageRemaining("design")} من ${CV_DAILY_LIMIT} (تصميم مميز) — ${cvUsageRemaining("ats")} من ${CV_DAILY_LIMIT} (ATS)</div>
...
function renderAiSearchPanel(){
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:16px;">
    <button type="button" class="btn btn-ghost" data-action="ai-search-courses" ${state.searchBusy?"disabled":""}>${state.searchBusy?"بيدور...":"🎓 دور على كورسات لنفس الموضوع"}</button>
  </div>
...
function renderChatTab(){
  ...
  return `
  <div class="chat-wrap">
    ${state.aiConnected?`<div class="note-box chat-status" style="background:#EAF6EF;color:#1F7A47;">متصل بـ Gemini ✓ — ${isAdmin()?"استخدامك كأدمن غير محدود":`متبقي ليك النهاردة ${aiUsageRemaining()} من ${AI_DAILY_LIMIT} طلب (شات + بحث ذكي مع بعض)`}. لو ظهرلك رسالة خطأ، ابعتهالي زي ما هي.</div>`:""}
...
// دلوقتي الحد ده بيغطي الشات وتحسين الـCV بس (البحث الشخصي اتشال خالص)، فقلّلناه.
...
async function tryConsumeAiUsage(){
  if(isAdmin()) return true; // الأدمن مالوش حد يومي خالص، لا في الواجهة ولا في السيرفر
...
// حد منفصل لعدد مرات عمل الـCV في اليوم، لكل مستخدم لوحده — ودلوقتي كل نسخة
// (تصميم مميز / ATS) بعداد مستقل عن التانية بالكامل، يعني مسموح بواحد تصميم
// وواحد ATS في نفس اليوم، مش واحد بس مشترك بين الاتنين.
const CV_DAILY_LIMIT = 1;
function getCvUsage(type){
...
function cvUsageRemaining(type){
  return Math.max(0, CV_DAILY_LIMIT - getCvUsage()[type]);
}
async function tryConsumeCvUsage(type){
  const usage = getCvUsage();
  if(usage[type] >= CV_DAILY_LIMIT) return false;
...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  ...
  try{
    ...
    const sourceHosts = [...new Set((gm && gm.groundingChunks || []).map(c=>c.web && (c.web.title||"").toLowerCase().replace(/^www\./,"")).filter(Boolean))];
...
const AUTO_SEARCH_TOPICS = ["منح دراسية للطلاب المصريين","تدريب صيفي للطلاب والخريجين الجدد","فرص تطوع للشباب","مسابقات ريادة أعمال وتكنولوجيا للطلاب","كورسات مجانية معتمدة أونلاين","مؤتمرات دولية ومحلية للطلاب والشباب","وظائف مبتدئين وحديثي التخرج","بوت كامب تدريبي مكثف في التكنولوجيا","برامج تبادل ثقافي وطلابي دولية"];
...
async function fetchAiOpportunities(topic){
  ...
  const prompt = `النهاردة تاريخ ${todayStr} (يعني إحنا في سنة ${currentYear}). استخدم بحث جوجل الحقيقي والحي دلوقتي (متعتمدش على معرفتك القديمة بس) عشان تلاقي أشهر وأهم الفرص **الحقيقية والمعروفة والمتكررة سنويًا** (بحد أقصى 15 فرصة) اللي مناسبة لموضوع: "${topic}"، وتستهدف طلاب أو خريجين مصريين أو فرص دولية متاحة لهم.
...
مصادر مقترحة (رشّح منها بس اللي فعلًا معروف ومستمر، وابحث فعليًا للتأكد): Coursera وedX وUdemy وFutureLearn وMicrosoft Learn وfreeCodeCamp وUN Volunteers وUN Careers وErasmus+ وEuropean Youth Portal وDAAD وChevening وBritish Council، وكمان مصادر مصرية زي وزارة الشباب والرياضة وITI وDigital Egypt وNTI وTIEC.
...
- **لازم تلاقي تاريخ deadline حقيقي ومحدد (YYYY-MM-DD) من المصدر نفسه.** لو مش لاقي تاريخ واضح ومؤكد لآخر موعد، **متضيفش الفرصة دي خالص** في النتيجة — ممنوع ترجع أي فرصة من غير تاريخ deadline حقيقي، حتى لو الفرصة نفسها موجودة وحقيقية.
- رشّح بس برامج مستقرة ومعروفة إنها بتتكرر كل سنة أو مستمرة طول الوقت. متختلقش اسم برنامج أو جهة مش متأكد من وجودها الحقيقي.
- الرابط لازم يكون رابط حقيقي من نتيجة البحث بتاعتك (الموقع الرسمي للجهة، أو صفحة التقديم مباشرة)، مش رابط مخترع.
- لكل فرصة، صنّفها بدقة على حسب محتواها الفعلي باستخدام الحقلين التاليين:
  1) "tags": اختار من القايمة دي بالظبط (ممكن أكتر من وسم لو مناسب، اكتبهم بالظبط زي ما هم من غير تعديل): [${TAGS.map(t=>`"${t}"`).join(", ")}]
  2) "stageTags": المراحل الدراسية المناسبة، اختار من القايمة دي بالظبط: ["middle","high","university","graduate"]. لو الفرصة مناسبة لكل المراحل رجّع مصفوفة فاضية [].
رجّع بس JSON array، من غير أي نص تاني قبله أو بعده، بالشكل ده بالظبط:
[{"title":"عنوان الفرصة","organization":"اسم الجهة","description":"وصف قصير من سطرين بالعربي دايمًا حتى لو المصدر إنجليزي، واذكر فيه إنها فرصة تتكرر سنويًا لو معروف كده","category":"scholarship أو internship أو job أو volunteering أو competition أو conference أو hackathon أو exchange أو course أو bootcamp أو event","deadline":"YYYY-MM-DD لازم تاريخ حقيقي ومؤكد","link":"رابط الموقع الرسمي المعروف للجهة","tags":["وسم1","وسم2"],"stageTags":["university"]}]
لو مش متأكد إن الفرصة دي حقيقية وموجودة ومفتوحة فعلًا دلوقتي وليها تاريخ deadline مؤكد، متضيفهاش خالص. لو مفيش فرص مفتوحة كفاية للموضوع ده رجّع [].`;
  try{
    const res = await fetch(GEMINI_PROXY_URL, {
      method:"POST",
      headers:{"Content-Type":"application/json"},
    ...
    // ملحوظة: uri الرجعة من groundingChunks بتكون رابط تحويل (redirect) بتاع
    // Google مش الدومين الحقيقي، فالاسم الحقيقي للمصدر موجود في title بس.
    const sourceHosts = sources.map(s=>(s.title||"").toLowerCase().replace(/^www\./,"")).filter(Boolean);
...
async function fetchAiCourses(topic){
  const todayStr = new Date().toISOString().slice(0,10);
  const prompt = `النهاردة تاريخ ${todayStr}. استخدم بحث جوجل الحقيقي والحي دلوقتي (متعتمدش على معرفتك القديمة بس) عشان تلاقي أشهر وأهم الكورسات المجانية أو شبه المجانية **الحقيقية والمعروفة والمتاحة فعلاً دلوقتي** (بحد أقصى 12 كورس) عن موضوع: "${topic}"، مناسبة لطلاب أو خريجين مصريين.
ركّز بس على منصات معروفة وموثوقة: Coursera، edX، FutureLearn، Udemy (الكورسات المجانية بس)، Microsoft Learn، freeCodeCamp، Khan Academy، Google Digital Garage / Skillshop، Google Career Certificates، Udacity (النسخة المجانية).
شروط أساسية ولازم تلتزم بيها بدقة:
- **افتح فعليًا (عن طريق البحث) صفحة كل كورس قبل ما ترشحه، وتأكد إنه لسه متاح ومنشور فعلاً دلوقتي** ومش صفحة قديمة اتشالت أو اتلغت.
- الرابط لازم يكون رابط حقيقي مباشر لصفحة الكورس نفسه من نتيجة البحث بتاعتك، مش رابط عام للمنصة ومش رابط مخترع.
- حدد بصراحة في "isPremium": true لو المحتوى نفسه محتاج اشتراك مدفوع عشان توصله، أو false لو المحتوى مجاني بالكامل أو متاح بنظام Audit مجاني (حتى لو الشهادة الرسمية مدفوعة).
- صنّف كل كورس بدقة باستخدام "tags" — اختار من القايمة دي بالظبط (ممكن أكتر من وسم لو مناسب): [${TAGS.map(t=>`"${t}"`).join(", ")}]
- الوصف يكون بالعربي دايمًا حتى لو الكورس بالإنجليزي.
رجّع بس JSON array، من غير أي نص تاني قبله أو بعده، بالشكل ده بالظبط:
[{"title":"عنوان الكورس + اسم المنصة بين قوسين","description":"وصف قصير من سطرين بالعربي","link":"رابط صفحة الكورس المباشر","tags":["وسم1","وسم2"],"isPremium":false}]
لو مش متأكد إن الكورس ده حقيقي وموجود ومتاح فعلًا دلوقتي، متضيفهوش خالص. لو مفيش كورسات موثوقة كفاية للموضوع ده رجّع [].`;
  try{
    const res = await fetch(GEMINI_PROXY_URL, {
      method:"POST",
      headers:{"Content-Type":"application/json"},
    ...
    let items;
    try{ items = JSON.parse(jsonMatch[0]); }catch(e){ return {error:"معرفتش أقرا رد الـAI، جرب تاني."}; }
    items = items.filter(it=>isValidLink(it.link));
    if(sourceHosts.length){
      items = items.filter(it=>{
        const h = hostnameOf(it.link);
        return h && sourceHosts.some(sh=>sh===h || sh.endsWith("."+h) || h.endsWith("."+sh));
    ...
    if(!items.length) return {error:"الـAI مالقاش كورسات موثوقة كفاية للموضوع ده دلوقتي، جرب تصيغه بطريقة تانية."};
    return {items};
  }catch(err){
    return {error:"مقدرتش أوصل للخدمة دلوقتي، تأكد من الإنترنت وجرب تاني."};
  }
}
// نفس فكرة searchForOpportunities بالظبط بس بتحفظ في resources مش opportunities،
// وبتتأكد إن الرابط مش موجود قبل كده (dedupe) عشان مانكررش نفس الكورس.
async function searchForCourses(topic){
  state.searchBusy = true; state.searchErr=""; render();
  const result = await fetchAiCourses(topic);
  ...
  for(const it of result.items){
    ...
    await addDoc(collection(db,"resources"), {
      title: it.title||"كورس بدون عنوان", type:"course", description: it.description||"",
      link: it.link||"", tags: safeTags, isPremium: !!it.isPremium
    });
  ...
  state.adminMsg = added ? `تمت إضافة ${added} كورس جديد للمصادر التعليمية ✓` : "الكورسات اللي لقيتها موجودة أصلًا في المصادر.";
  render();
  setTimeout(()=>{ state.adminMsg=""; render(); }, 4000);
}
...
async function searchForOpportunities(topic){
  ...
  for(const it of result.items){
    const safeTags = Array.isArray(it.tags) ? it.tags.filter(t=>TAGS.includes(t)) : [];
    const safeStages = Array.isArray(it.stageTags) ? it.stageTags.filter(s=>STAGES.some(x=>x.id===s)) : [];
...
app.addEventListener("click",(e)=>{
  ...
  else if(action==="ai-search-courses"){
    const topic = document.getElementById("ai-search-input").value.trim();
    if(topic) searchForCourses(topic);
  }
