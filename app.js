...
// تثبيت/إلغاء تثبيت فرصة كإعلان مهم — الفرص المثبّتة بتظهر في بانر ثابت أعلى
// قائمة الفرص المقترحة لكل المستخدمين، بغض النظر عن مرحلتهم أو نسبة التطابق.
async function toggleOppPinned(id){
  if(!isAdmin()) return;
  const o = state.opportunities.find(x=>x.id===id);
  if(!o) return;
  try{
    await updateDoc(doc(db,"opportunities", id), {isPinned: !o.isPinned});
    await loadOpportunities();
  }catch(err){
    console.error("toggleOppPinned error:", err);
    state.toastMsg = "حصل خطأ أثناء التثبيت، جرب تاني.";
    render(); setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
  }
}
// نفس الفكرة بالظبط بس للكورسات — كورس مثبّت بيظهر برضه في نفس بانر الإعلانات
// فوق قائمة الفرص المقترحة، عشان محتوى تعليمي مهم جدًا يوصل لكل الشباب.
async function toggleResourcePinned(id){
  if(!isAdmin()) return;
  const r = state.resources.find(x=>x.id===id);
  if(!r) return;
  try{
    await updateDoc(doc(db,"resources", id), {isPinned: !r.isPinned});
    await loadResources();
  }catch(err){
    console.error("toggleResourcePinned error:", err);
    state.toastMsg = "حصل خطأ أثناء التثبيت، جرب تاني.";
    render(); setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
  }
}
...
async function generateCertificateText(courseName, userName){
  ...
  try{
    ...
    const res = await fetch(GEMINI_PROXY_URL, {
      ...
      body: JSON.stringify({ contents:[{role:"user", parts:[{text:prompt}]}], uid: state.user && state.user.uid, feature:"cv" })
...
// دالة موحّدة لأي استدعاء AI محتاج مهلة زمنية أطول (تحسين CV، البحث عن فرص).
// المساعد الذكي (الشات) سريع أصلًا ومش محتاج ده. الفكرة: بدل ما نفشل على طول
// أول ما نلاقي timeout أو مشكلة شبكة/سيرفر مؤقتة، نجرب تاني مرة واحدة زيادة
// قبل ما نستسلم — ده اللي كان مفقود وبيخلي البحث وتحسين الـCV يفشلوا أسرع
// وأكتر من الشات من غير داعي. مبنعيدش المحاولة أبدًا لو السبب quota (429)
// لإن ده مش هيتصلح بإعادة المحاولة.
async function callAiWithRetry(body, {timeoutMs=45000, retries=1}={}){
  let lastErr = null;
  for(let attempt=0; attempt<=retries; attempt++){
    ...
    try{
      ...
      if(!res.ok && res.status!==429 && attempt<retries){
        console.warn(`callAiWithRetry: HTTP ${res.status} في المحاولة ${attempt+1}، بيجرب تاني...`);
        continue;
      }
      return {ok:true, res};
    }catch(err){
      ...
      lastErr = err;
      if(attempt<retries){
        console.warn(`callAiWithRetry: فشلت المحاولة ${attempt+1} (${err && err.name})، بيجرب تاني...`);
        continue;
      }
    }
  }
...
رد بصيغة JSON بس من غير أي كلام زيادة قبله أو بعده ومن غير أي علامات markdown، بالشكل ده بالظبط: {"goal":"النص المحسّن للهدف","achievements":"سطر1\\nسطر2\\nسطر3"}`;
    const {ok, res, err:callErr} = await callAiWithRetry({
      contents:[{role:"user", parts:[{text: prompt}]}],
      uid: state.user && state.user.uid,
      feature:"cv"
    }, {timeoutMs:45000, retries:1});
    if(!ok){
      console.error("polishCvContent: call failed after retry:", callErr);
      const reason = (callErr && callErr.name==="AbortError") ? "الـAI استغرق وقت طويل جدًا حتى بعد إعادة المحاولة" : "مشكلة شبكة أثناء الاتصال بالـAI حتى بعد إعادة المحاولة";
      return {profile:p, polished:false, reason};
    }
    if(!res.ok){
      const errData = await res.json().catch(()=>null);
      const msg = (errData && errData.error && errData.error.message) || `كود ${res.status}`;
      console.error("polishCvContent: server rejected:", msg);
      return {profile:p, polished:false, reason: res.status===429 ? "وصلت للحد اليومي لاستخدام الذكاء الاصطناعي" : `السيرفر رفض الطلب: ${msg}`};
    }
    const data = await res.json();
    const cand = data && data.candidates && data.candidates[0];
    const text = (cand && cand.content && cand.content.parts && cand.content.parts.map(x=>x.text||"").join("") || "").trim();
    ...
    return {profile:p, polished:false, reason:"حصل خطأ غير متوقع أثناء تحسين الصياغة"}; // أي مشكلة تانية (JSON مش مظبوط...) نرجع للنص الأصلي بدل ما نوقف الـCV
  }
}
...
async function submitAdminResource(form){
  ...
  const section = type==="course" ? "courses" : (fd.get("section")==="practical" ? "practical" : "tips");
  const res = { title: fd.get("title"), type, section, description: fd.get("description")||"", tags: [...state.adminResTags], isPremium: fd.get("isPremium")==="true" };
  if(type==="course"){
...
function renderOppList(){
  ...
  // بانر الإعلانات: فرص وكورسات ثبّتها الأدمن يدويًا — بتظهر فوق كل حاجة تانية
  // لكل المستخدمين بغض النظر عن المرحلة أو البحث أو الفلاتر، زي إعلان رسمي.
  const pinnedOpps = list.filter(o=>o.isPinned);
  const pinnedCourses = (state.resources||[]).filter(r=>r.type==="course" && r.isPinned);
  const pinnedBanner = (pinnedOpps.length || pinnedCourses.length) ? `
    <div class="section-label">📌 مهم لكل الشباب</div>
    ${pinnedOpps.map(o=>`
      <div class="card opp-card" style="border:1.5px solid var(--gold-300);" data-action="open-detail" data-id="${o.id}">
        <div class="opp-main">
          <span class="opp-cat">${CATEGORIES[o.category]||o.category}</span>
          <span class="badge-verified" style="background:#FDF3E2;color:#8A5B10;">📌 إعلان</span>
          <div class="opp-title">${escapeHtml(o.title)}</div>
          <div class="opp-org">${escapeHtml(o.organization||"")}</div>
        </div>
      </div>`).join("")}
    ${pinnedCourses.map(r=>`
      <div class="card opp-card" style="border:1.5px solid var(--gold-300);" data-action="tab" data-tab="resources">
        <div class="opp-main">
          <span class="opp-cat">كورس</span>
          <span class="badge-verified" style="background:#FDF3E2;color:#8A5B10;">📌 إعلان</span>
          <div class="opp-title">${escapeHtml(r.title)}</div>
          ${r.description?`<div class="opp-org">${escapeHtml(r.description)}</div>`:""}
        </div>
      </div>`).join("")}
  ` : "";
  ...
  if(list.length===0){
    if(state.searchQuery.trim()){
      return pinnedBanner + searchBar + `<div class="empty-state"><h3>مفيش نتايج لـ "${escapeHtml(state.searchQuery)}"</h3><p>جرب كلمة تانية أو امسح البحث.</p></div>`;
    }
    return pinnedBanner + searchBar + `<div class="empty-state"><h3>مفيش فرص متاحة دلوقتي</h3><p>لو انت الأدمن، ضيف فرص حقيقية من تبويب "الإدارة" — يدويًا أو بالبحث بالـAI.</p></div>`;
  }
  // الفرص المثبّتة ظاهرة أصلًا في بانر الإعلانات فوق، فبنستبعدها من القائمة
  // العادية تحت عشان ما تتكررش.
  const withScores = list.filter(o=>!o.isPinned).map(o=>({o, m:calcMatch(state.profile,o)}));
  ...
  return pinnedBanner + searchBar + (withScores.length===0 ? "" : withScores.map(({o,m})=>{
...
function renderAdminResourceForm(){
  ...
  <form id="admin-res-form">
    ...
    <div class="field"><label>يظهر في قسم (متجاهلة لو النوع "كورس" — الكورسات بتروح لتبويب الكورسات تلقائيًا)</label><select name="section"><option value="tips">💡 نصائح (فيديو قصير)</option><option value="practical">🛠️ نصائح عملية (فيديو طويل)</option></select></div>
...
function renderResourceCard(r){
  ...
  <div class="card" style="padding:16px;margin-bottom:12px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center;">
      ...
      ${isAdmin() && isCourse?`<button class="pill ${r.isPinned?"selected":""}" data-action="toggle-pin-resource" data-id="${r.id}">${r.isPinned?"📌 مثبّت كإعلان":"📌 ثبّته كإعلان"}</button>`:""}
...
function renderResourcesTab(){
  ...
  // section الافتراضي "tips" لو مصدر قديم متسجّلش عليه section أصلًا (توافق مع
  // المحتوى اللي كان موجود قبل إضافة القسم التالت).
  const tips = state.resources.filter(r=>r.type!=="course" && (r.section||"tips")==="tips");
  const practical = state.resources.filter(r=>r.type!=="course" && r.section==="practical");
  const courses = state.resources.filter(r=>r.type==="course");
  const sub = ["tips","practical","courses"].includes(state.learningSubTab) ? state.learningSubTab : "tips";
  const subTabs = `
    <div class="tabs" style="margin-bottom:14px;">
      <button class="tab ${sub==="tips"?"active":""}" data-action="learning-subtab" data-subtab="tips">💡 نصائح</button>
  ...
  const list = sub==="courses" ? courses : (sub==="practical" ? practical : tips);
  const emptyMsg = sub==="courses"
    ? `<div class="empty-state"><h3>لسه مفيش كورسات</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`
    : (sub==="practical"
      ? `<div class="empty-state"><h3>لسه مفيش نصائح عملية</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`
...
const CV_DAILY_LIMIT = 2;
const ADMIN_CV_DAILY_LIMIT = 6;
...
استخدم البيانات دي عشان تدّي نصايح مخصصة قدر الإمكان، ومتقولش إنك موديل لغوي أو تتكلم عن نفسك كـAI إلا لو اتسأل مباشرة.`;
  ...
  try{
    const controller = new AbortController();
    const timeoutId = setTimeout(()=>controller.abort(), 25000); // 25 ثانية بالحد الأقصى بدل ما يفضل معلّق لغير نهاية
    let res;
    try{
      res = await fetch(GEMINI_PROXY_URL + "/chat", {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
    ...
    if(err && err.name==="AbortError") return {ok:false, text: "⚠️ المساعد استغرق وقت طويل جدًا (أكتر من 25 ثانية) ومردش، جرب تاني — ممكن يكون في ضغط مؤقت على الخدمة."};
...
async function fetchAiOpportunities(topic){
  ...
  const prompt = `النهاردة ${todayStr} (سنة ${currentYear}). من معرفتك (من غير بحث حي)، اقترح 5 فرص حقيقية ومعروفة ومستقرة (بتتكرر سنويًا أو مستمرة) مناسبة لموضوع: "${topic}"، لطلاب/خريجين مصريين أو فرص دولية متاحة لهم.
"فرصة" = منحة/تدريب/وظيفة/تطوع/مسابقة/هاكاثون/تبادل/كورس مجاني/بوت كامب/مؤتمر — حسب الموضوع، مش بس منح.
قواعد مهمة: رشّح بس برامج مستقرة معروفة (متختلقش اسم جهة). لو مش متأكد من تاريخ الـdeadline بالظبط اكتب "غير معلن - راجع الموقع الرسمي" بدل ما تخمّن. الرابط لازم يكون الموقع الرسمي الحقيقي للجهة.
لكل فرصة حدد "tags" (1-4 من: [${TAGS.map(t=>`"${t}"`).join(", ")}]) و"stageTags" (من: ["middle","high","university","graduate"]، أو [] لو للكل).
رجّع JSON array بس من غير أي نص زيادة، بالشكل ده: [{"title":"","organization":"","description":"وصف سطرين بالعربي","category":"scholarship أو internship أو job أو volunteering أو competition أو conference أو hackathon أو exchange أو course أو bootcamp أو event","deadline":"YYYY-MM-DD أو: غير معلن - راجع الموقع الرسمي","link":"","tags":[],"stageTags":[]}]
لو مش متأكد إن الفرصة حقيقية، متضيفهاش. لو مفيش فرص كفاية رجّع [].`;
  try{
    // مهلة أطول من الشات (45 ثانية × محاولتين لو الأولى فشلت بسبب طول الرد أو
    // مشكلة سيرفر مؤقتة) لإن الطلب ده بيولّد لحد 5 فرص بتفاصيلها كـJSON منظّم،
    // وده بياخد وقت أطول من رد شات عادي.
    const {ok, res, err:callErr} = await callAiWithRetry({
      contents:[{role:"user", parts:[{text:prompt}]}],
    ...
    if(!ok){
      console.error("fetchAiOpportunities: call failed after retry:", callErr);
      return {error: (callErr && callErr.name==="AbortError") ? "⚠️ الـAI استغرق وقت طويل جدًا حتى بعد إعادة المحاولة، جرب تاني كمان شوية." : "⚠️ مشكلة شبكة أثناء الاتصال بالـAI حتى بعد إعادة المحاولة."};
    }
    const data = await res.json();
    if(!res.ok){
      if(res.status===429){
    ...
    if(!jsonMatch){
      ...
      return {error:`الـAI مرجعش نتايج واضحة. [تشخيص: ${diag}] الرد الفعلي كان:\n${text ? text.slice(0,400) : "(رد فاضي تمامًا)"}`};
    }
    let items;
    ...
    console.error("fetchAiOpportunities error:", err);
    return {error:"حصل خطأ غير متوقع أثناء قراءة رد الـAI، جرب تاني."};
  }
}
...
[{"id":"...","tags":["وسم1","وسم2"],"stageTags":["high","university"]}]`;
    try{
      const res = await fetch(GEMINI_PROXY_URL, {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ contents:[{role:"user", parts:[{text:prompt}]}], uid: state.user && state.user.uid, feature:"classify" })
        break;
      }
      const cand = data && data.candidates && data.candidates[0];
      const text = cand && cand.content && cand.content.parts && cand.content.parts.map(p=>p.text||"").join("") || "";
...
["id1","id2"]`;
    try{
      const res = await fetch(GEMINI_PROXY_URL, {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ contents:[{role:"user", parts:[{text:prompt}]}], uid: state.user && state.user.uid, feature:"cleanup" })
      const cand = data && data.candidates && data.candidates[0];
      const text = cand && cand.content && cand.content.parts && cand.content.parts.map(p=>p.text||"").join("") || "";
      const jsonMatch = text.match(/\[[\s\S]*\]/);
      let ids = [];
...
async function searchForOpportunities(topic){
  ...
  const writes = result.items.map(it=>{
    const safeTags = Array.isArray(it.tags) ? it.tags.filter(t=>TAGS.includes(t)) : [];
    const validStages = STAGES.map(s=>s.id);
    const safeStages = Array.isArray(it.stageTags) ? it.stageTags.filter(s=>validStages.includes(s)) : [];
...
function renderModal(){
  ...
  <div class="modal-overlay" data-action="close-detail">
    <div class="modal-sheet" data-action="noop">
      ...
      <button class="pill ${o.isPinned?"selected":""}" style="margin-top:8px;width:100%;" data-action="toggle-pin-opp" data-id="${o.id}">${o.isPinned?"📌 مثبّتة كإعلان — دوس لإلغاء التثبيت":"📌 ثبّتها كإعلان في أعلى الفرص"}</button>`:""}
    </div>
  </div>`;
}
...
app.addEventListener("click",(e)=>{
  ...
  else if(action==="toggle-pin-opp"){ toggleOppPinned(t.dataset.id); }
  else if(action==="toggle-pin-resource"){ toggleResourcePinned(t.dataset.id); }
