...
async function fetchAiOpportunities(topic){
  ...
  const prompt = `النهاردة تاريخ ${todayStr} (يعني إحنا في سنة ${currentYear}). استخدم بحث جوجل الحقيقي والحي دلوقتي (متعتمدش على معرفتك القديمة بس) عشان تلاقي لحد 15 فرصة حقيقية ومعروفة ومناسبة لموضوع: "${topic}"، تستهدف طلاب أو خريجين مصريين أو فرص دولية متاحة لهم.
"فرصة" = منحة/تدريب/وظيفة/تطوع/مسابقة/هاكاثون/تبادل/كورس مجاني/بوت كامب/مؤتمر — حسب الموضوع، مش بس منح.
شروط مهمة:
- افتح فعليًا (عن طريق البحث) صفحة كل فرصة قبل ما ترشحها، وتأكد إنها حقيقية وموجودة فعلًا.
