...
async function submitAdminResource(form){
  ...
  if(type==="course"){
    ...
    const lessons = lines.map(line=>{
      ...
      return (title && isValidLink(link)) ? {title, link} : null;
    }).filter(Boolean);
    if(lessons.length===0){
...
function renderAdminTab(){
  ...
  <div class="card" style="padding:20px;margin-top:0;">
    ...
    <div class="note-box" style="margin-bottom:12px;">
      عندك ${n} فرصة قديمة من غير وسوم (تصنيف) — عشان كده نسبة التطابق بتاعتها بتطلع 50% ثابتة لأي حد. دوس الزرار ده والـAI هيصنّفها كلها مرة واحدة.
      <div style="margin-top:8px;"><button type="button" class="btn btn-gold" data-action="bulk-fix-tags" ${state.searchBusy?"disabled":""}>${state.searchBusy?"بيصنّف...":"🏷️ صلّح وسوم الفرص القديمة تلقائيًا"}</button></div>
    </div>`:""; })()}
...
function renderAdminResourceForm(){
  ...
  <form id="admin-res-form">
    ...
    <div class="field"><label>فيديوهات الكورس (لو النوع "كورس" بس — سطر لكل فيديو)</label>
      <textarea name="lessonsRaw" rows="5" placeholder="عنوان الفيديو | رابط اليوتيوب"></textarea>
    </div>
    <div class="note-box">مثال سطر: المقدمة وأساسيات المونتاج | https://youtu.be/xxxxxxxx</div>
...
function renderResourcesTab(){
  const introNote = `<div class="note-box" style="margin-bottom:14px;">📚 دي كورساتي وملفاتي واختباراتي الخاصة اللي بحطها بنفسي.</div>`;
  if(state.resources.length===0){
    if(isAdmin()){
      return introNote + `<div class="empty-state"><h3>لسه مفيش مصادر تعليمية</h3><p>ضيف أول مصدر من تبويب "الإدارة".</p></div>`;
    }
    return introNote + `<div class="empty-state"><h3>لسه مفيش مصادر تعليمية</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`;
  }
  const typeLabel = {video:"فيديو",pdf:"ملف PDF",article:"مقال",course:"كورس (فيديوهات)"};
  return introNote + state.resources.map(r=>{
...
async function bulkFixOpportunityTags(){
  if(!isAdmin()) return;
  const untagged = state.opportunities.filter(o=>!(o.tags&&o.tags.length)).slice(0,40);
  ...
  const allowed = await tryConsumeAiUsage();
  if(!allowed){
    state.adminMsg = `وصلت للحد اليومي لاستخدام الذكاء الاصطناعي (${AI_DAILY_LIMIT} طلب في اليوم). جرب تاني بكرة.`;
    render(); setTimeout(()=>{ state.adminMsg=""; render(); }, 4000); return;
  }
  state.searchBusy = true; render();
  const list = untagged.map(o=>`{"id":"${o.id}","title":${JSON.stringify(o.title||"")},"description":${JSON.stringify(o.description||"")}}`).join(",\n");
  const prompt = `دي قايمة فرص من منصة NextStep AI مالهاش تصنيف (tags) دلوقتي. لكل فرصة، اختار 1 لـ4 وسوم مناسبة بس من القايمة دي بالظبط (من غير ما تخترع وسم برا القايمة): [${TAGS.map(t=>`"${t}"`).join(", ")}]
الفرص:
[${list}]
...
[{"id":"...","tags":["وسم1","وسم2"]}]`;
  try{
    ...
    const data = await res.json();
    if(!res.ok){
      state.adminMsg = (data && data.error && data.error.message) || "حصل خطأ أثناء التصنيف، جرب تاني.";
      state.searchBusy=false; render(); setTimeout(()=>{ state.adminMsg=""; render(); }, 5000); return;
    }
    const cand = data && data.candidates && data.candidates[0];
    const text = cand && cand.content && cand.content.parts && cand.content.parts.map(p=>p.text||"").join("") || "";
    const jsonMatch = text.match(/\[[\s\S]*\]/);
    ...
    for(const it of items){
      ...
      try{
        await updateDoc(doc(db,"opportunities", it.id), {tags: safeTags});
        fixed++;
    ...
    state.adminMsg = fixed
      ? `تم تصنيف ${fixed} فرصة ✓${remaining?` — لسه فاضل ${remaining}، دوس الزرار تاني.`:""}`
      : "الـAI مرجعش تصنيف واضح، جرب تاني.";
  }catch(err){
  ...
  setTimeout(()=>{ state.adminMsg=""; render(); }, 5000);
}
async function searchForOpportunities(topic){
  ...
  for(const it of result.items){
    ...
    await addDoc(collection(db,"opportunities"), {
      ...
      tags: safeTags, stageTags:[], requirements:[], reviewed:true,
