...
let state = {
  ...
  learningSubTab:"tips",
  pendingOpps:[], searchTopic:"", searchBusy:false, searchErr:"", searchNote:"",
...
async function toggleOppFeatured(id){
  ...
  try{
    ...
    state.toastMsg = "حصل خطأ أثناء تحديث حالة التمييز، جرب تاني.";
  }
  render();
  setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
}
...
async function searchForOpportunities(topic){
  state.searchBusy = true; state.searchErr=""; state.searchNote=""; render();
