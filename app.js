...
let state = {
  ...
  searchModel:"gemini-3.5-flash", searchDepth:"deep", searchResultCount:15,
...
function renderDashboard(){
  ...
  const NAV_ITEMS = [
    ...
    {id:"tracking", icon:"📌", label:"متابعتي"},
    {id:"guides", icon:"📚", label:"أدلة ومصادر"},
    {id:"resources", icon:"🎓", label:"مركز التعلم"},
    {id:"assessments", icon:"🧠", label:"الاختبارات"},
    {id:"stories", icon:"🌟", label:"قصص النجاح"},
  ...
  app.innerHTML = `
    <div class="topbar">
      <div style="display:flex;align-items:center;gap:10px;">
        <button class="icon-btn nav-hamburger" data-action="toggle-nav" title="القائمة">☰</button>
    ...
    ${state.navOpen?`<div class="nav-backdrop" data-action="toggle-nav"></div>`:""}
    <div class="dash-body">
      <nav class="sidebar ${state.navOpen?"open":""} ${state.navCollapsed?"collapsed":""}">
        <button class="sidebar-collapse-btn" data-action="toggle-nav-collapse" title="${state.navCollapsed?"توسيع القائمة":"طي القائمة"}">${state.navCollapsed?"»":"« طي القائمة"}</button>
        ${NAV_ITEMS.map(it=>`<button class="nav-item ${tab===it.id?"active":""}" data-action="tab" data-tab="${it.id}" title="${it.label}"><span class="nav-icon">${it.icon}</span><span class="nav-label">${it.label}</span></button>`).join("")}
      </nav>
      <div class="main-col">
        ...
        <div class="content ${tab==="chat"?"content-chat":""}">
          ...
          ${tab==="tracking"?renderTrackingTab():""}
          ${tab==="guides"?renderGuidesTab():""}
          ${tab==="resources"?renderResourcesTab():""}
          ${tab==="assessments"?renderAssessmentsTab():""}
...
const HELP_TEXT = {
  overview:{title:"الرئيسية", body:"لوحة سريعة بتلخّصلك آخر الفرص المناسبة، ونسبة تقدمك، وأي حاجة جديدة اتضافت من آخر زيارة ليك. دوس على أي كارت عشان تفتح تفاصيله مباشرة."},
  ...
  tracking:{title:"متابعتي", body:"هنا بتتابع كل الفرص اللي قررت تقدّم عليها: غيّر حالتها (سأقدم، تم التقديم، مقبول..) وسجّل ملاحظاتك الشخصية عشان تفضل منظم ومتذكّر مواعيدك."},
  guides:{title:"أدلة ومصادر", body:"دليل جامعات وموارد مجانية (قوالب CV، أدوات، منح) تساعدك في رحلتك، من غير ما تدفع فلوس."},
  resources:{title:"مركز التعلم", body:"محتوى تعليمي مجاني مقسّم لكورسات كاملة (بشهادة إتمام)، ونصائح سريعة. اختار حسب الوقت المتاح ليك، وكمّل الدروس عشان الشهادة تفضل ليك."},
  assessments:{title:"الاختبارات", body:"اختبارات ثابتة وسريعة (قيادة، إدارة وقت، ميول مهنية..) بتساعدك تفهم نفسك أكتر، النتيجة بتتحفظ تلقائيًا في ملفك."},
  stories:{title:"قصص النجاح", body:"قصص حقيقية عن ناس حصلوا على منح وتطوع ومسابقات، ممكن تديك أفكار وتحفيز لرحلتك أنت."},
  chat:{title:"المساعد الذكي", body:"اسأل عن أي حاجة تخص تطوير مسارك المهني — تحسين الـCV، تعلم مهارة، الاستعداد لمقابلة، وغيرها. الردود جاهزة ومبنية على بيانات حقيقية، مش توليد حر."},
  profile:{title:"ملفي", body:"بياناتك الشخصية، اهتماماتك ومهاراتك (اللي بتحدد دقة الفرص المقترحة ليك)، وزرار تحميل CV احترافي بضغطة واحدة."},
...
function renderOverviewTab(){
  ...
  const xp = computeXP(p), level = computeLevel(xp);

  return `
  <div class="card" style="padding:20px;margin-top:0;">
    <h3 style="font-size:15.5px;font-weight:800;margin-bottom:10px;">أهلاً ${escapeHtml((p.name||"").split(" ")[0]||"")} 👋</h3>
    <div style="display:flex;gap:14px;flex-wrap:wrap;margin-bottom:14px;">
      <div class="pill" style="background:#FDF3E2;color:#8A5B10;">المستوى ${level} — ${xp} XP</div>
      ${(p.streakCount||0)>0?`<div class="pill">🔥 متتالية ${p.streakCount} يوم</div>`:""}
      <div class="pill">اكتمال الملف ${profileCompleteness(p)}%</div>
      <div class="pill">${live.length} فرصة متاحة دلوقتي</div>
    </div>
    ${lv ? `
    <div class="note-box" style="background:#EAF6EF;color:#1F7A47;margin-bottom:14px;">
    ...
    </div>` : `<div class="note-box" style="margin-bottom:14px;">أول مرة بتفتح "الرئيسية" — من الزيارة الجاية هنعرضلك هنا كل حاجة جديدة اتضافت من آخر مرة دخلت فيها.</div>`}

    <div class="section-label">متابعتي للفرص</div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px;">
      ${Object.entries(TRACK_STATUSES).map(([k,label])=>`<span class="pill">${label}: ${trackCounts[k]||0}</span>`).join("")}
    </div>
    <button type="button" class="link-btn" data-action="tab" data-tab="tracking">افتح "متابعتي" بالتفصيل ↗</button>

    ${lastRun?`<div style="margin-top:16px;font-size:12.5px;color:var(--ink-muted);">آخر بحث تلقائي بالـAI: ${new Date(lastRun).toLocaleString("ar-EG")}</div>`:""}
  </div>`;
}
function renderTrackingTab(){
  ...
  <div class="cat-chip-row">
    <button type="button" class="pill ${!state.trackFilter?"selected":""}" data-action="track-filter" data-status="">الكل</button>
    ${Object.entries(TRACK_STATUSES).map(([k,label])=>`<button type="button" class="pill ${state.trackFilter===k?"selected":""}" data-action="track-filter" data-status="${k}">${label}</button>`).join("")}
  </div>
...
function renderGuidesTab(){
  ...
  let body = "";
  if(sub==="universities"){
    body = UNIVERSITIES_GUIDE.map(group=>`
      <div class="section-label">${group.region}</div>
...
const ASSISTANT_KB = [
  ...
  {keys:["قصص نجاح","success stories","ناس نجحت","امثلة نجاح"], a:"شوف تبويب \"قصص النجاح\" فيه قصص حقيقية عن منح وتطوع ومسابقات لناس زيك، ممكن تديك دفعة وأفكار عملية تطبّقها في رحلتك."},
...
function renderStoriesTab(){
  const filtered = state.successStoryFilter ? state.successStories.filter(s=>s.category===state.successStoryFilter) : state.successStories;
  return `
  <div style="max-width:760px;margin:0 auto;">
    <h2 style="font-size:19px;font-weight:800;margin-bottom:10px;">قصص نجاح</h2>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;">
      <button class="pill ${state.successStoryFilter===""?"selected":""}" data-action="story-filter" data-cat="">الكل</button>
      ${Object.keys(STORY_CATEGORY_LABEL).map(c=>`<button class="pill ${state.successStoryFilter===c?"selected":""}" data-action="story-filter" data-cat="${c}">${STORY_CATEGORY_LABEL[c]}</button>`).join("")}
    </div>
    ${isAdmin()?renderAdminStoryForm():""}
    ${filtered.length===0?`<div class="note-box">لسه مفيش قصص في القسم ده.</div>`:
      filtered.map(s=>`
