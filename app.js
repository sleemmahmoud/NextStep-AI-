...
async function polishCvContent(p){
  ...
  if(!allowed) return {profile:p, polished:false};
  try{
...
async function submitAdminResource(form){
  ...
  if(type==="course"){
    ...
    res.lessons = lessons;
    res.link = "";
    // ملف PDF اختياري خاص بالكورس كله (مش لكل فيديو لوحده)
    const pdfLinkRaw = (fd.get("pdfLink")||"").trim();
    res.pdfLink = isValidLink(pdfLinkRaw) ? pdfLinkRaw : "";
    // روابط إضافية اختيارية (سطر لكل رابط: عنوان | رابط) — زي مصادر خارجية مرتبطة بالكورس
...
function renderOppList(){
  ...
  // الفرص اللي مصنّفة بالظبط لمرحلة المستخدم بتتحط الأول، وبعدين ترتيب الباقي
  // (بما فيه الفرص المتاحة للكل) حسب نسبة التطابق العادية.
  withScores.sort((a,b)=>{
...
function renderProfileTab(){
  ...
  <div style="font-size:11.5px;color:var(--ink-muted);margin-bottom:10px;">${isAdmin()?"استخدامك غير محدود (أدمن) لتحميل الـCV":`متبقي ليك النهاردة: ${cvUsageRemaining("design")} من ${CV_DAILY_LIMIT} (تصميم مميز) — ${cvUsageRemaining("ats")} من ${CV_DAILY_LIMIT} (ATS)`}</div>
...
function renderAdminResourceForm(){
  ...
  <form id="admin-res-form">
    ...
    <div class="field"><label>ملف PDF للكورس كله (اختياري — لو النوع "كورس")</label><input name="pdfLink" type="url" placeholder="https://..."></div>
    <div class="field"><label>روابط إضافية للكورس (اختياري — سطر لكل رابط)</label>
      <textarea name="extraLinksRaw" rows="3" placeholder="عنوان الرابط | https://..."></textarea>
    </div>
    <div class="note-box">الحقلين اللي فوق دول اختياريين تمامًا، سيبهم فاضيين عادي لو مش محتاجهم.</div>
  ...
  return introNote + state.resources.map(r=>{
    ...
    <div class="card" style="padding:16px;margin-bottom:12px;">
      ...
      ${isCourse ? `
        ...
        <div class="section-label">ملفات وروابط إضافية للكورس</div>
...
function renderChatTab(){
  ...
  return `
  <div class="chat-wrap">
    ${state.aiConnected?`<div class="note-box chat-status" style="background:#EAF6EF;color:#1F7A47;">متصل بـ Gemini ✓ — ${isAdmin()?"استخدامك غير محدود (أدمن)":`متبقي ليك النهاردة ${aiUsageRemaining()} من ${AI_DAILY_LIMIT} طلب (شات + بحث ذكي مع بعض)`}. لو ظهرلك رسالة خطأ، ابعتهالي زي ما هي.</div>`:""}
...
// رفعنا الحد من 5 لـ20 (أقصى حاجة معقولة من غير ما نضغط على الحصة المجانية
// لـGemini)، ولو عايز ترفعه أكتر لاحقًا لو فعّلت باقة مدفوعة، غيّر الرقم هنا
// وفي worker.ts مع بعض (لازم يفضلوا متطابقين).
// ملحوظة مهمة: السيرفر (worker.ts) أصلًا بيدّي الأدمن استخدام من غير حد خالص،
// فخلينا العداد هنا (بتاع الواجهة) يطابق نفس السلوك بدل ما يوقف الأدمن بالغلط.
...
// الأدمن (ADMIN_EMAILS) مالوش حد خالص — نفس السلوك بالظبط بتاع السيرفر.
...
// ATS في نفس اليوم من غير ما استخدام واحد يأثر على التاني. الأدمن معفى برضه.
