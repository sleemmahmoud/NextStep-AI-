...
// بيحوّل خطأ Firestore لرسالة واضحة بدل ما نفضل عايشين على رسالة عامة واحدة
// لكل حالات الفشل. لو مش permission-denied ولا فيه code معروف، بيرجّع نفس
// الرسالة الافتراضية اللي إحنا حاطينها أصلًا (سلوك زي الأول بالظبط).
function friendlyErr(err, fallback){
  if(err && err.code === "permission-denied"){
    return "مفيش صلاحية لعمل الحركة دي — راجع Firestore Security Rules.";
  }
  if(err && err.code){
...
let state = {
  ...
  learningSubTab:"tips", learningCenterSection:"content", religiousSubTab:"advice", profileSection:"info", chatSection:"ai",
  ...
  aggregatorTriggering:false, aggregatorLastRun:null, aggregatorMsg:"", connectorStatus:null,
...
// ⚠️ ملحوظة مهمة عن الأداء: ده لسه بيجيب مجموعة محدودة (مش كل حاجة) عشان
// منجيبش آلاف/مئات آلاف الفرص دفعة واحدة للمتصفح (ده كان بيجيب الـ
// collection كله قبل كده من غير أي limit خالص). عمدًا من غير orderBy هنا:
// لو فيه فرص قديمة في القاعدة من غير حقل createdAt، orderBy كان هيشيلها من
// النتيجة تمامًا (Firestore بيستبعد أي document ناقصه الحقل المرتّب بيه) —
// وده كان هيكسر ميزة موجودة فعليًا. الترتيب الفعلي (مميز/تاريخ...) بيحصل
// أصلًا client-side بعد الجلب. لو عدد الفرص هيتعدى 1000 فعليًا، لازم
// Pagination حقيقي (limit + startAfter) — راجع التقرير المرفق.
...
async function saveUniversityDraft(){
  ...
  const payload = {
    ...
    faculties: (document.getElementById("uni-d-faculties")||{}).value?.trim() || "",
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء الحفظ، حاول تاني.");
...
async function deleteUniversity(id){
  ...
  }catch(err){ console.error("deleteUniversity error:", err); state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حذف الجامعة."); setTimeout(()=>{state.toastMsg="";render();},4000); }
  render();
}
async function saveCollegeDraft(){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حفظ الكلية.");
    setTimeout(()=>{state.toastMsg=null; render();}, 2500);
  }
  render();
}
async function deleteCollege(uniId, idx){
  ...
  }catch(err){ console.error("deleteCollege error:", err); state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حذف الكلية."); setTimeout(()=>{state.toastMsg="";render();},4000); }
  render();
}
...
// دليل الكليات: كتالوج مستقل لكل كلية/تخصص موجود في مصر (زي "كلية الطب"،
// "كلية هندسة الحاسبات") بمعلوماته الكاملة، من غير ما يكون مربوط بجامعة
// واحدة بعينها — مختلف عن مصفوفة colleges الجوّه كل جامعة (اللي بتوصف
// إيه الكليات المتاحة في الجامعة دي بالذات).
async function loadCollegesGuide(){
  try{
    const snap = await getDocs(collection(db,"collegesGuide"));
    state.collegesGuide = snap.docs.map(d=>({id:d.id, ...d.data()}));
  }catch(err){
    console.error("loadCollegesGuide error:", err);
    state.collegesGuide = state.collegesGuide||[];
  }
}
// دليل المنح: برامج منح دراسية دائمة (تركية، مجرية، سويسرية، إلخ) بمعلومات
// مرجعية، مش فرص مؤقتة بديدلاين زي تبويب "الفرص المقترحة".
async function loadScholarshipsGuide(){
  try{
    const snap = await getDocs(collection(db,"scholarshipsGuide"));
    state.scholarshipsGuide = snap.docs.map(d=>({id:d.id, ...d.data()}));
  }catch(err){
    console.error("loadScholarshipsGuide error:", err);
    state.scholarshipsGuide = state.scholarshipsGuide||[];
  }
}
// ============ دليل الكليات: CRUD يدوي بالكامل من غير أي AI ============
async function saveCollegeGuideDraft(){
  ...
  try{
    if(d.id){
      await updateDoc(doc(db,"collegesGuide",d.id), payload);
      state.collegesGuide = state.collegesGuide.map(c=>c.id===d.id?{...c,...payload}:c);
    } else {
      const ref = await addDoc(collection(db,"collegesGuide"), payload);
      state.collegesGuide = [...state.collegesGuide, {id:ref.id, ...payload}];
    }
    state.adminCollegeGuideDraft = null;
  }catch(err){
    console.error("saveCollegeGuideDraft error:", err);
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء الحفظ، حاول تاني.");
    setTimeout(()=>{state.toastMsg=null; render();}, 2500);
  }
  render();
}
async function deleteCollegeGuideEntry(id){
  if(!window.confirm("متأكد إنك عايز تمسح الكلية دي من الدليل؟")) return;
  try{
    await deleteDoc(doc(db,"collegesGuide",id));
    state.collegesGuide = state.collegesGuide.filter(c=>c.id!==id);
    if(state.openCollegeGuideId===id) state.openCollegeGuideId = null;
  }catch(err){ console.error("deleteCollegeGuideEntry error:", err); state.toastMsg = friendlyErr(err, "حصل خطأ أثناء الحذف من دليل الكليات."); setTimeout(()=>{state.toastMsg="";render();},4000); }
  render();
}
// ============ دليل المنح: CRUD يدوي بالكامل من غير أي AI ============
async function saveScholarshipGuideDraft(){
  const d = state.adminScholarshipGuideDraft || {};
  const payload = {
    name: (document.getElementById("sg-d-name")||{}).value?.trim() || "",
    country: (document.getElementById("sg-d-country")||{}).value?.trim() || "",
    level: (document.getElementById("sg-d-level")||{}).value || "",
    fieldsCovered: (document.getElementById("sg-d-fields")||{}).value?.trim() || "",
    fundingDetails: (document.getElementById("sg-d-funding")||{}).value?.trim() || "",
    eligibility: (document.getElementById("sg-d-eligibility")||{}).value?.trim() || "",
    whyChooseIt: (document.getElementById("sg-d-why")||{}).value?.trim() || "",
    applicationNotes: (document.getElementById("sg-d-application")||{}).value?.trim() || "",
    deadlineNote: (document.getElementById("sg-d-deadline")||{}).value?.trim() || "",
    website: (document.getElementById("sg-d-website")||{}).value?.trim() || "",
    updatedAt: Date.now(),
  };
  if(!payload.name){
    state.toastMsg = "لازم تكتب اسم المنحة"; render();
    setTimeout(()=>{state.toastMsg=null; render();}, 2200);
    return;
  }
  try{
    if(d.id){
      await updateDoc(doc(db,"scholarshipsGuide",d.id), payload);
      state.scholarshipsGuide = state.scholarshipsGuide.map(s=>s.id===d.id?{...s,...payload}:s);
    } else {
      const ref = await addDoc(collection(db,"scholarshipsGuide"), payload);
      state.scholarshipsGuide = [...state.scholarshipsGuide, {id:ref.id, ...payload}];
    }
    state.adminScholarshipGuideDraft = null;
  }catch(err){
    console.error("saveScholarshipGuideDraft error:", err);
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء الحفظ، حاول تاني.");
    setTimeout(()=>{state.toastMsg=null; render();}, 2500);
  }
  render();
}
async function deleteScholarshipGuideEntry(id){
  if(!window.confirm("متأكد إنك عايز تمسح المنحة دي من الدليل؟")) return;
  try{
    await deleteDoc(doc(db,"scholarshipsGuide",id));
    state.scholarshipsGuide = state.scholarshipsGuide.filter(s=>s.id!==id);
    if(state.openScholarshipGuideId===id) state.openScholarshipGuideId = null;
  }catch(err){ console.error("deleteScholarshipGuideEntry error:", err); state.toastMsg = friendlyErr(err, "حصل خطأ أثناء الحذف من دليل المنح."); setTimeout(()=>{state.toastMsg="";render();},4000); }
  render();
}
...
async function deleteSuccessStory(id){
  ...
  catch(err){ console.error("deleteSuccessStory error:", err); state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حذف قصة النجاح."); render(); }
}
async function toggleStoryFeatured(id){
  ...
  catch(err){ console.error("toggleStoryFeatured error:", err); state.toastMsg = friendlyErr(err, "حصل خطأ أثناء التحديث."); render(); }
}
async function approvePending(id){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء الموافقة على الفرصة، جرب تاني.");
    setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
  }
  render();
}
async function rejectPending(id){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء رفض الفرصة، جرب تاني.");
    setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
  }
  render();
}
...
async function deleteOpportunity(id){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حذف الفرصة، جرب تاني.");
  }
  render();
  setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
}
...
async function updateOpportunity(form){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حفظ التعديلات، جرب تاني.");
  }
...
async function toggleResourceFeatured(id){
  ...
  try{
    ...
    await loadCollegesGuide();
    await loadScholarshipsGuide();
...
async function toggleResourceHidden(id){
  if(!isAdmin()) return;
  const r = state.resources.find(x=>x.id===id);
  if(!r) return;
  const hidden = !r.hidden;
  try{
    await updateDoc(doc(db,"resources", id), {hidden});
    await loadResources();
    state.toastMsg = hidden ? "تم إخفاء المحتوى — مش هيظهر للمستخدمين لحد ما تنشره تاني." : "تم النشر ✓ المحتوى بقى ظاهر للجميع.";
  }catch(err){
    console.error("toggleResourceHidden error:", err);
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء تحديث حالة النشر.");
  }
  render();
  setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
}
...
async function deleteResource(id){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حذف المصدر، جرب تاني.");
  }
  render();
  setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
}
...
async function toggleResourceComplete(resId){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حفظ التقدم، جرب تاني.");
    render();
    setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
  }
}
...
async function submitQuizAnswers(){
  ...
  try{
    ...
    state.toastMsg = friendlyErr(err, "حصل خطأ أثناء حفظ نتيجة الاختبار، النتيجة ظهرت بس ممكن متتحفظش.");
    render();
    setTimeout(()=>{ state.toastMsg=""; render(); }, 4000);
  }
}
...
async function generateCertificateText(courseName, userName){
  ...
  const result = await callAiProxy("", { contents:[{role:"user", parts:[{text:prompt}]}] }, {timeoutMs:30000, retries:0});
...
// تحسين محلي بقواعد ثابتة (بدون AI خالص) — بيتستخدم كـfallback حقيقي لما
// الـAI يفشل، بدل ما نرجع النص الخام زي ما هو بصمت. مش بديل كامل لجودة
// الـAI، بس تحسين ملموس وفوري ومجاني (قسم 4 في وثيقة Sprint 2).
const CV_WEAK_TO_STRONG_VERBS = [
  [/\bكنت مسؤول عن\b/g, "قدت"],
  [/\bكنت بساعد في\b/g, "ساهمت في"],
  [/\bساعدت في\b/g, "ساهمت في"],
  [/\bعملت على\b/g, "طوّرت"],
  [/\bاشتغلت على\b/g, "نفّذت"],
  [/\bكان شغلي\b/g, "قدت"],
];
function locallyEnhanceCvText(text){
  if(!text || !text.trim()) return text;
  let out = text.trim();
  // إزالة ضمائر شخصية زيادة شائعة في بداية الجمل.
  out = out.replace(/^(أنا|انا)\s+/g, "").replace(/\n(أنا|انا)\s+/g, "\n");
  // استبدال أفعال ضعيفة شائعة بأفعال أقوى.
  for(const [pattern, replacement] of CV_WEAK_TO_STRONG_VERBS){
    out = out.replace(pattern, replacement);
  }
  // إزالة مسافات/أسطر زيادة.
  out = out.replace(/[ \t]+/g, " ").replace(/\n{3,}/g, "\n\n").trim();
  return out;
}
// رسالة ودّية للمستخدم بعد محاولة تحسين الـCV — منورّيش أي تفصيل تقني خام
// (كوتا/تايم أوت/500...) خالص، بس نوضح المصدر (AI ولا تحسين محلي بسيط).
function cvPolishToastMessage(polishResult){
  if(polishResult.polishedBy==="ai") return "تم تحميل الـCV بعد تحسين الصياغة بالذكاء الاصطناعي ✓";
  if(polishResult.polishedBy==="local") return "تم تحميل الـCV بعد تحسين بسيط للصياغة ✓";
  return "تم تحميل الـCV بصياغتك الأصلية.";
}
...
رد بصيغة JSON بس من غير أي كلام زيادة قبله أو بعده ومن غير أي علامات markdown، بالشكل ده بالظبط: {"goal":"النص المحسّن للهدف","achievements":"سطر1\\nسطر2\\nسطر3"}`;
    ...
    const result = await callAiProxy("", { contents:[{role:"user", parts:[{text: prompt}]}] }, {timeoutMs:45000, retries:0});
    if(!result.ok){
    ...
    if(!jsonMatch){
      ...
      return {profile:{...p, goal:locallyEnhanceCvText(p.goal), achievements:locallyEnhanceCvText(p.achievements)}, polished:true, polishedBy:"local", reason:"رد الـAI مكنش بصيغة متوقعة"};
    }
    ...
    return {profile: {...p, goal:newGoal, achievements:newAch}, polished: changed, polishedBy: changed?"ai":"none", reason: changed?"":"رد الـAI مطابق للنص الأصلي"};
  }catch(err){
    console.error("polishCvContent error:", err);
    // فشل غير متوقع — بدل ما نرجع النص الخام زي ما هو، بنطبّق تحسين محلي
    // بقواعد ثابتة (locallyEnhanceCvText) عشان المستخدم يخرج بحاجة أحسن
    // شوية على الأقل، مع توضيح واضح إن ده مش من الـAI.
    return {profile:{...p, goal:locallyEnhanceCvText(p.goal), achievements:locallyEnhanceCvText(p.achievements)}, polished:true, polishedBy:"local", reason:"حصل خطأ غير متوقع أثناء تحسين الصياغة بالـAI"};
  }
}
async function generateCV(){
  ...
  try{
    ...
    state.toastMsg = cvPolishToastMessage(polishResult);
...
async function submitAdminOpp(form){
  ...
  try{
    if(isAdmin()){
      await addDoc(collection(db,"opportunities"), {...opp, reviewed:true, createdAt: Date.now()});
      await loadOpportunities();
...
// ============ حفظ موضع التمرير عبر عمليات الـre-render ============
// السبب الحقيقي وراء بندين مختلفين ظاهريًا في التعديلات المطلوبة (رجوع
// القوائم لأول الصفحة + رجوع الاختبار للسؤال الأول) هو نفسه: كل الشاشات هنا
// بترندر بـ`app.innerHTML = ...` بالكامل، وده بيصفّر أي scroll (سواء تمرير
// الصفحة أو تمرير جوه أي مودال زي مودال الاختبار) كل مرة، لإن العناصر
// القديمة بتتشال ومفيش أي مرجع يرجّع المتصفح لمكانه القديم.
// الحل: بنحسب "مفتاح الشاشة الحالية" من الحقول اللي فعليًا بتغيّر الشاشة
// المعروضة. لو المفتاح اتغيّر (تنقّل حقيقي لشاشة جديدة) بنحفظ مكان التمرير
// القديم في الذاكرة عشان نرجّعله لو المستخدم رجع تاني، ولو الشاشة جديدة
// تمامًا (زي فتح تفصيلة لأول مرة) بنبدأ من فوق زي المتوقع بالظبط.
// لو المفتاح ثابت (مجرد تحديث داخل نفس الشاشة، زي اختيار إجابة في اختبار
// أو تغيير فلتر) بنرجّع بالظبط لنفس مكان التمرير القديم (صفحة أو مودال)
// بعد الرندر — وده اللي بيمنع "الاختبار يرجع للسؤال الأول" لأن الاختبار
// نفسه مبيعملش أي reset، بس الـscroll جوه المودال كان بيترجع صفر.
const _scrollMemory = {};
let _lastViewKey;
function _computeViewKey(){
  return [
    state.screen, state.activeTab,
    state.openOppId||"", state.openUniId||"",
    state.openCollegeGuideId||"", state.openScholarshipGuideId||"",
    state.activeQuizId||""
  ].join("|");
}
function render(){
  const newKey = _computeViewKey();
  const sameView = _lastViewKey!==undefined && _lastViewKey===newKey;
  const beforeScrollY = window.scrollY;
  const modalNodeBefore = document.querySelector(".modal-sheet, .modal-card");
  const beforeModalScroll = modalNodeBefore ? modalNodeBefore.scrollTop : null;
  if(!sameView && _lastViewKey!==undefined){
    _scrollMemory[_lastViewKey] = beforeScrollY;
  }
  // 🩹 إصلاح باگ التبويبات: أي شريط تبويبات بيلف أفقيًا (زي فلاتر الفئات أو
  // تبويبات مركز التعلم) كان بيرجع لأول الشريط في كل render() حتى لو نفس
  // الشاشة بالظبط — لإن كل الـHTML بيتبني من جديد كامل. بنحفظ موضع اللف
  // الأفقي لكل شريط قبل إعادة البناء، ونرجّعه بعدها لو لسه نفس الشاشة.
  const tabBarsBefore = Array.from(document.querySelectorAll(".tabs, .cat-chip-row"));
  const tabScrollPositions = sameView ? tabBarsBefore.map(el=>el.scrollLeft) : null;
  ...
  requestAnimationFrame(()=>{
    if(sameView){
      ...
      const modalNodeAfter = document.querySelector(".modal-sheet, .modal-card");
      if(modalNodeAfter && beforeModalScroll!=null) modalNodeAfter.scrollTop = beforeModalScroll;
      if(tabScrollPositions){
        const tabBarsAfter = document.querySelectorAll(".tabs, .cat-chip-row");
        tabBarsAfter.forEach((el,i)=>{ if(tabScrollPositions[i]!=null) el.scrollLeft = tabScrollPositions[i]; });
      }
    } else {
      window.scrollTo(0, _scrollMemory[newKey] || 0);
    }
  });
}
...
const HELP_TEXT = {
  ...
  resources:{title:"مركز التعلم", body:"محتوى تعليمي مجاني (كورسات ونصائح)، بالإضافة لقسمي الاختبارات الشخصية وأدلة الجامعات والكليات والمنح والموارد المجانية — كلهم دلوقتي هنا في مكان واحد. اختار القسم من الأزرار فوق."},
...
// بيولّد سطر تمهيدي وتعديلات على الخطوات بناءً على بيانات ملف المستخدم
// الحقيقية (مش AI خالص — قواعد ثابتة بس): المرحلة الدراسية، مستوى
// الإنجليزي، وهل عنده خبرة سابقة مذكورة في "الإنجازات" بنفس المهارة.
// التركيبة (مرحلة × خبرة سابقة × مستوى لغة × كل مهارة) بتنتج سيناريوهات
// كتير مختلفة فعليًا من غير أي استدعاء AI.
function personalizeRoadmap(skill, baseSteps, profile){
  const p = profile || {};
  const hasPriorExperience = (p.achievements||"").toLowerCase().includes(skill.toLowerCase());
  const isSchoolStage = p.stage==="middle" || p.stage==="high";
  const weakEnglish = p.englishLevel==="beginner" || p.englishLevel==="ضعيف" || p.englishLevel==="مبتدئ";

  let steps = [...baseSteps];
  let intro = "";

  if(hasPriorExperience){
    intro = `شايف من إنجازاتك إن عندك خبرة سابقة في ${escapeHtml(skill)} — ممكن تتخطى الخطوة الأولى وتركّز على التطبيق العملي والمشاريع مباشرة.`;
    steps = steps.slice(1); // نشيل خطوة "الأساسيات" لإنه غالبًا عدّاها بالفعل
  } else if(isSchoolStage){
    intro = `إنت لسه في مرحلة دراسية مبكرة، فخد وقتك في الأساسيات من غير ضغط — مفيش داعي تستعجل الخطوات الجاية.`;
  } else {
    intro = `دي خطة مقترحة لتعلم ${escapeHtml(skill)} خطوة بخطوة.`;
  }

  if(weakEnglish){
    steps = [...steps, "دوّر على نسخ عربي أو مترجمة من المصادر لو حسّيت إن حاجز اللغة بيبطّئك — الفهم أهم من مصدر اللغة."];
  }
...
function renderRoadmapBlock(){
  ...
  return wants.map(skill=>{
    const baseSteps = ROADMAP_DB[skill] || ["دوّر على مصدر تعليمي موثوق (كورس أو دليل) عن \""+skill+"\" في مركز التعلم","ابدأ بالأساسيات وخصص وقت ثابت أسبوعيًا للمذاكرة","اتمرن عمليًا بمشروع أو تطبيق بسيط لما تحس مستعد","اطلب فيدباك من حد أعرف منك في المجال ده","ضيف اللي اتعلمته في الـCV والبورتفوليو بتاعك"];
    const {intro, steps} = personalizeRoadmap(skill, baseSteps, p);
    return `
    <div class="card" style="padding:16px 18px;margin-bottom:10px;">
      <div style="font-weight:800;font-size:14.5px;margin-bottom:6px;">🗺️ روودماب: ${escapeHtml(skill)}</div>
      ${intro?`<div style="font-size:12.5px;color:#5C7278;margin-bottom:8px;">${intro}</div>`:""}
...
function renderAdminUniForm(){
  ...
  <div class="card" style="padding:18px;margin:14px 0;">
    ...
    <div class="field"><label>نبذة مختصرة (Overview)</label><textarea id="uni-d-brief" rows="3">${escapeHtml(d.briefInfo||"")}</textarea></div>
    <div class="field"><label>الكليات</label><textarea id="uni-d-faculties" rows="2" placeholder="كل كلية في سطر">${escapeHtml(d.faculties||"")}</textarea></div>
...
{"type":"اختار قيمة واحدة من: gov أو national أو private أو tech أو intl","city":"المحافظة (Location)","briefInfo":"نبذة عامة (Overview) من 2-3 أسطر بالعربي","faculties":"أهم الكليات، كل كلية في سطر","admissionNotes":"شروط القبول باختصار","tuitionNotes":"المصروفات التقريبية لو معروفة، أو نص فاضي","features":"أهم المميزات","website":"الموقع الرسمي الحقيقي"}
لو مش متأكد من معلومة معينة أو مش لاقيها في نتائج البحث، سيبها نص فاضي "" بدل ما تختلقها أو تخمّنها.`;
  // مكالمة Gemini واحدة بالظبط، من غير أي إعادة محاولة (retries:0 هو الافتراضي دلوقتي).
  const result = await callAiProxy("", {contents:[{role:"user",parts:[{text:prompt}]}], tools:[{google_search:{}}]}, {timeoutMs:45000, retries:0});
...
// ============================================================
// دليل الكليات — كتالوج مستقل لكل الكليات/التخصصات الموجودة في مصر، مش
// مربوط بجامعة بعينها. بيتاح فيه معلومات كاملة (أقسام، فرص عمل، رواتب،
// مهارات، شخصية مناسبة، شروط قبول، منح، تخصصات قريبة). كل المحتوى هنا
// بيتضاف يدويًا من الإدارة، من غير أي AI خالص.
// ============================================================
function renderCollegesGuide(){
  if(state.openCollegeGuideId){
    const c = state.collegesGuide.find(x=>x.id===state.openCollegeGuideId);
    if(!c) state.openCollegeGuideId = null;
    else return renderCollegeGuideDetail(c);
  }
  ...
  <div class="card opp-card" data-action="open-college-guide" data-id="${c.id}" style="cursor:pointer;display:flex;gap:12px;align-items:flex-start;">
    ...
    <div style="flex:1;">
      <div class="opp-title">${escapeHtml(c.name)}</div>
      ${c.universities?`<div class="opp-org">${escapeHtml(c.universities)}</div>`:""}
      ${c.description?`<div class="desc-text" style="margin-top:6px;">${escapeHtml(c.description.slice(0,120))}${c.description.length>120?"...":""}</div>`:""}
    </div>
  </div>`).join("")}
  `;
}
...
// ============================================================
// دليل المنح — كتالوج مستقل لبرامج المنح الدائمة (بعد الثانوية أو بعد
// الجامعة)، بمعلومات مرجعية بتشرح ليه المنحة دي مناسبة، مش فرصة مؤقتة
// بديدلاين. كل المحتوى هنا بيتضاف يدويًا من الإدارة، من غير أي AI خالص.
// ============================================================
const SCHOLARSHIP_LEVEL_LABEL = {afterHighSchool:"بعد الثانوية", university:"بعد/أثناء الجامعة", postgraduate:"دراسات عليا", any:"كل المراحل"};
function renderScholarshipsGuide(){
  if(state.openScholarshipGuideId){
    const s = state.scholarshipsGuide.find(x=>x.id===state.openScholarshipGuideId);
    if(!s) state.openScholarshipGuideId = null;
    else return renderScholarshipGuideDetail(s);
  }
  const q = normalizeAr(state.scholarshipGuideSearchQuery||"");
  let list = state.scholarshipsGuide.slice();
  if(q){
    list = list.filter(s=>normalizeAr([s.name,s.country,s.fieldsCovered].filter(Boolean).join(" ")).includes(q));
  }
  if(state.scholarshipGuideFilterLevel) list = list.filter(s=>s.level===state.scholarshipGuideFilterLevel);
  return `
  <div class="note-box" style="margin-bottom:12px;">
    🌍 دليل المنح — منح دراسية دائمة من دول مختلفة (تركيا، المجر، سويسرا، وغيرها) بعد الثانوية أو أثناء/بعد الجامعة، بمعلومات كاملة عن التمويل والأهلية وليه تختارها.
  </div>
  <form id="sg-search-form" style="display:flex;gap:8px;margin-bottom:10px;">
    <input id="sg-search-input" type="text" value="${escapeHtml(state.scholarshipGuideSearchQuery)}" placeholder="دور باسم المنحة أو الدولة أو التخصص..." style="flex:1;padding:11px 14px;border:1.5px solid var(--border);border-radius:10px;font-size:14px;">
    <button type="submit" class="btn btn-ghost" style="padding:11px 18px;">بحث</button>
  </form>
  <div class="cat-chip-row">
    <button type="button" class="pill ${!state.scholarshipGuideFilterLevel?"selected":""}" data-action="sg-filter-level" data-level="">كل المراحل</button>
    ${Object.entries(SCHOLARSHIP_LEVEL_LABEL).map(([k,v])=>`<button type="button" class="pill ${state.scholarshipGuideFilterLevel===k?"selected":""}" data-action="sg-filter-level" data-level="${k}">${v}</button>`).join("")}
  </div>
  ${isAdmin()?`<button type="button" class="btn btn-ghost btn-block" style="margin:12px 0;" data-action="admin-new-scholarship-guide">+ أضف منحة (إدارة)</button>`:""}
  ${isAdmin()&&state.adminScholarshipGuideDraft?renderAdminScholarshipGuideForm():""}
  ${list.length===0?`<div class="empty-state"><h3>لسه مفيش منح مضافة${q?" بالبحث ده":""}</h3><p>${isAdmin()?"دوس \"أضف منحة\" فوق عشان تبدأ تبني الدليل.":"الدليل لسه بيتبنى — تابعنا هيتزوّد قريب."}</p></div>`:
  list.map(s=>`
  <div class="card opp-card" data-action="open-scholarship-guide" data-id="${s.id}" style="cursor:pointer;">
    <span class="opp-cat">${SCHOLARSHIP_LEVEL_LABEL[s.level]||""}</span>
    <div class="opp-title" style="margin-top:6px;">${escapeHtml(s.name)}</div>
    <div class="opp-org">${escapeHtml(s.country||"")}</div>
    ${s.fundingDetails?`<div class="desc-text" style="margin-top:6px;">${escapeHtml(s.fundingDetails.slice(0,120))}${s.fundingDetails.length>120?"...":""}</div>`:""}
  </div>`).join("")}
  `;
}
function renderScholarshipGuideDetail(s){
  return `
  <button type="button" class="link-btn" data-action="close-scholarship-guide" style="margin-bottom:10px;">→ رجوع لدليل المنح</button>
  <div class="card" style="padding:20px;margin-bottom:14px;">
    <span class="opp-cat">${SCHOLARSHIP_LEVEL_LABEL[s.level]||""}</span>
    <h2 style="font-size:19px;font-weight:800;margin:8px 0 4px;">${escapeHtml(s.name)}</h2>
    ${s.country?`<div style="color:var(--ink-muted);font-size:13px;margin-bottom:10px;">🌍 ${escapeHtml(s.country)}</div>`:""}
    ${s.whyChooseIt?`<div class="note-box" style="margin-bottom:10px;"><b>ليه تختارها؟</b> ${escapeHtml(s.whyChooseIt)}</div>`:""}
    ${s.fieldsCovered?`<div style="font-size:13px;margin-bottom:6px;"><b>📚 التخصصات المتاحة:</b> ${escapeHtml(s.fieldsCovered)}</div>`:""}
    ${s.fundingDetails?`<div style="font-size:13px;margin-bottom:6px;"><b>💰 تفاصيل التمويل:</b> ${escapeHtml(s.fundingDetails)}</div>`:""}
    ${s.eligibility?`<div style="font-size:13px;margin-bottom:6px;"><b>✅ شروط الأهلية:</b> ${escapeHtml(s.eligibility)}</div>`:""}
    ${s.applicationNotes?`<div style="font-size:13px;margin-bottom:6px;"><b>📝 خطوات التقديم:</b> ${escapeHtml(s.applicationNotes)}</div>`:""}
    ${s.deadlineNote?`<div style="font-size:13px;margin-bottom:6px;"><b>📅 مواعيد التقديم (تقريبية):</b> ${escapeHtml(s.deadlineNote)}</div>`:""}
    ${s.website?`<a href="${escapeHtml(s.website)}" target="_blank" rel="noopener" class="link-btn">الموقع الرسمي ↗</a>`:""}
    ${s.updatedAt?`<div style="font-size:11px;color:var(--ink-muted);margin-top:8px;">آخر تحديث: ${new Date(s.updatedAt).toLocaleDateString("ar-EG")}</div>`:""}
    ${isAdmin()?`<div style="display:flex;gap:8px;margin-top:12px;"><button class="pill" data-action="admin-edit-scholarship-guide" data-id="${s.id}">✏️ تعديل</button><button class="pill" data-action="admin-delete-scholarship-guide" data-id="${s.id}" style="color:#B4232C;">🗑️ حذف</button></div>`:""}
  </div>`;
}
function renderAdminScholarshipGuideForm(){
  const d = state.adminScholarshipGuideDraft || {};
  return `
  <div class="card" style="padding:18px;margin-bottom:14px;">
    <div style="font-weight:800;margin-bottom:10px;">${d.id?"تعديل منحة":"منحة جديدة"}</div>
    <div class="field"><label>اسم المنحة</label><input id="sg-d-name" type="text" value="${escapeHtml(d.name||"")}" placeholder="مثال: المنحة التركية Türkiye Bursları"></div>
    <div class="field"><label>الدولة/الجهة المانحة</label><input id="sg-d-country" type="text" value="${escapeHtml(d.country||"")}"></div>
    <div class="field"><label>المرحلة</label>
      <select id="sg-d-level">
        <option value="">اختار...</option>
        ${Object.entries(SCHOLARSHIP_LEVEL_LABEL).map(([k,v])=>`<option value="${k}" ${d.level===k?"selected":""}>${v}</option>`).join("")}
      </select>
    </div>
    <div class="field"><label>ليه تختارها؟</label><textarea id="sg-d-why" rows="3">${escapeHtml(d.whyChooseIt||"")}</textarea></div>
    <div class="field"><label>التخصصات المتاحة</label><textarea id="sg-d-fields" rows="2">${escapeHtml(d.fieldsCovered||"")}</textarea></div>
    <div class="field"><label>تفاصيل التمويل (كامل/جزئي، بيغطي إيه)</label><textarea id="sg-d-funding" rows="2">${escapeHtml(d.fundingDetails||"")}</textarea></div>
    <div class="field"><label>شروط الأهلية</label><textarea id="sg-d-eligibility" rows="2">${escapeHtml(d.eligibility||"")}</textarea></div>
    <div class="field"><label>خطوات/ملاحظات التقديم</label><textarea id="sg-d-application" rows="2">${escapeHtml(d.applicationNotes||"")}</textarea></div>
    <div class="field"><label>مواعيد التقديم (تقريبية)</label><input id="sg-d-deadline" type="text" value="${escapeHtml(d.deadlineNote||"")}" placeholder="مثال: يفتح التقديم عادة من يناير لفبراير كل سنة"></div>
    <div class="field"><label>الموقع الرسمي</label><input id="sg-d-website" type="text" value="${escapeHtml(d.website||"")}"></div>
    <div style="display:flex;gap:8px;">
      <button type="button" class="btn btn-gold" style="flex:1;" data-action="save-scholarship-guide-draft">حفظ</button>
      <button type="button" class="btn btn-ghost" style="flex:1;" data-action="cancel-scholarship-guide-draft">إلغاء</button>
    </div>
  </div>`;
}
...
function renderGuidesTab(){
  ...
  const subTabs = `
    <div class="tabs" style="margin-bottom:14px;">
      <button class="tab ${sub==="universities"?"active":""}" data-action="guides-subtab" data-subtab="universities">🎓 دليل الجامعات والكليات</button>
  ...
  let body = "";
  if(sub==="universities"){
    const inner = ["colleges","universities","scholarships"].includes(state.uniGuideInnerTab) ? state.uniGuideInnerTab : "colleges";
    const innerTabs = `
    <div class="tabs" style="margin-bottom:14px;">
      <button class="tab ${inner==="colleges"?"active":""}" data-action="uni-guide-inner-tab" data-inner="colleges">🏛️ دليل الكليات</button>
      <button class="tab ${inner==="universities"?"active":""}" data-action="uni-guide-inner-tab" data-inner="universities">🎓 دليل الجامعات</button>
      <button class="tab ${inner==="scholarships"?"active":""}" data-action="uni-guide-inner-tab" data-inner="scholarships">🌍 دليل المنح</button>
    </div>`;
    let innerBody = "";
    if(inner==="colleges") innerBody = renderCollegesGuide();
    else if(inner==="scholarships") innerBody = renderScholarshipsGuide();
    else innerBody = renderUniversitiesGuide();
    body = innerTabs + innerBody;
  } else if(sub==="courses"){
...
function renderAdminTab(){
  ...
  <div class="card" style="padding:20px;margin-top:0;">
    ...
    <div class="note-box" style="margin-bottom:12px;">
      نظام جمع الفرص التلقائي (GitHub Actions) بيشتغل مرة كل يوم لوحده. لو عايز تشغّله دلوقتي من غير ما تستنى:
      <div style="margin-top:8px;"><button type="button" class="btn btn-ghost" data-action="trigger-aggregator" ${state.aggregatorTriggering?"disabled":""}>${state.aggregatorTriggering?"بيشغّل...":"🔄 تحديث الفرص الآن"}</button></div>
      ${state.aggregatorMsg?`<div style="margin-top:8px;font-size:13px;">${state.aggregatorMsg}</div>`:""}
      ${renderAggregatorLastRun()}
    </div>
...
async function postAnnouncement(){
  ...
  try{
    ...
    let msg = "حصل خطأ أثناء نشر الإعلان.";
    if(err && err.code === "permission-denied"){
      msg = "مفيش صلاحية لنشر الإعلان — لازم تتأكد من Firestore Security Rules (قاعدة إذن الكتابة على announcements لازم تسمح للأدمن).";
    } else if(err && err.code){
      msg = "خطأ (" + err.code + "): " + (err.message || "غير معروف");
    }
    state.toastMsg = msg; render();
    setTimeout(()=>{state.toastMsg="";render();},6000);
  }
}
async function deleteAnnouncement(id){
  ...
  try{ await deleteDoc(doc(db,"announcements",id)); await loadAnnouncements(); render(); }
    ...
    state.toastMsg = err && err.code === "permission-denied"
      ? "مفيش صلاحية لحذف الإعلان — راجع Firestore Rules."
      : "حصل خطأ أثناء حذف الإعلان.";
    render();
    setTimeout(()=>{state.toastMsg="";render();},6000);
  }
}
...
// ============================================================
// السجل المركزي لكل "أقسام المحتوى" في مركز التعلم — لوحة الإدارة الأصلية
// (نفس الفورم، نفس الـcollection "resources") بقت CMS عام قابل للتوسع:
// أي قسم محتوى جديد مستقبلًا (منصات كورسات، أدوات تعلم، كتب، مقالات...)
// بيتضاف بسطر واحد هنا بس، من غير أي تعديل في أي كود تاني.
// ============================================================
const RESOURCE_SECTIONS = [
  {id:"tips", icon:"💡", label:"نصائح صغيرة"},
  {id:"courses", icon:"🎓", label:"الكورسات"},
  {id:"practical", icon:"🛠️", label:"نصائح عملية"},
  {id:"course-platforms", icon:"🎓", label:"منصات كورسات"},
  {id:"learning-tools", icon:"🧰", label:"أدوات تعلم"},
  {id:"learning-sites", icon:"🌐", label:"مواقع تعلم"},
  {id:"books", icon:"📚", label:"كتب"},
  {id:"articles", icon:"📰", label:"مقالات"},
  {id:"religious-advice", icon:"🕌", label:"نصيحة دينية"},
  {id:"religious-quran", icon:"📖", label:"آية قرآنية تريح بها قلبك"},
];
function resourceSectionMeta(id){
  return RESOURCE_SECTIONS.find(s=>s.id===id) || {id, icon:"📄", label:id};
}
// محتوى "مُخفي" (r.hidden) بيفضل ظاهر للأدمن بس (عشان يقدر ينشره تاني)،
// ومختفي تمامًا عن باقي المستخدمين — جزء من نظام النشر/إلغاء النشر.
function isResourceVisible(r){
  return isAdmin() || !r.hidden;
}
    ...
    <div class="form-card">
      ...
      <div class="field-row">
        ...
        <div class="field"><label>هيتحط فين؟</label><select name="section">
          ${RESOURCE_SECTIONS.filter(s=>s.id!=="courses").map(s=>`<option value="${s.id}">${s.icon} ${s.label}</option>`).join("")}
        </select></div>
      </div>
...
function renderAiSearchPanel(){
  ...
  const quotaLine = state.aiQuotaLoading ? "بيجيب حد الاستخدام..."
    ...
    : "";
  return `
  <div class="note-box" style="margin-bottom:14px;">الـAI بيدور بجوجل الفعلي (بحث سريع) ويجيب لحد 5 فرص بمكالمة Gemini واحدة بس، بس برضو راجع كل فرصة وتأكد من التفاصيل قبل الموافقة — خصوصًا آخر موعد.${quotaLine?`<div style="margin-top:6px;font-weight:700;">${quotaLine}</div>`:""}</div>
...
function renderResourceCard(r){
  ...
  <div class="card" style="padding:16px;margin-bottom:12px;${r.featured?"border:1.5px solid #E8A33D;":""}">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center;">
      ...
      ${isAdmin()?`<button class="pill" data-action="toggle-resource-hidden" data-id="${r.id}">${r.hidden?"👁️ نشر":"🙈 إخفاء"}</button>`:""}
...
function renderLearningCenterTab(){
  const section = ["content","assessments","guides","religious"].includes(state.learningCenterSection) ? state.learningCenterSection : "content";
  const sectionTabs = `
    <div class="tabs" style="margin-bottom:14px;">
      ...
      <button class="tab ${section==="religious"?"active":""}" data-action="learning-center-section" data-section="religious">🕌 ديني</button>
    </div>`;
  if(section==="assessments") return sectionTabs + renderAssessmentsTab();
  if(section==="guides") return sectionTabs + renderGuidesTab();
  if(section==="religious") return sectionTabs + renderReligiousTab();
  return sectionTabs + renderResourcesTab();
}
function renderReligiousTab(){
  const introNote = `<div class="note-box" style="margin-bottom:14px;">🕌 محتوى ديني مبسّط يعينك في رحلتك — نصائح قصيرة وآيات قرآنية تريّح بيها قلبك.</div>`;
  const sub = ["advice","quran"].includes(state.religiousSubTab) ? state.religiousSubTab : "advice";
  const subTabs = `
    <div class="tabs" style="margin-bottom:14px;">
      <button class="tab ${sub==="advice"?"active":""}" data-action="religious-subtab" data-subtab="advice">🕌 نصائح دينية</button>
      <button class="tab ${sub==="quran"?"active":""}" data-action="religious-subtab" data-subtab="quran">📖 آيات قرآنية تريح بها قلبك</button>
    </div>`;
  const wanted = sub==="advice" ? "religious-advice" : "religious-quran";
  const byFeaturedFirst = (list)=>[...list].sort((a,b)=>(b.featured?1:0)-(a.featured?1:0));
  const list = byFeaturedFirst(state.resources.filter(r=>r.section===wanted && isResourceVisible(r)));
  const emptyMsg = sub==="advice"
    ? `<div class="empty-state"><h3>لسه مفيش نصائح دينية</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`
    : `<div class="empty-state"><h3>لسه مفيش آيات مضافة</h3><p>هتظهر هنا أول ما الأدمن يضيفها.</p></div>`;
...
function renderResourcesTab(){
  ...
  const byFeaturedFirst = (list)=>[...list].sort((a,b)=>(b.featured?1:0)-(a.featured?1:0));
  const bySection = {
    tips: byFeaturedFirst(state.resources.filter(r=>resourceSection(r)==="tips" && isResourceVisible(r))),
    courses: byFeaturedFirst(state.resources.filter(r=>resourceSection(r)==="courses" && isResourceVisible(r))),
    practical: byFeaturedFirst(state.resources.filter(r=>resourceSection(r)==="practical" && isResourceVisible(r)))
  };
...
// مفتاح Gemini مش موجود في كود العميل خالص — مخبّى بالكامل في السيرفر
// (Vercel Serverless Function جوه /api/chat)، والموقع بيكلم البروكسي ده بس.
// (Firebase apiKey اللي فوق في firebaseConfig شيء مختلف تمامًا — ده مفتاح
// عميل عادي مقصود إنه يبقى ظاهر حسب توثيق Firebase نفسه، والحماية الحقيقية
// بتيجي من Firestore Security Rules مش من إخفاء المفتاح ده.)
const GEMINI_PROXY_URL = "/api/chat";

// ============ نداء موحّد لكل استخدامات الذكاء الاصطناعي ============
// ميزات الـAI الوحيدة المتبقية في الموقع هي: البحث عن فرص (بحث سريع بس)،
// تحسين الـCV، توليد نص الشهادة، تصنيف الفرص بالجملة، ومساعد بيانات الجامعة —
// كل واحدة فيهم بتعمل مكالمة Gemini واحدة بالظبط لكل ضغطة زرار. الـretries
// بقى صفر افتراضيًا (retries=0) عن قصد: لو الطلب فشل (تايم أوت، شبكة، أو
// رفض من السيرفر) بنرجّع خطأ واضح للمستخدم على طول من غير أي إعادة محاولة
// تلقائية تستهلك كوتا إضافية — ده كان السبب الرئيسي في استهلاك الكوتا المجانية
// بسرعة قبل كده. مهلة (timeout) طويلة بما يكفي لكل نوع طلب بس من غير retry.
async function callAiProxy(path, body, {timeoutMs=60000, retries=0} = {}){
  ...
  for(let attempt=0; attempt<=retries; attempt++){
    ...
    try{
      // بنستخدم Firebase ID Token موقّع بدل ما نبعت uid/email كنص عادي في الـ
      // body — لو بعتنا uid/email كنص عادي، أي حد يقدر يدّعي إنه أدمن أو
      // يدّعي uid حد تاني ويسرق الكوتا بتاعه. التوكن ده السيرفر بيتحقق منه
      // فعليًا عن طريق Firebase Admin SDK (verifyIdToken) قبل ما يوثق فيه.
      const idToken = state.user ? await state.user.getIdToken().catch(()=>null) : null;
      ...
      try{ data = await res.clone().json(); }
        ...
        let rawText = "";
        try{ rawText = await res.text(); }catch(e2){ rawText = "(تعذّرت قراءة النص الخام كمان)"; }
        console.error(`[AI ${path||"/"}][ERROR] رد السيرفر مكنش JSON صحيح`, JSON.stringify({
          status: res.status, statusText: res.statusText, requestUrl: url, elapsedMs: elapsed, rawText
        }, null, 2));
      ...
      if(!res.ok){
        ...
        // Diagnostic logging كامل: status, الرد الكامل من غير تلخيص، الموديل
        // (لو موجود في الطلب نفسه)، ورابط الطلب (نقطة /api/chat على Vercel — مفيش أي
        // API key هنا خالص، الـkey موجود على السيرفر بس ومش بيوصل للمتصفح).
        console.error(`[AI ${path||"/"}][ERROR]`, JSON.stringify({
          status: res.status,
          statusText: res.statusText,
          requestUrl: url,
          requestedModel: body && body.model || "(افتراضي من السيرفر)",
          elapsedMs: elapsed,
...
const ASSISTANT_UNSURE_MARKER = "مش متأكد إني فاهم قصدك بالظبط";
async function sendChatMessage(text){
  ...
  const localReply = localAssistantReply(text.trim());
  if(!localReply.startsWith(ASSISTANT_UNSURE_MARKER)){
    // القواعد المحلية الموجودة (بيانات شخصية حقيقية من ملف المستخدم وفرص
    // المنصة) لقت رد واضح — نستخدمه على طول، صفر تكلفة وصفر تأخير شبكة.
    state.chatMessages.push({role:"assistant", text: localReply});
    state.chatBusy = false; render();
    return;
  }
  // 🆕 مفيش تطابق محلي واضح — نصعّد لسلسلة الـFallback على السيرفر
  // (Knowledge Base موسّعة، وبعدها Gemini لو المستخدم عنده كوتا). الكوتا
  // بتتخصم بس لو Gemini فعليًا اتنده ونجح.
  try{
    const idToken = state.user ? await state.user.getIdToken().catch(()=>null) : null;
    if(!idToken){
      state.chatMessages.push({role:"assistant", text: localReply});
    ...
    const res = await fetch(GEMINI_PROXY_URL, {
      ...
      body: JSON.stringify({action:"assistantChat", question:text.trim()})
    });
    const data = await res.json().catch(()=>null);
    // مفيش أي رسالة تقنية خام بتوصل هنا — السيرفر نفسه دايمًا بيرجّع رد
    // جاهز للعرض (rules/knowledge-base/ai/fallback) أو رسالة ودّية.
    state.chatMessages.push({role:"assistant", text: (data && data.answer) || localReply});
  }catch(err){
    console.error("sendChatMessage server fallback error:", err);
    state.chatMessages.push({role:"assistant", text: localReply});
  }
  state.chatBusy = false; render();
}
...
// بحث سريع بس (Fast Search) — موديل واحد ثابت، مكالمة Gemini واحدة بالظبط،
// وحد أقصى 5 فرص. كل خيارات العمق (عميق/عميق جدًا) والموديلات المتعددة
// وعدد النتائج القابل للتعديل اتشالوا خالص عشان نضمن استهلاك كوتا ثابت
// ومتوقّع في كل عملية بحث.
const FAST_SEARCH_MODEL = "gemini-3.5-flash";
const FAST_SEARCH_MAX_RESULTS = 5;
async function fetchAiOpportunities(topic){
  const model = FAST_SEARCH_MODEL;
  const resultCount = FAST_SEARCH_MAX_RESULTS;
...
شروط مهمة:
- افتح المصدر الأساسي وتأكد بسرعة إنه حقيقي.
...
لو مش متأكد إن الفرصة حقيقية، متضيفهاش. لو مفيش فرص كفاية رجّع [].`;
  try{
    // مكالمة Gemini واحدة بالظبط، من غير أي إعادة محاولة (retries:0) — لو فشلت
    // بترجع خطأ للمستخدم على طول بدل ما تعمل طلب تاني وتستهلك كوتا إضافية.
    ...
    // حد أمان إضافي هنا كمان (مش بس في البرومبت) — حتى لو Gemini رجّع أكتر من
    // المطلوب، منقصّه إحنا يدويًا قبل ما نرجّع النتيجة للمستخدم.
...
// بتستخرج ID فيديو اليوتيوب من أي شكل رابط (watch?v=, youtu.be/, shorts/,
// embed/, live/, youtube-nocookie.com، وv= في أي مكان في الـquery string
// مش شرط يكون أول باراميتر). بنستخدم URL API الحقيقي بدل regex هش واحد
// عشان نتعامل صح مع أي ترتيب باراميترات أو دومين فرعي (m.youtube.com).
function extractYoutubeId(url){
  ...
  try{
    ...
    // youtube.com أو youtube-nocookie.com
    const vParam = u.searchParams.get("v");
    if(vParam && /^[A-Za-z0-9_-]{6,}$/.test(vParam)) return vParam;

    const parts = u.pathname.split("/").filter(Boolean); // ["embed","ID"] أو ["shorts","ID"] أو ["live","ID"]
    const prefixIdx = parts.findIndex(p=>["embed","shorts","live"].includes(p));
    ...
    // رابط مش صالح كـURL كامل (نادر) — نرجع لمحاولة regex بسيطة كحل احتياطي
    // أخير بدل ما نفشل تمامًا.
    const m = url.match(/(?:[?&]v=|\/(?:embed|shorts|live)\/|youtu\.be\/)([A-Za-z0-9_-]{6,})/);
    return m ? m[1] : null;
  }
}
...
// بتاخد حد أقصى 40 فرصة وبتبعتهم في طلب AI واحد بس — مكالمة Gemini وحدة بالظبط
// في كل ضغطة زرار. لو عندك فرص أكتر من كده محتاجة تصنيف، دوس الزرار تاني
// بعد ما الأولانية تخلص وتتحدّث الشاشة (كل ضغطة = طلب واحد جديد، مش لفة تلقائية).
// ملحوظة: كانت فيه نسخة قديمة بتلف تلقائيًا على *كل* الدفعات في نفس الكبسة
// (لحد 40 طلب AI ورا بعض)، وده كان بيستهلك حصة الأدمن اليومية كاملة من ضغطة
// واحدة ويطلّع خطأ quota — رجّعناها لمكالمة واحدة بس عشان كده بالظبط.
const BULK_TAG_BATCH_SIZE = 40;
async function bulkFixOpportunityTags(){
  ...
  const untagged = state.opportunities.filter(o=>!(o.tags&&o.tags.length));
...
[{"id":"...","tags":["وسم1","وسم2"],"stageTags":["high","university"]}]`;
    // مكالمة Gemini واحدة بس — من غير أي إعادة محاولة على الكوتا (retries:0
    // بيعيد بس لو تايم أوت/شبكة، مش لو السيرفر رفض بكوتا، زي ما موضّح في callAiProxy).
    const result = await callAiProxy("", { contents:[{role:"user", parts:[{text:prompt}]}] }, {timeoutMs:60000, retries:0});
    if(!result.ok){
      state.adminMsg = result.message || "حصل خطأ أثناء التصنيف.";
    } else {
      const data = result.data;
      const cand = data && data.candidates && data.candidates[0];
      const text = cand && cand.content && cand.content.parts && cand.content.parts.map(p=>p.text||"").join("") || "";
      ...
      if(totalFixed===0){
        state.adminMsg = "الـAI مقدرش يصنّف الدفعة دي، جرب تاني أو راجعهم يدوي.";
      } else if(remaining===0){
        state.adminMsg = `تم تصنيف ${totalFixed} فرصة ✓ (كل الفرص بقى عندها تصنيف)`;
      } else {
        state.adminMsg = `تم تصنيف ${totalFixed} من ${totalAtStart} — دوس الزرار تاني عشان تصنّف الباقي (${remaining}).`;
      }
    }
  }catch(err){
    console.error("bulkFixOpportunityTags error:", err);
    state.adminMsg = "مقدرتش أوصل للخدمة دلوقتي، تأكد من الإنترنت وجرب تاني.";
  }finally{
    state.searchBusy=false; render();
    setTimeout(()=>{ state.adminMsg=""; render(); }, 6000);
  }
}
// كشف الفرص المكررة/الوهمية بقى قاعدة بيانات بحتة (Rule-based) من غير أي
// نداء Gemini خالص — مش محتاج ذكاء اصطناعي أصلًا: تطابق رابط، تطابق عنوان
// (حتى مع اختلاف بسيط في الصياغة عن طريق تشابه نصي)، أو نفس الجهة + نفس
// الموعد النهائي مع بعض. ده بيشتغل فورًا من غير ما يستهلك أي كوتا.
...
// كافي جدًا لكشف "نفس العنوان بصياغة مختلفة شوية".
...
const TITLE_SIMILARITY_THRESHOLD = 0.72; // نسبة تشابه العناوين اللي فوقها بنعتبرهم "نفس الفرصة"

// ============ زر "تحديث الفرص الآن" — بيشغّل GitHub Actions مباشرة ============
// الزرار ده معملش أي جمع فرص بنفسه — بيبعت طلب لـ/api/admin/trigger-aggregator
// اللي بدوره بيشغّل نفس الـGitHub Actions workflow (opportunity-aggregator.yml)
// اللي شغّال أصلًا يوميًا. النتيجة (فرص جديدة/مكررة/أخطاء) بتتقرا من
// meta/aggregatorLastRun في Firestore بعد ما الـworkflow يخلّص (بياخد دقيقة
// أو دقيقتين تقريبًا، مش فوري).
function renderAggregatorLastRun(){
  const r = state.aggregatorLastRun;
  if(!r) return `<div style="margin-top:8px;font-size:12.5px;color:#5C7278;">مفيش نتيجة تشغيلة محفوظة لسه.</div>`;
  const when = new Date(r.finishedAt).toLocaleString("ar-EG");
  const checked = r.connectorsChecked ?? r.sourcesChecked ?? 0; // توافق مع تقارير قديمة قبل إعادة الهيكلة
  return `<div style="margin-top:8px;font-size:12.5px;color:#5C7278;">
    آخر تشغيلة (${when}) — مصادر: ${checked}، جديدة: ${r.totalNew||0}، محدَّثة: ${r.totalUpdated||0}، مرفوضة: ${r.totalRejected||0}، تكرار محتمل: ${r.totalPossibleDuplicates||0}، أخطاء: ${r.totalErrors||0}
  </div>
  ${renderConnectorStatusTable()}`;
}
function renderConnectorStatusTable(){
  const connectors = state.connectorStatus;
  if(!connectors || !Object.keys(connectors).length) return "";
  const rows = Object.values(connectors).map(c=>{
    const stateLabel = {ok:"✅ شغال", down:"⚠️ فشل آخر تشغيلة", "circuit-open":"⛔ معطّل مؤقتًا"}[c.state] || c.state;
    const when = c.lastRunAt ? new Date(c.lastRunAt).toLocaleString("ar-EG") : "—";
    return `<tr>
      <td style="padding:4px 8px;">${escapeHtml(c.connectorId)}</td>
      <td style="padding:4px 8px;">${stateLabel}</td>
      <td style="padding:4px 8px;">${when}</td>
      <td style="padding:4px 8px;">${c.lastError ? escapeHtml(String(c.lastError)).slice(0,60) : "—"}</td>
    </tr>`;
  }).join("");
  return `<div style="margin-top:10px;overflow-x:auto;">
    <table style="width:100%;border-collapse:collapse;font-size:12px;">
      <thead><tr style="text-align:right;color:#5C7278;"><th style="padding:4px 8px;">المصدر</th><th style="padding:4px 8px;">الحالة</th><th style="padding:4px 8px;">آخر تشغيل</th><th style="padding:4px 8px;">آخر خطأ</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div>`;
}
async function loadAggregatorLastRun(){
  try{
    const snap = await getDoc(doc(db,"meta","aggregatorLastRun"));
    state.aggregatorLastRun = snap.exists() ? snap.data() : null;
  }catch(err){ console.error("loadAggregatorLastRun error:", err); }
...
async function loadConnectorStatus(){
  try{
    const idToken = state.user ? await state.user.getIdToken().catch(()=>null) : null;
    if(!idToken) return;
    const res = await fetch("/api/admin/connector-status", {
      headers:{"Authorization":"Bearer "+idToken}
    });
    const data = await res.json().catch(()=>({}));
    if(res.ok) state.connectorStatus = data.connectors || {};
  }catch(err){ console.error("loadConnectorStatus error:", err); }
}
async function triggerOpportunityAggregator(){
  if(!isAdmin() || state.aggregatorTriggering) return;
  state.aggregatorTriggering = true; state.aggregatorMsg = ""; render();
  try{
    const idToken = state.user ? await state.user.getIdToken().catch(()=>null) : null;
    if(!idToken){ throw Object.assign(new Error("مش مسجّل دخول"), {code:"no-auth"}); }
    const res = await fetch("/api/admin/trigger-aggregator", {
      method:"POST",
      headers:{"Content-Type":"application/json","Authorization":"Bearer "+idToken}
    });
    const data = await res.json().catch(()=>({}));
    if(res.ok && data.ok){
      state.aggregatorMsg = data.message || "تم تشغيل التحديث ✓";
    } else {
      state.aggregatorMsg = "❌ " + ((data.error && data.error.message) || "حصل خطأ أثناء التشغيل.");
    }
  }catch(err){
    console.error("triggerOpportunityAggregator error:", err);
    state.aggregatorMsg = "❌ حصل خطأ أثناء الاتصال بالسيرفر.";
  }
  state.aggregatorTriggering = false; render();
  // منستناش الـworkflow يخلّص فعليًا (بياخد دقيقة/دقيقتين)، فبنجرب نجيب آخر
  // نتيجة محفوظة بعد شوية تلقائيًا، مرتين، عشان الأدمن مايضطرش يعمل refresh يدوي.
  setTimeout(async ()=>{ await loadAggregatorLastRun(); render(); }, 60000);
  setTimeout(async ()=>{ await loadAggregatorLastRun(); render(); }, 120000);
}

async function cleanupFakeAndDuplicateOpportunities(){
  if(!isAdmin()) return;
  if(state.searchBusy){
    state.adminMsg = "فيه عملية تانية شغالة دلوقتي — استناها تخلص الأول.";
    render(); setTimeout(()=>{ state.adminMsg=""; render(); }, 4000); return;
  }
  state.searchBusy = true; state.adminMsg = "بيراجع الفرص (قاعدة بيانات محلية، من غير AI)..."; render();
  let removedExact = 0, removedSimilar = 0;
  try{
  // الخطوة 1: تكرار بنفس الرابط بالظبط — حذف مباشر وأكيد 100%.
  ...
  // الخطوة 2: تشابه بالقاعدة (عنوان قريب جدًا + نفس الجهة أو نفس الموعد
  // النهائي) — كل ده منطق محلي بسيط، مفيهوش أي طلب Gemini خالص.
  const remaining = [...state.opportunities];
  ...
  for(let i=0;i<remaining.length;i++){
    ...
    for(let j=i+1;j<remaining.length;j++){
      ...
      const a = remaining[i];
      const sameOrg = normalizeForCompare(a.organization) && normalizeForCompare(a.organization)===normalizeForCompare(b.organization);
      const sameDeadline = a.deadline && a.deadline===b.deadline && a.deadline!=="غير معلن";
      const titleSim = textSimilarity(a.title, b.title);
      if(titleSim >= TITLE_SIMILARITY_THRESHOLD && (sameOrg || sameDeadline)){
        toRemove.add(b.id); // نسيب أول واحدة (الأقدم) ونعتبر التانية مكررة
      }
    }
  }
  for(const id of toRemove){
    try{ await deleteDoc(doc(db,"opportunities", id)); removedSimilar++; }
    catch(err){ console.error("cleanup similar-dup delete error:", err); }
  }
  await loadOpportunities();
  const total = removedExact + removedSimilar;
  state.adminMsg = total>0
    ? `تم حذف ${total} فرصة مكررة (${removedExact} رابط مطابق + ${removedSimilar} عنوان/جهة متشابهة) ✓`
    : "مفيش فرص مكررة اتلاقت.";
  }catch(err){
    console.error("cleanupFakeAndDuplicateOpportunities outer error:", err);
    state.adminMsg = (removedExact+removedSimilar)>0 ? `اتحذف ${removedExact+removedSimilar} فرصة قبل ما يحصل خطأ غير متوقع.` : "حصل خطأ غير متوقع، جرب تاني.";
  }finally{
    state.searchBusy=false; render();
    setTimeout(()=>{ state.adminMsg=""; render(); }, 6000);
  }
...
app.addEventListener("click",(e)=>{
  ...
  else if(action==="trigger-aggregator"){ triggerOpportunityAggregator(); }
  ...
  else if(action==="religious-subtab"){ state.religiousSubTab = t.dataset.subtab; render(); }
  ...
  else if(action==="admin-edit-college-guide"){
    const c = state.collegesGuide.find(x=>x.id===t.dataset.id);
    if(c) state.adminCollegeGuideDraft = {...c};
    render();
  }
  else if(action==="cancel-college-guide-draft"){ state.adminCollegeGuideDraft = null; render(); }
  else if(action==="save-college-guide-draft"){ saveCollegeGuideDraft(); }
  else if(action==="admin-delete-college-guide"){ deleteCollegeGuideEntry(t.dataset.id); }
  else if(action==="open-scholarship-guide"){ state.openScholarshipGuideId = t.dataset.id; render(); }
  else if(action==="close-scholarship-guide"){ state.openScholarshipGuideId = null; render(); }
  else if(action==="sg-filter-level"){ state.scholarshipGuideFilterLevel = t.dataset.level; render(); }
  else if(action==="admin-new-scholarship-guide"){ state.adminScholarshipGuideDraft = {}; render(); }
  else if(action==="admin-edit-scholarship-guide"){
    const s = state.scholarshipsGuide.find(x=>x.id===t.dataset.id);
    if(s) state.adminScholarshipGuideDraft = {...s};
    render();
  }
  else if(action==="cancel-scholarship-guide-draft"){ state.adminScholarshipGuideDraft = null; render(); }
  else if(action==="save-scholarship-guide-draft"){ saveScholarshipGuideDraft(); }
  else if(action==="admin-delete-scholarship-guide"){ deleteScholarshipGuideEntry(t.dataset.id); }
  ...
  else if(action==="toggle-resource-hidden"){ toggleResourceHidden(t.dataset.id); }
...
app.addEventListener("submit",(e)=>{
  ...
  else if(e.target.id==="cg-search-form"){
    state.collegeGuideSearchQuery = document.getElementById("cg-search-input").value;
    render();
  }
  else if(e.target.id==="sg-search-form"){
    state.scholarshipGuideSearchQuery = document.getElementById("sg-search-input").value;
    render();
  }
...
try{
  onAuthStateChanged(auth, async (user)=>{
    ...
    await loadScholarshipsGuide();
          await loadPendingOpps();
          if(isAdmin()) await loadAggregatorLastRun();
          await loadQuizzes();
