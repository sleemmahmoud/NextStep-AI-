...
function certificateHtml(recipientName, itemLabel, itemTitle, bodyText, dateStr){
  // تصميم مستوحى من شهادات Coursera/Google الشهيرة: بانر علوي بالبراند، اسم
  // المتدرب وعنوان الكورس/الاختبار بيتكتبوا مباشرة من الكود (مش من نص الـAI)
  // عشان يبقوا مضمونين 100% صح، وجملة قصيرة من الـAI تحتهم كلمسة شخصية بس.
  const cornerStyle = (pos)=>{
    const map = {
      tl: "top:26px;left:26px;border-top:3px solid #C98A1F;border-left:3px solid #C98A1F;",
      tr: "top:26px;right:26px;border-top:3px solid #C98A1F;border-right:3px solid #C98A1F;",
      bl: "bottom:26px;left:26px;border-bottom:3px solid #C98A1F;border-left:3px solid #C98A1F;",
      br: "bottom:26px;right:26px;border-bottom:3px solid #C98A1F;border-right:3px solid #C98A1F;"
    };
    return `position:absolute;width:34px;height:34px;${map[pos]}`;
  };
  return `
  <div style="width:1200px;height:850px;background:#FBFAF7;position:relative;box-sizing:border-box;font-family:'Cairo',Tahoma,Arial,sans-serif;color:#1A2B2E;">
    <div style="position:absolute;inset:0;border:1px solid #D9C79A;margin:14px;"></div>
    <div style="${cornerStyle('tl')}"></div>
    <div style="${cornerStyle('tr')}"></div>
    <div style="${cornerStyle('bl')}"></div>
    <div style="${cornerStyle('br')}"></div>

    <div style="position:absolute;top:0;left:0;right:0;height:86px;background:linear-gradient(90deg,#0A2C35,#0E3A45);display:flex;align-items:center;justify-content:center;">
      <div style="font-family:'Montserrat',sans-serif;font-weight:800;font-size:24px;letter-spacing:3px;color:#fff;">NextStep AI</div>
    </div>

    <div style="position:absolute;top:150px;left:0;right:0;text-align:center;padding:0 90px;">
      <div style="font-size:13px;letter-spacing:4px;color:#C98A1F;font-weight:700;">CERTIFICATE OF COMPLETION</div>
      <div style="font-size:40px;font-weight:800;margin:10px 0 4px;">شهادة إتمام</div>
      <div style="width:90px;height:3px;background:#C98A1F;margin:0 auto 28px;"></div>

      <div style="font-size:15px;color:#5B6B6E;">تُمنح هذه الشهادة إلى</div>
      <div style="font-family:'Amiri','Cairo',serif;font-size:44px;font-weight:800;color:#0A2C35;margin:10px 0;">${escapeHtml(recipientName)}</div>
      <div style="width:340px;height:2px;background:#C98A1F;margin:0 auto 22px;"></div>

      <div style="font-size:16px;color:#3A4749;line-height:1.7;">تقديرًا لإتمامه بنجاح ${itemLabel} <strong>"${escapeHtml(itemTitle)}"</strong></div>
      <div style="font-size:14.5px;color:#5B6B6E;max-width:760px;margin:12px auto 0;line-height:1.8;">${escapeHtml(bodyText)}</div>
    </div>

    <div style="position:absolute;bottom:52px;left:70px;text-align:center;">
      <div style="font-size:13px;font-weight:700;color:#1A2B2E;">${dateStr}</div>
      <div style="width:130px;height:2px;background:#D9C79A;margin:6px auto 4px;"></div>
      <div style="font-size:12px;color:#5B6B6E;">تاريخ الإصدار</div>
    </div>

    <div style="position:absolute;bottom:36px;left:50%;transform:translateX(-50%);width:92px;height:92px;border-radius:50%;background:radial-gradient(circle at 32% 30%, #F3C267, #B87F1D);display:flex;align-items:center;justify-content:center;box-shadow:0 0 0 4px #FBFAF7, 0 0 0 6px #C98A1F;">
      <div style="position:absolute;bottom:-16px;left:16px;width:14px;height:30px;background:#0A2C35;transform:rotate(18deg);"></div>
      <div style="position:absolute;bottom:-16px;right:16px;width:14px;height:30px;background:#0A2C35;transform:rotate(-18deg);"></div>
      <div style="font-size:34px;color:#fff;">✓</div>
    </div>

    <div style="position:absolute;bottom:52px;right:70px;text-align:center;">
      <div style="font-family:'Brush Script MT','Segoe Script',cursive;font-size:30px;color:#C98A1F;">Saleem Mahmoud</div>
      <div style="width:170px;height:2px;background:#D9C79A;margin:4px auto 6px;"></div>
      <div style="font-size:12px;color:#5B6B6E;">سليم محمود — المدرب ومؤسس المنصة</div>
    </div>
  </div>`;
}
...
async function submitAdminResource(form){
  ...
  // اختيار القسم صريح من الأدمن (نصائح سريعة / كورسات / نصائح عملية) — لو مالوش
  // قيمة لأي سبب، نرجع لسلوك افتراضي منطقي حسب النوع (كورس => قسم الكورسات، غيره => نصائح سريعة).
  const section = fd.get("section") || (type==="course" ? "courses" : "tips");
...
function renderAdminResourceForm(){
  ...
  <form id="admin-res-form">
    ...
    <div class="field"><label>هيتحط في أي قسم بمركز التعلّم؟</label><select name="section">
      <option value="tips">💡 نصائح سريعة (فيديوهات قصيرة)</option>
      <option value="courses">🎓 الكورسات</option>
      <option value="practical">🛠️ نصائح عملية (فيديوهات طويلة)</option>
    </select></div>
