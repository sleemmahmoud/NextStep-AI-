...
export const CONFIG = {
  ...
  // 🆕 حالة كل Connector (لصفحة المراقبة في لوحة الإدارة) — مستند واحد فيه
  // آخر حالة لكل مصدر.
  CONNECTOR_HEALTH_DOC: "meta/connectorHealth",

  // 🆕 لو Connector فشل بالكامل (كل محاولاته) في كذا تشغيلة يومية متتالية،
  // بيتحط "معطّل مؤقتًا" (Circuit Breaker) بدل ما يحاول ويفشل بصمت كل يوم.
  CIRCUIT_BREAKER_CONSECUTIVE_FAILURES: 3,
  CIRCUIT_BREAKER_COOLDOWN_HOURS: 24,

  // 🆕 أقصى عدد فرص موجودة بالفعل (بنفس التصنيف) بيتقارنوا مع كل فرصة
  // جديدة عشان نكتشف تكرار محتمل بين مصادر مختلفة (قسم 5 في الوثيقة) —
  // سقف عشان منعملش مقارنة مكلفة على آلاف المستندات في كل تشغيلة.
  DEDUP_CANDIDATES_LIMIT: 300,
} as const;
