...
const DAILY_LIMIT = 5; // نفس الرقم المستخدم في app.js، غيّرهم مع بعض
...
async function checkAndConsumeQuota(uid: string | undefined, email: string | undefined) {
  if (email && ADMIN_EMAILS.includes(email)) {
    return { allowed: true }; // الأدمن مالوش حد يومي خالص، من غير ما نعدّ عليه أو نلمس الـKV
  }
  if (!uid) {
...
const COURSE_SEARCH_TOPICS = [
  "كورسات برمجة مجانية",
  "كورسات تصميم UX/UI مجانية",
  "كورسات تسويق رقمي مجانية",
  "كورسات لغة إنجليزية مجانية",
  "كورسات ريادة أعمال وإدارة مشاريع مجانية",
  "كورسات علوم بيانات وذكاء اصطناعي مجانية",
  "كورسات مهارات تواصل وقيادة مجانية",
];

function buildCoursePrompt(topic: string): string {
  const todayStr = new Date().toISOString().slice(0, 10);
  return `النهاردة تاريخ ${todayStr}. استخدم بحث جوجل الحقيقي والحي دلوقتي عشان تلاقي أشهر وأهم الكورسات المجانية أو شبه المجانية **الحقيقية والمتاحة فعلاً دلوقتي** (بحد أقصى 10 كورسات) عن موضوع: "${topic}"، مناسبة لطلاب أو خريجين مصريين.
...
شروط أساسية:
- افتح فعليًا (عن طريق البحث) صفحة كل كورس، وتأكد إنه لسه متاح ومنشور فعلاً دلوقتي مش صفحة اتشالت.
- الرابط لازم يكون رابط حقيقي مباشر لصفحة الكورس نفسه من نتيجة البحث، مش رابط عام للمنصة.
- "isPremium": true لو المحتوى نفسه محتاج اشتراك مدفوع، أو false لو مجاني بالكامل أو متاح بنظام Audit مجاني.
- صنّف كل كورس بـ"tags" من القايمة دي بالظبط: [${TAGS.map((t) => `"${t}"`).join(", ")}]
- الوصف بالعربي دايمًا حتى لو الكورس بالإنجليزي.
رجّع بس JSON array بالشكل ده بالظبط: [{"title":"عنوان الكورس + اسم المنصة بين قوسين","description":"وصف قصير بالعربي","link":"رابط صفحة الكورس المباشر","tags":["وسم1"],"isPremium":false}]
لو مش متأكد إن الكورس حقيقي ومتاح فعلًا دلوقتي، متضيفهوش خالص. لو مفيش كورسات موثوقة كفاية رجّع [].`;
}

// deno-lint-ignore no-explicit-any
async function fetchCoursesForTopic(topic: string): Promise<any[]> {
  const res = await callGemini({
    contents: [{ role: "user", parts: [{ text: buildCoursePrompt(topic) }] }],
    tools: [{ google_search: {} }],
  });
  if (!res.ok) {
    console.error(`[auto-search-courses] Gemini error for topic "${topic}": ${res.status}`);
    return [];
  }
  const data = await res.json();
  const cand = data?.candidates?.[0];
...
// كل مواضيع الكورسات، تتأكد إن كل حاجة مش موجودة قبل كده (عشان منكررش)،
// وتنشرها لايف في opportunities أو resources حسب النوع.
async function runAutoSearch() {
  ...
  let coursesAdded = 0, coursesSkipped = 0;
  for (const topic of COURSE_SEARCH_TOPICS) {
    const items = await fetchCoursesForTopic(topic);
    for (const it of items) {
      try {
        if (isValidLink(it.link) && (await firestoreLinkExists(it.link, token, "resources"))) {
          coursesSkipped++;
          continue;
        }
        await createFirestoreDoc({
          title: it.title || "كورس بدون عنوان",
          type: "course",
          description: it.description || "",
          link: it.link || "",
          tags: Array.isArray(it.tags) ? it.tags.filter((t: string) => TAGS.includes(t)) : [],
        ...
        console.error(`[auto-search-courses] فشل حفظ كورس من موضوع "${topic}":`, err);
      }
    }
  }
  ...
  console.log(`[auto-search] خلص. فرص: اتضاف ${added}، اتجاهل ${skipped} مكررة. كورسات: اتضاف ${coursesAdded}، اتجاهل ${coursesSkipped} مكررة.`);
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const quota = await checkAndConsumeQuota(uid, email);
