...
async function firestoreLinkExists(link: string, token: string): Promise<boolean> {
  const res = await fetch(
    `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents:runQuery`,
    {
      ...
      body: JSON.stringify({
        structuredQuery: {
          from: [{ collectionId: "opportunities" }],
          where: {
            fieldFilter: {
...
async function createFirestoreDoc(
  ...
  await fetch(
    `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents/opportunities`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
...
function buildPrompt(topic: string): string {
  ...
  return `النهاردة تاريخ ${todayStr} (يعني إحنا في سنة ${currentYear}). من معرفتك (من غير بحث حي على الإنترنت)، اقترح لي أشهر وأهم الفرص **الحقيقية والمعروفة والمتكررة سنويًا** (بحد أقصى 15 فرصة) اللي مناسبة لموضوع: "${topic}"، وتستهدف طلاب أو خريجين مصريين أو فرص دولية متاحة لهم.
...
شروط أساسية: رشّح بس برامج مستقرة ومعروفة إنها بتتكرر كل سنة. متختلقش اسم برنامج أو جهة مش متأكد من وجودها الحقيقي. متكتبش deadline إلا لو متأكد، وإلا اكتب "غير معلن - راجع الموقع الرسمي". الرابط لازم يكون الموقع الرسمي المعروف للجهة.
رجّع بس JSON array، من غير أي نص تاني قبله أو بعده، بالشكل ده بالظبط:
[{"title":"عنوان الفرصة","organization":"اسم الجهة","description":"وصف قصير من سطرين بالعربي","category":"scholarship أو internship أو job أو volunteering أو competition أو conference أو hackathon أو exchange أو course أو bootcamp أو event","deadline":"YYYY-MM-DD أو: غير معلن - راجع الموقع الرسمي","link":"رابط الموقع الرسمي المعروف للجهة"}]
لو مش متأكد إن الفرصة دي حقيقية وموجودة فعلًا، متضيفهاش خالص. لو مفيش فرص معروفة كفاية رجّع [].`;
}

// deno-lint-ignore no-explicit-any
async function fetchOpportunitiesForTopic(topic: string): Promise<any[]> {
  const res = await callGemini({ contents: [{ role: "user", parts: [{ text: buildPrompt(topic) }] }] });
  if (!res.ok) {
  ...
  const sourceHosts = [...new Set(sources.map((s: { uri?: string; url?: string }) => hostnameOf(s.uri || s.url || "")).filter(Boolean))];
...
// الدالة الرئيسية اللي بيشغّلها الـcron — بتلف على كل المواضيع، تتأكد إن كل
// فرصة مش موجودة قبل كده (عشان منكررش نفس الفرص كل يوم)، وتنشرها لايف.
async function runAutoSearch() {
  ...
  for (const topic of AUTO_SEARCH_TOPICS) {
    ...
    for (const it of items) {
      try {
        if (isValidLink(it.link) && (await firestoreLinkExists(it.link, token))) {
          skipped++;
          continue;
        }
  ...
  console.log(`[auto-search] خلص. اتضاف ${added} فرصة جديدة، اتجاهل ${skipped} مكررة.`);
}
