...
async function callGemini(geminiBody: unknown) {
  ...
  console.log(`[gemini] رد بعد ${Date.now() - startedAt}ms — status ${res.status}`);
  return res;
}
...
async function callGeminiWithRetry(geminiBody: unknown, maxRetries = 1): Promise<{ res: Response; text: string }> {
  let res: Response, text: string;
  for (let attempt = 0; ; attempt++) {
    res = await callGemini(geminiBody);
    text = await res.text();
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { uid, email, ...geminiBody } = parsedBody;
    ...
    const { res: googleRes, text: data } = await callGeminiWithRetry(geminiBody);
    if (googleRes.ok) {
