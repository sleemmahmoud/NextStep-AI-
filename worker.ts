...
// كل ميزة بتستخدم الـAI ليها عداد يومي منفصل تمامًا عن باقي الميزات. ده بيمنع
// أي ميزة إنها تاكل من كوتا ميزة تانية، خصوصًا إن "تصنيف الفرص القديمة" و"كشف
// وحذف الفرص المكررة/الوهمية" بيشتغلوا على دفعات (ممكن يبعتوا عشرات الطلبات
// في نفس الكبسة)، فمعقول جدًا يستهلكوا كل كوتا اليوم لو فضلوا مشتركين مع
// الشات أو البحث اليدوي.
// - chat: المساعد الذكي (شات).
// - search: البحث اليدوي عن فرص جديدة بالـAI.
// - classify: تصنيف الفرص القديمة تلقائيًا (bulkFixOpportunityTags) — دفعات.
// - cleanup: كشف وحذف الفرص المكررة/الوهمية تلقائيًا — دفعات هي كمان.
// - cv: تحسين صياغة الـCV + متن الشهادة.
// search/classify/cleanup أدوات أدمن بس أصلًا (isAdmin() في app.js)، فحد
// المستخدم العادي فيها صفر كإجراء أمان إضافي حتى لو حصل تلاعب من الفرونت إند.
const FEATURE_LIMITS: Record<string, { user: number; admin: number }> = {
  chat: { user: 5, admin: 5 },
  search: { user: 0, admin: 8 },
  classify: { user: 0, admin: 12 },
  cleanup: { user: 0, admin: 12 },
  cv: { user: 2, admin: 6 },
};
