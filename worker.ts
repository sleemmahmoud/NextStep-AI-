...
// ده الموديل الافتراضي (البحث التلقائي اليومي وأي طلب عادي بيستخدمه دايمًا).
const DEFAULT_GEMINI_MODEL = "gemini-3.5-flash";
// الموديلات اللي الأدمن بس يقدر يختار بينها من لوحة تحكم البحث اليدوي (مش
// البحث التلقائي — ده فاضل ثابت على DEFAULT_GEMINI_MODEL دايمًا عشان الاستقرار).
// لو Google أطلقت موديل فري تير جديد، ضيفه هنا وهيظهر تلقائي في القايمة.
const ALLOWED_MANUAL_MODELS = ["gemini-3.5-flash", "gemini-2.5-flash", "gemini-2.0-flash"];
...
// حد يومي واحد بسيط لكل مستخدم (وواحد تاني أعلى بكتير للأدمن) — بيتحسب على كل
// استخدامات الـAI مع بعض من غير أي تقسيم بميزات، عشان النظام يفضل أبسط وأسهل
// في المتابعة. المساعد الذكي (الشات) بقى محلي 100% ومبيستهلكش من الحصة دي
// خالص، فحد المستخدم العادي دلوقتي بيغطي بس محاولات تحسين الـCV بالـAI (اتقلل
// أكتر لإن الشات مش محتاجه خالص دلوقتي). الأدمن محتاج حد أعلى بكتير عشان
// عمليات البحث بالـAI وتصنيف/تنظيف الفرص بتبعت كذا طلب متتالي (دفعة لكل 50-60
// فرصة) — لو الحد قليل هتفشل نص العملية في النص.
const DAILY_LIMIT = 2;
...
async function checkQuota(uid: string | undefined, email: string | undefined): Promise<QuotaCheckResult> {
  ...
  const key: Deno.KvKey = ["aiUsage", idKey, today];
  const current = (await kv.get<number>(key)).value ?? 0;
  if (current >= limit) return { allowed: false, limit, idKey, key, current };
  return { allowed: true, limit, idKey, key, current };
}
...
async function callGemini(geminiBody: unknown, model: string = DEFAULT_GEMINI_MODEL) {
  ...
  // لوج زمن استجابة Gemini الفعلي — بيساعد نفرّق بين "Gemini نفسه بطيء" و"مشكلة
  // في المتصفح/الشبكة بتاعة اليوزر" لو حصلت شكوى استقرار تانية في المستقبل.
  console.log(`[gemini] رد بعد ${Date.now() - startedAt}ms — status ${res.status} — موديل ${model}`);
  return res;
}

// بيدوّر على retryDelay اللي Google نفسها بترجعه جوه رد الـ429 (RetryInfo) —
// ده وقت مؤكد من Google نفسها لحد ما الـrate limit المؤقت (RPM) يفضى، وده
// مختلف تمامًا عن الكوتا اليومية اللي بترجع بعد ساعات مش ثواني. بنستخدمه بدل
// ما نخمّن مدة الانتظار.
function extractRetryDelaySeconds(bodyText: string): number | null {
  try {
    const data = JSON.parse(bodyText);
    // deno-lint-ignore no-explicit-any
    const details = data?.error?.details as any[] | undefined;
    if (Array.isArray(details)) {
      const retryInfo = details.find((d) => typeof d?.["@type"] === "string" && d["@type"].includes("RetryInfo"));
      const raw = retryInfo?.retryDelay; // شكلها بالظبط "5.097709175s"
      if (typeof raw === "string") {
        const n = parseFloat(raw.replace("s", ""));
        if (!isNaN(n)) return n;
      }
    }
  } catch { /* مش JSON صحيح أو مفيش تفاصيل retry — نتجاهل ونستخدم قيمة افتراضية */ }
  return null;
}

// نداء Gemini + إعادة محاولة تلقائية واحدة لو الرد كان 429 (rate limit مؤقت
// بيتصفر خلال ثواني — زي اللي بيحصل لو أكتر من مستخدم/الـcron ضربوا Gemini في
// نفس اللحظة تقريبًا)، بدل ما نفشل الطلب فورًا ونعرض رسالة الكوتا للمستخدم من
// غير داعي. بنستنى بالظبط المدة اللي Google قالتها (بحد أقصى 10 ثواني عشان
// الطلب ككل ميستناش أكتر من اللازم على حساب مهلة المتصفح). لو لسه 429 بعد
// المحاولة التانية، نرجّع الرد زي ما هو (فيه رسالة Google الحقيقية) للطبقة
// اللي فوق تقرر، بدل ما نستهلك وقت أكتر في محاولات هتفشل غالبًا.
async function callGeminiWithRetry(
  ...
  for (let attempt = 0; ; attempt++) {
    res = await callGemini(geminiBody, model);
    text = await res.text();
    if (res.status !== 429 || attempt >= maxRetries) break;
    const delaySec = extractRetryDelaySeconds(text) ?? 3;
    const waitMs = Math.min(delaySec, 10) * 1000;
    console.log(`[gemini] 429 مؤقت — بينتظر ${waitMs}ms ويعيد المحاولة (${attempt + 1}/${maxRetries})...`);
...
async function fetchOpportunitiesForTopic(topic: string): Promise<any[]> {
  const { res, text: rawText } = await callGeminiWithRetry({
...
async function runAutoSearch() {
  ...
  for (const topic of AUTO_SEARCH_TOPICS) {
    ...
    // فاصل زمني بين كل موضوع والتاني (5 ثواني) — بحساب الجولة كلها كانت بتضرب
    // Gemini بـ9 نداءات ورا بعض من غير أي استراحة، وده كان بيقرّب جدًا من حد
    // الـrate limit المجاني (اللي بيتصفر خلال ثواني قليلة أصلًا) قبل حتى ما أي
    // مستخدم حقيقي يستخدم الشات أو البحث في نفس اليوم.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { uid, email, action, model: requestedModel } = parsedBody;
    // بدل ما نستثني email/uid/action/model من اللي بيتبعت لـGemini (طريقة قديمة
    // بتعتمد إننا محددين كل حقل غريب ممكن يجي)، بقينا نبني جسم الطلب لـGemini
    // بقايمة سماح صريحة: أي حقل مش من دول بيتشال تلقائيًا مهما كان اسمه —
    // فمستحيل تتسرب email أو أي حاجة تانية تكسر الطلب زي ما حصل قبل كده.
    ...
    // action:"quotaStatus" — بيرجّع الاستهلاك اليومي الحالي بس، من غير ما يكلم
    // Gemini أو يستهلك أي حصة، عشان لوحة تحكم البحث في app.js تعرض "مستخدم
    // كذا من كذا" للأدمن قبل ما يضغط دور على فرص.
    if (action === "quotaStatus") {
      const quota = await checkQuota(uid, email);
      ...
      return new Response(
        JSON.stringify({ limit: quota.limit, used: quota.current, remaining: (quota.limit ?? 0) - (quota.current ?? 0) }),
        { headers: { ...CORS_HEADERS, "Content-Type": "application/json" } },
      );
    }
    ...
    if (!quota.allowed) {
      ...
      return quotaExceededResponse(`وصلت للحد اليومي لاستخدام الذكاء الاصطناعي (${quota.limit} طلب في اليوم)، هيتصفر تلقائيًا بكرة.`);
    }

    // الموديل قابل للاختيار بس للأدمن، وبس من القايمة المسموحة — أي حد تاني أو
    // أي قيمة غريبة بيترجع تلقائيًا للموديل الافتراضي الثابت من غير ما يفشل الطلب.
    const effectiveModel = (isAdminUser && typeof requestedModel === "string" && ALLOWED_MANUAL_MODELS.includes(requestedModel))
      ? requestedModel
      : DEFAULT_GEMINI_MODEL;

    console.log(`[gemini] طلب جديد — id=${quota.idKey} (${quota.current}/${quota.limit} النهاردة) — موديل ${effectiveModel}`);
    const { res: googleRes, text: data } = await callGeminiWithRetry(geminiBody, 1, effectiveModel);
    if (googleRes.ok) {
