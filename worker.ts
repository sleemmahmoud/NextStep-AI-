...
const DAILY_LIMIT = 3; // نفس الرقم المستخدم في app.js، غيّرهم مع بعض — قللناه عشان الكوتا المجانية
const ADMIN_DAILY_LIMIT = 8; // الأدمن ليه حد أعلى شوية بس برضه محدود، مش لا نهائي — نفس الرقم لازم يفضل مطابق لـADMIN_AI_DAILY_LIMIT في app.js
const GEMINI_MODEL = "gemini-3.6-flash"; // موديل جديد أرخص وأحسن، طلعته Google في 21 يوليو 2026
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!quota.allowed) {
      const limitShown = (email && ADMIN_EMAILS.includes(email)) ? ADMIN_DAILY_LIMIT : DAILY_LIMIT;
      return quotaExceededResponse(`وصلت للحد اليومي لاستخدام الذكاء الاصطناعي (${limitShown} طلب في اليوم)، هيتصفر تلقائيًا بكرة.`);
    }
